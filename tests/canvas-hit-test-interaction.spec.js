...
async function stepInside(page) {
    ...
    await page.waitForFunction(
        () => {
            const appState = window.__APP_STATE__ ?? window.__TEST_STATE__ ?? {}
            return appState?.trailDepth === 2 && appState?.semanticDiveMode === true
        },
        { timeout: 15000 }
    )
}
...
test.describe('canvas hit-test: proving canvas does not intercept UI clicks', () => {
    ...
    test('desktop: Escape after Step Inside resets state (keyboard path preserved, not canvas-blocked)', async ({
        page
    }) => {
        test.setTimeout(120000)
        await openApp(page, { width: 1440, height: 900 })
    ...
    test('mobile: Escape after Step Inside resets state (mobile keyboard path not canvas-blocked)', async ({
        page
    }) => {
        test.setTimeout(120000)
        await openApp(page, { width: 390, height: 844 })
    ...
    test('tablet: search-input and clear button both receive real clicks (canvas not blocking 768px)', async ({
        page
    }) => {
        test.setTimeout(120000)
     ...
     */
    test('mobile focus-search: journey-compass is reachable via elementsFromPoint (canvas not blocking)', async ({
        page
    }) => {
        test.setTimeout(120000)
        await openApp(page, { width: 390, height: 844 })
        ...
        const hitResult = await page.evaluate(() => {
            ...
            const isTopmostCompass = Boolean(topmost && (topmost === compass || topmost.closest?.('.journey-compass')))
        ...
        expect(
            hitResult.isTopmostCompass,
            'journey-compass must be the topmost element at its center; canvas must not intercept'
        ).toBe(true)
        expect(hitResult.isBlockedByCanvas, 'canvas must NOT be in the hit-test stack for journey-compass center').toBe(
            false
        )
        expect(hitResult.canvasPointerEvents, 'canvas-container pointer-events must be none in focus-search').toBe(
            'none'
        )
    })

    test('mobile focus-search: Step Inside control is reachable via elementsFromPoint (canvas not blocking)', async ({
        page
    }) => {
        test.setTimeout(120000)
        await openApp(page, { width: 390, height: 844 })
        ...
        const hitResult = await page.evaluate(() => {
            ...
            const isTopmostDiveBtn = Boolean(
                topmost &&
                (topmost === diveBtn ||
    ...
    test('short-landscape: btn-focus-dive click activates semantic dive at 844x390', async ({ page }) => {
        ...
        const state = await probe(page)
        expect(state.state.semanticDiveMode, 'Step Inside should activate semanticDiveMode at short-landscape').toBe(
            true
        )
