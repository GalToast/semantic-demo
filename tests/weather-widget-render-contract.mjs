...
async function waitForWeatherApi(page) {
    ...
    await page.waitForFunction(() => Boolean(window.state && window._weather?.updateWeatherUi), null, { timeout: 8000 });
}
...
async function injectLiveWeather(page) {
    await page.evaluate(() => {
        ...
        window.state.lastSuccessfulFetch = Date.now();
...
async function main() {
    ...
    try {
        ...
        await page.evaluate(() => {
            ...
            window.state.lastSuccessfulFetch = 0;
            window._weather.updateWeatherUi();
        });
