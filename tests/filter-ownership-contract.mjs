...
class FakeElement {
    ...
    click() {
        if (this.onclick) this.onclick({ target: this, currentTarget: this })
    }
}
...
assert(filterStateSource.includes('export function setActiveFilter'), 'filter-state.ts must export setActiveFilter')
assert(
...
assert(activeFiltersWriters.length > 0, 'filter-state.ts must contain activeFilters writes')
assert(activeClusterWriters.length > 0, 'filter-state.ts must contain activeClusterFilter writes')

console.log(
    `PASS CONTRACT 2: filter-state.ts owns all activeFilters writes (${activeFiltersWriters.length}) and activeClusterFilter writes (${activeClusterWriters.length})`
)
...
const CHROME_CHAIN = [MODULE_PATHS['event-bindings.ts']].filter((p) => fs.existsSync(p))
...
function chromeChainImportsFilterStateApi(path) {
    ...
    if (path.endsWith('.svelte')) {
        return (
            FILTER_STATE_API_NAMES.every((name) => source.includes(name)) &&
            source.includes('@lib/stores/filter.svelte')
        )
    }
    return scanImports(path).some(
        (imp) =>
...
if (CHROME_CHAIN.length === 0) {
    ...
    console.log(
        `PASS CONTRACT 5: filter chrome chain delegates to filter-state owner APIs (${chainWithImports.length}/${CHROME_CHAIN.length} files import the owner APIs, 0 direct writes)`
    )
}
...
const usRestoreImports = usImports.filter(
    (i) =>
...
const ssFilterReexport = ssImports.filter(
    (i) => i.from.includes('filter.svelte') && i.spec.includes('pointMatchesActiveFilters')
)
assert(ssFilterReexport.length > 0, 'search-state.ts must import pointMatchesActiveFilters from the filter owner API')
