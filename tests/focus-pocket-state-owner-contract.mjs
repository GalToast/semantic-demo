...
const ownerRanges = ownerFunctionNames
    .map((name) => {
...
for (const file of listJsFiles(MODULES_DIR)) {
    ...
    const directWrite =
        /state\.navState\.focusPocket(?:Indices|Meta|RoleByIndex)\s*=|state\.pocketMotionByIndex\s*=|state\.navState\.focusPocketRoleByIndex\.set\(|state\.pocketMotionByIndex\.set\(/g
    let match
...
if (
    !source.includes('// === Focus Pocket Owner API ===') &&
