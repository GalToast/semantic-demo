...
test('isDemoActive excludes COMPLETE state', () => {
    const hasCorrectLogic =
        demoStoreSource.includes("phase !== 'IDLE' && phase !== 'COMPLETE' && phase !== 'CANCELLED'") &&
...
test('cancelDemo has guard for IDLE, COMPLETE, CANCELLED', () => {
    const hasGuard =
        cancelDemoSource.includes("if (phase === 'IDLE' || phase === 'COMPLETE' || phase === 'CANCELLED')") &&
...
test('cancelDemo returns false in COMPLETE state', () => {
    const hasTerminalCheck =
        cancelDemoSource.includes("phase === 'COMPLETE'") && cancelDemoSource.includes('return false')
...
test('cancelDemo returns false in CANCELLED state', () => {
    const hasCancelledCheck =
        cancelDemoSource.includes("phase === 'CANCELLED'") && cancelDemoSource.includes('return false')
...
test('cancelDemo returns false in IDLE state', () => {
    const hasIdleCheck = cancelDemoSource.includes("phase === 'IDLE'") && cancelDemoSource.includes('return false')
...
test('cancelDemo has explanatory comment', () => {
    const hasComment =
        cancelDemoSource.includes('Mirror the legacy choreography guard') &&
