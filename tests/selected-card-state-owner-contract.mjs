...
// Proves no runtime module writes appState.selectedPoint outside
...
//     fade animation, document.title reset).
//
...
//     callbacks that copy next.selectedBusiness into
...
const WRITE_REGEX = /(?<![?])(\bappState|\bstate)\.selectedPoint\s*=(?!=)/g
...
for (const file of listTsFiles(MODULES_DIR)) {
    ...
    while ((match = WRITE_REGEX.exec(fileSource)) !== null) {
        ...
        console.error(
            `FAIL: ${relative(join(__dirname, '..'), file)}:${lineNum} writes selectedPoint outside owner/mirror:\n  ${line}`
        )
        failures += 1
    }
}
...
if (!/\bselectedPoint\s*:\s*Point\s*\|\s*null/.test(stateTypesSource)) {
    console.error('FAIL: state-types.ts does not declare selectedPoint: Point | null')
    failures += 1
}

const appStateSource = readFileSync(APP_STATE_PATH, 'utf8')
if (!/selectedPoint\s*=\s*\$state/.test(appStateSource)) {
    console.error('FAIL: app.svelte.ts does not declare selectedPoint = $state')
    failures += 1
}
...
// the focus store via focusOnPoint). The mirror module should have 2
// writes inside .update((s) => ...) callbacks.
