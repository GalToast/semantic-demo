...
// ── 1. api/config.php ──────────────────────────────────────────────────────
{
  ...
  assert(
    src.includes('function _env('),
    'api/config.php must define a _env() helper for env-driven lookups'
  );
  ...
  assert(
    /EMBED_SERVICE_HEALTH_URL/.test(src),
    'api/config.php: must support EMBED_SERVICE_HEALTH_URL env override'
  );
  assert(
    /ASK_MOCO_HEALTH_URL/.test(src),
    'api/config.php: must support ASK_MOCO_HEALTH_URL env override'
  );
...
// ── 2. deploy.sh ──────────────────────────────────────────────────────────
{
  ...
  assert(
    /DEPLOY_SSH_TARGET/.test(src),
    'deploy.sh: must support DEPLOY_SSH_TARGET env override'
  );
  assert(
    /DEPLOY_REMOTE_DIR/.test(src),
    'deploy.sh: must support DEPLOY_REMOTE_DIR env override'
  );
  assert(
    /DEPLOY_REMOTE_ROOT/.test(src),
    'deploy.sh: must support DEPLOY_REMOTE_ROOT env override'
  );
  assert(
    /DEPLOY_PORT/.test(src),
    'deploy.sh: must support DEPLOY_PORT env override'
  );

  // The hardcoded default path should still be present as a fallback.
  assert(
    src.includes('/home/u741831384'),
    'deploy.sh: must retain /home/u741831384 as backward-compatible default'
  );
...
// ── 3. deploy.ps1 ─────────────────────────────────────────────────────────
{
  ...
  assert(
    /DEPLOY_PORT|env:DEPLOY_PORT/.test(src),
    'deploy.ps1: must support DEPLOY_PORT env override'
  );

  // The hardcoded default path should still be present as a fallback.
  assert(
    src.includes('/home/u741831384'),
    'deploy.ps1: must retain /home/u741831384 as backward-compatible default'
  );
...
// ── 5. .env.example documents SEMANTIC_SERVICE_HOME ────────────────────────
{
  ...
  assert(
    /SEMANTIC_SERVICE_HOME/.test(src),
    '.env.example must document the SEMANTIC_SERVICE_HOME override'
  );
  assert(
    /DEPLOY_SSH_TARGET/.test(src),
    '.env.example must document the DEPLOY_SSH_TARGET override'
  );
...
// ── 6. deploy.sh ↔ deploy.ps1 alignment ────────────────────────────────────
{
  ...
  assert(
    sh.includes('leadEnrichment'),
    'deploy.sh: must deploy leadEnrichment.public.json'
  );
  assert(
    ps.includes('leadEnrichment'),
    'deploy.ps1: must deploy leadEnrichment.public.json (drift from deploy.sh)'
  );
  ...
  assert(
    /\$BACKUP_DIR\/scripts/.test(sh),
    'deploy.sh: backup mkdir must include scripts/ dir'
  );
  assert(
    /\$BackupDir\/scripts/.test(ps),
    'deploy.ps1: backup mkdir must include scripts/ dir'
  );

  // Scanner source must be env-overridable in both scripts.
  assert(
    /DEPLOY_SCANNER_SOURCE/.test(sh),
    'deploy.sh: scanner source must be env-overridable via DEPLOY_SCANNER_SOURCE'
  );
  assert(
    /DEPLOY_SCANNER_SOURCE/.test(ps),
    'deploy.ps1: scanner source must be env-overridable via DEPLOY_SCANNER_SOURCE'
  );
