...
await test('setTrailDepth mirrors journey, nav, and legacy state', () => {
  ...
  assert(/updateNavState\s*\(\s*\{\s*trailDepth:\s*nextDepth\s*\}\s*\)/.test(body), 'setTrailDepth mirrors navStore.trailDepth');
  assert(/appState\.trailDepth\s*=\s*nextDepth/.test(body), 'setTrailDepth mirrors legacy top-level trailDepth');
  assert(/appState\.navState\.trailDepth\s*=\s*nextDepth/.test(body), 'setTrailDepth mirrors legacy navState.trailDepth');
...
await test('setMyceliumMode delegates to navigation store owner', () => {
  assert(/setMyceliumMode\s+as\s+_setMyceliumMode/.test(lifecycleSrc), 'lifecycle imports navigation setMyceliumMode owner');
  assert(/export\s+const\s+setMyceliumMode\s*=\s*_setMyceliumMode/.test(lifecycleSrc), 'lifecycle exports delegated setMyceliumMode');
...
await test('setSemanticDiveMode is owned by focus store', () => {
  assert(/setSemanticDiveMode\s+as\s+_setSemanticDiveMode/.test(lifecycleSrc), 'lifecycle imports focus setSemanticDiveMode owner');
  assert(/export\s+const\s+setSemanticDiveMode\s*=\s*_setSemanticDiveMode/.test(lifecycleSrc), 'lifecycle exports delegated setSemanticDiveMode');
...
await test('applyStoryPrompt maps story prompts to mode/filter owners', () => {
  const body = clusterFilterSrc;
  assert(/story\s*===\s*['"]signal-rich['"][\s\S]*?setMyceliumMode\s*\(\s*['"]bloom['"]/.test(body), 'signal-rich story maps to bloom mode');
  assert(/story\s*===\s*['"]bridge-businesses['"][\s\S]*?setMyceliumMode\s*\(\s*['"]bridge['"]/.test(body), 'bridge-businesses story maps to bridge mode');
  assert(/story\s*===\s*['"]mapped-food['"]/.test(body), 'mapped-food story is handled');
  assert(/overwriteActiveFilters\s*\(/.test(body), 'applyStoryPrompt resets or updates active filters through store owner');
  assert(/storeSetClusterFilter\s*\(\s*null\s*\)/.test(body), 'applyStoryPrompt clears active cluster filter through store owner');
  ...
  assert(!/window\.(applyFilters|setMyceliumMode|syncFilterControls)\s*\(/.test(body), 'applyStoryPrompt avoids window UI bridge calls');
});
