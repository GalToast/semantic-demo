...
test.describe('focus-neighborhood interaction', () => {
    ...
    test('desktop: a non-anchor neighbor can be hovered independently without anchor re-selection', async ({
        ...
        const entryIndex = await page.evaluate(() => {
            ...
            for (let i = 0; i < Math.min(pts.length, 30); i++) {
                const node = (window.__APP_STATE__ ?? window.__TEST_STATE__).semanticNeighborMapByLeadId?.get(
                    pts[i]?.lead_id
                )
    ...
    test('short-landscape: non-anchor neighbors are on-screen and reachable at 844x390', async ({ page }) => {
        ...
        expect(snap.neighborIndices?.length ?? 0, 'short-landscape pocket must have non-anchor nodes').toBeGreaterThan(
            0
        )
        ...
        expect(
            onScreen.length,
            'at least one non-anchor neighbor must be on-screen at short-landscape'
        ).toBeGreaterThan(0)

        expect(
            snap.reachableCount,
            `short-landscape must have reachable non-anchor neighbors, got ${snap.reachableCount}`
        ).toBeGreaterThan(0)
        ...
        expect(
            separatedCount,
            `at least one neighbor must be ≥${MIN_DISTANCE_PX}px from anchor at short-landscape`
        ).toBeGreaterThan(0)
    })
    ...
    test('short-landscape: a non-anchor neighbor can be hovered independently at 844x390', async ({ page }) => {
        ...
        expect(
            neighbor.hoverIndex,
            `short-landscape hover neighbor must not be anchor (${pre.focusedNode}), got ${neighbor.hoverIndex}`
        ).not.toBe(pre.focusedNode)
    ...
    test('short-landscape: focus-search neighbor rail exposes selectable pills on short-landscape', async ({
        page
    }) => {
        test.setTimeout(FOCUS_NEIGHBORHOOD_TEST_TIMEOUT_MS)
        await openApp(page, { width: 844, height: 390 })
        ...
        await page.waitForFunction(() => ['focus', 'focus-search'].includes(document.body?.dataset?.panelSurface), {
            timeout: 15000
        })
        await page
        ...
        expect(['focus', 'focus-search'], `short-landscape: panel surface must be focus/focus-search`).toContain(
            metrics.panelSurface
        )
        ...
        expect(metrics.firstPill?.pointerEvents, `short-landscape: first neighbor pill must be selectable`).not.toBe(
            'none'
        )
        expect(
            metrics.firstPill?.height ?? 0,
            `short-landscape: first neighbor pill must have touch/click area`
        ).toBeGreaterThanOrEqual(40)
        const visibleHeight =
            Math.min(metrics.firstPill?.bottom ?? Infinity, 390) - Math.max(metrics.firstPill?.y ?? Infinity, 0)
        const hasScrollableRail =
            metrics.rail?.overflowY === 'auto' ||
            metrics.list?.overflowY === 'auto' ||
            metrics.card?.overflowY === 'auto'
        expect(
    ...
    test('mobile-portrait: focus-search neighbor rail exposes selectable pills on 390x844', async ({ page }) => {
        ...
        expect(['focus', 'focus-search'], `mobile-portrait: panel surface must be focus/focus-search`).toContain(
            metrics.panelSurface
        )
        ...
        expect(metrics.firstPill?.pointerEvents, `mobile-portrait: first neighbor pill must be selectable`).not.toBe(
            'none'
        )
        expect(
            metrics.firstPill?.height ?? 0,
            `mobile-portrait: first neighbor pill must have touch/click area`
        ).toBeGreaterThanOrEqual(40)
        const visibleHeight =
            Math.min(metrics.firstPill?.bottom ?? Infinity, 844) - Math.max(metrics.firstPill?.y ?? Infinity, 0)
    ...
    test('desktop: Tab reaches neighbor pill and Enter changes focus to that neighbor', async ({ page }) => {
        ...
        if (activeEl.cls?.includes('focus-stage-neighbor-pill') && activeEl.dataIndex !== undefined) {
            ...
            const movedToNeighbor =
                after.focusedNode !== anchorIndex &&
    ...
    test('desktop: neighbor pill has visible focus and semantic aria-label when focused', async ({ page }) => {
        ...
        const hasSemanticContent =
            focused.ariaLabel?.toLowerCase().includes('explore') ||
            focused.ariaLabel?.toLowerCase().includes('connection') ||
            focused.ariaLabel?.toLowerCase().includes('neighbor')
        expect(hasSemanticContent, `pill aria-label must convey semantic intent, got "${focused.ariaLabel}"`).toBe(true)
    })
    ...
    test('short-landscape: Tab reaches neighbor pill and Enter changes focus at 844x390', async ({ page }) => {
        ...
        const neighborCount = await page.evaluate(() => document.querySelectorAll('.focus-stage-neighbor-pill').length)
        ...
        expect(
            isPillOrAction,
            `short-landscape: Tab must reach a pill or action button within ${activeEl.tabPresses} presses, got cls="${activeEl.cls}"`
        ).toBe(true)
    ...
    test('mobile-portrait: Tab reaches neighbor pill at 390x844', async ({ page }) => {
        ...
        expect(
            isPillOrAction,
            `mobile-portrait: Tab must reach a pill or action button within ${activeEl.tabPresses} presses, got cls="${activeEl.cls}"`
        ).toBe(true)
    })

    test('mobile-portrait: a non-anchor neighbor can be clicked without re-selecting the anchor at 390x844', async ({
        page
    }) => {
        test.setTimeout(FOCUS_NEIGHBORHOOD_TEST_TIMEOUT_MS)
        await openApp(page, { width: 390, height: 844 })
        ...
        if (!neighbor) {
            ...
            const snap = await probeNeighborhood(page)
            expect(
                snap.neighborIndices.length,
                'mobile-portrait: pocket must have non-anchor indices in state'
            ).toBeGreaterThan(0)
    ...
    test('tablet: non-anchor neighbors are on-screen and reachable at 1024x768', async ({ page }) => {
        ...
        expect(
            snap.reachableCount,
            `tablet must have reachable non-anchor neighbors, got ${snap.reachableCount}`
        ).toBeGreaterThan(0)
        ...
        expect(
            separatedCount,
            `tablet: at least one neighbor must be ≥${MIN_DISTANCE_PX}px from anchor`
        ).toBeGreaterThan(0)
    })
    ...
    test('tablet: a non-anchor neighbor can be hovered independently at 1024x768', async ({ page }) => {
        ...
        expect(
            neighbor.hoverIndex,
            `tablet hover neighbor must not be anchor (${pre.focusedNode}), got ${neighbor.hoverIndex}`
        ).not.toBe(pre.focusedNode)
    ...
    test('tablet: focus-search neighbor rail exposes selectable pills at 1024x768', async ({ page }) => {
        ...
        expect(['focus', 'focus-search'], `tablet: panel surface must be focus/focus-search`).toContain(
            metrics.panelSurface
        )
        ...
        expect(
            metrics.firstPill?.height ?? 0,
            `tablet: first neighbor pill must have touch/click area`
        ).toBeGreaterThanOrEqual(40)
        const visibleHeight =
            Math.min(metrics.firstPill?.bottom ?? Infinity, 768) - Math.max(metrics.firstPill?.y ?? Infinity, 0)
        const hasScrollableRail =
            metrics.rail?.overflowY === 'auto' ||
            metrics.list?.overflowY === 'auto' ||
            metrics.card?.overflowY === 'auto'
    ...
    test('desktop: anchor and neighbor spore regions are visually distinguishable by pixel delta', async ({ page }) => {
        ...
        for (const entryIndex of candidateEntryIndices) {
            ...
            const anchorReachable = await isReachableScreenCoordinate(
                page,
                candidateAnchor.screenX,
                candidateAnchor.screenY
            )
        ...
        if (bestNeighborTarget) {
            ...
            outer: for (const entryIndex of candidateEntryIndices) {
                await focusNodeViaApp(page, entryIndex)
                await page.waitForFunction(() => window.__TEST_STATE__?.navState?.mode === 'focus', { timeout: 15000 })
                await page
                    .waitForFunction(
        ...
        expect(
            bestEntryIndex,
            'a focus entry must place anchor and neighbor inside the reachable canvas'
        ).not.toBeNull()
        ...
        expect(fs.existsSync(anchorScreenshotPath), `anchor screenshot must be saved to ${anchorScreenshotPath}`).toBe(
            true
        )
        ...
        expect(
            fs.existsSync(neighborScreenshotPath),
            `neighbor screenshot must be saved to ${neighborScreenshotPath}`
        ).toBe(true)
