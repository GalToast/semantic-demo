...
function matchingBlocks(cssText, selectorSpec) {
    return selectorRuleBlocks(cssText).filter((block) =>
...
function hasImportant(cssText, selectorSpec) {
    return selectorRuleBlocks(cssText).some(
        (block) =>
            selectorSpec.every((frag) => preludeContains(block.prelude, frag)) && block.body.includes('!important')
    )
}
 ...
 */
const STABLE_GEOMETRY_RE =
    /\b(?:width|height|position|top|left|right|bottom|margin|padding|border|display|z-index|flex|grid|float|clear|overflow)\s*:/
...
if (!focusPrimitiveOwner.includes('Focus stage action/button primitives')) {
    violations.push(
        `${FOCUS_ACTION_PRIMITIVE_GUARD.canonicalFile} must document and own focus-stage action/button primitives`
    )
}
...
for (const selector of MOBILE_COMPASS_TRANSITION_GUARD.selectors) {
    const transitionAllBlocks = matchingBlocks(mobileCompassTransitionOwner, [selector]).filter((block) =>
        /\btransition\s*:\s*all\b/i.test(block.body)
    )
...
for (const file of cssFiles) {
    ...
    for (const limit of FORWARD_ONLY_LIMITS) {
        ...
        for (const [selector, maxCount] of Object.entries(limit.limits)) {
            ...
            if (count > maxCount) {
                violations.push(
                    `${file} defines ${selector} ${count} time(s), baseline limit is ${maxCount} — ${limit.message}`
                )
            }
        }
    }
    ...
    for (const sel of TRANSIENT_TRANSITIONING_SELECTORS) {
        const blocks = selectorRuleBlocks(content).filter(
            (block) => block.prelude.includes('data-semantic-dive="transitioning"') && block.prelude.includes(sel)
        )
