...
test.describe('3D accessibility, fallback, and performance contracts', () => {
    test('keyboard contract: canvas stays out of tab order and focused controls expose a visible ring', async ({
        ...
        expect(keyboardSurface.canvasTabIndex, 'canvas tabindex must be absent or explicitly non-tabbable').not.toBe(
            '0'
        )
        ...
        expect(
            hasVisibleFocus,
            `focused search input should expose focus styling: ${JSON.stringify(keyboardSurface)}`
        ).toBe(true)
    })
    ...
    test('webgl unavailable contract: loading resolves to fallback/error or completes, not an endless spinner', async ({
        page
    }) => {
        ...
        expect(
            state.hasError || state.loadingGone,
            `WebGL-unavailable state must resolve: ${JSON.stringify(state)}`
        ).toBe(true)
    })
    ...
    test('performance contract: renderer diagnostics are finite and stable through repeated focus', async ({
        page
    }) => {
        ...
        expect(after.rendererMemory.geometries, 'focus operations should not leak many geometries').toBeLessThanOrEqual(
            before.rendererMemory.geometries + 5
        )
    })
})
