...
function rtPass(name) {
    rt.passed++
    console.log(`  PASS  runtime  ${name}`)
}
function rtFail(name, msg) {
    rt.failed++
    console.error(`  FAIL  runtime  ${name} — ${msg}`)
}
...
try {
    ...
    if (typeof urlMod.copyCurrentViewLink === 'function') rtPass('R1:copyCurrentViewLink is function')
    else rtFail('R1:copyCurrentViewLink', `type=${typeof urlMod.copyCurrentViewLink}`)

    // R2: On clipboard success, returns a string href (not null)
    try {
        const result = await urlMod.copyCurrentViewLink()
        if (typeof result === 'string' && result.length > 0) rtPass('R2:success returns string href')
        else if (result === null)
            rtFail('R2:success returns null', 'clipboard write succeeded but function returned null')
        else rtFail('R2:success', `got ${typeof result}: "${result}"`)
    } catch (e) {
    ...
    try {
        ...
        const result = await urlMod.copyCurrentViewLink()
        if (result === null) rtPass('R3:clipboard failure returns null')
        else rtFail('R3:clipboard failure', `expected null, got "${result}"`)
    ...
    if (globalThis.__clipboardText) {
        ...
        if (!hasCb && !hasLead) rtPass('R4:cb and lead params removed from clipboard output')
        else rtFail('R4:param cleaning', `cb=${hasCb}, lead=${hasLead}`)
    } else {
