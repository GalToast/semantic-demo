    ...
    return page.evaluate(() => {
        ...
        return {
            ...
            state: {
                ...
                focusPocketIndices: state.navState?.focusPocketIndices ? [...state.navState.focusPocketIndices] : [],
            ...
            ui: {
                ...
                focusStageActions: document.querySelectorAll(
                    '.focus-stage-neighbor-actions,.focus-stage-neighbor-action'
                ).length,
                threadInspectorItems: document.querySelectorAll('.thread-item,.thread-candidate,.ti-item,.ti-candidate')
                    .length,
                nodeDetailCards: document.querySelectorAll('.node-detail,.lead-detail,.person-card,.node-info-card')
                    .length,
...
async function enterFocusByIndex(page, index) {
    ...
    await page.waitForFunction(
        () => (window.__APP_STATE__?.navState?.mode ?? window.__TEST_STATE__?.navState?.mode ?? '') === 'focus',
        { timeout: 15000 }
    )
    await page
        .waitForFunction(() => new Promise((r) => requestAnimationFrame(() => r(true))), { timeout: 5000 })
        .catch(() => {}) // allow pocket animation to begin
}
...
test.describe('focus-pocket node selectability', () => {
    ...
    test('desktop: entering focus sets focusedNode, populates focusPocketIndices, and exposes selectable UI', async ({
        ...
        expect(
            ['focus', 'focus-search', 'semantic-dive'],
        ...
        const pocketNonEmpty = Array.isArray(snap.state.focusPocketIndices) && snap.state.focusPocketIndices.length > 0
        expect(
            pocketNonEmpty,
        ...
        const roleMapNonEmpty =
            snap.state.focusPocketRoleByIndex && Object.keys(snap.state.focusPocketRoleByIndex).length > 0
        expect(
            roleMapNonEmpty,
        ...
        expect(
            snap.state.focusPocketMeta.active,
            'focusPocketMeta.active must be true when pocket is formed'
        ).toBeTruthy()

        // --- nodesAreSettling must be true during pocket animation ---
        expect(
            snap.state.nodesAreSettling,
            'nodesAreSettling should be true during focus-pocket animation'
        ).toBeTruthy()
        ...
        const hasThreadCandidates = Array.isArray(snap.state.threadCandidates) && snap.state.threadCandidates.length > 0
        const hasFallbackCandidates =
            Array.isArray(snap.state.focusPocketIndices) && snap.state.focusPocketIndices.length > 0

        expect(
            hasThreadCandidates || hasFallbackCandidates,
    ...
    test('desktop: search-result click enters focus and populates pocket + relationship UI', async ({ page }) => {
        ...
        expect(snap.body.panelSurface, 'panelSurface must reflect focus mode').toMatch(/focus|semantic-dive/)
        ...
        expect(
            hasPocketIndices || hasThreadCandidates,
    ...
    test('mobile: focusOnNode at 844x390 populates pocket state and relationship UI is visible', async ({ page }) => {
        ...
        expect(
            snap.state.nodesAreSettling,
            'nodesAreSettling should be true during mobile focus-pocket animation'
        ).toBeTruthy()
    })
    ...
    test('tablet: all focusPocketIndices have valid in-bounds screen coordinates at 1024x768', async ({ page }) => {
        ...
        expect(
            ...
            `tablet pocket must have at least 1 in-bounds screen node, got ${pocket.reachableCount} of ${pocket.pocketSize}`
        ).toBeGreaterThan(0)
    })
    ...
    test('short-landscape: focus mode enters cleanly at 844x390 with pocket evidence', async ({ page }) => {
        ...
        expect(snap.state.focusPocketMeta.active, 'focusPocketMeta.active must be true at short-landscape').toBeTruthy()
    ...
    test('desktop: all focusPocketIndices have valid in-bounds screen coordinates', async ({ page }) => {
        ...
        expect(
            pocket.reachableCount,
            `pocket must have at least 1 in-bounds screen node, got ${pocket.reachableCount} of ${pocket.pocketSize}`
        ).toBeGreaterThan(0)
    ...
    test('tablet: focus pocket has reachable nodes at 1024x768', async ({ page }) => {
        ...
        expect(
            pocket.reachableCount,
            `tablet must have reachable pocket nodes, got ${pocket.reachableCount}`
        ).toBeGreaterThan(0)
    })
    ...
    test('short-landscape: focus pocket has reachable nodes at 844x390', async ({ page }) => {
        ...
        expect(
            pocket.reachableCount,
            `short-landscape must have reachable pocket nodes, got ${pocket.reachableCount}`
        ).toBeGreaterThan(0)
    })
    ...
    test('desktop: Step Inside (trailDepth=2) maintains selectable relationship path', async ({ page }) => {
        ...
        expect(
            ['focus', 'semantic-dive', 'trail', 'map-trail'],
        ...
        expect(hasSelectablePath, 'Selectable relationship path must survive Step Inside (trailDepth=2)').toBeTruthy()

        // Relationship UI must still be visible
        const hasRelationshipUI =
            snap.ui.threadInspectorItems > 0 || snap.ui.focusStageActions > 0 || snap.ui.nodeDetailCards > 0

        expect(hasRelationshipUI, 'Relationship UI must remain visible after Step Inside').toBeTruthy()
    })
})
