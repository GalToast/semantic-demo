 ...
 *
 * Fast Node contract test for src/lib/journey/semantic-guide-payload.ts
 *
 ...
 *
 * Usage (canonical — the `@lib` alias only resolves with the ts-resolve-loader;
 * a bare `node tests/...mjs` fails with ERR_MODULE_NOT_FOUND for `@lib/state`):
 *   node --experimental-transform-types --import ./tests/helpers/svelte-rune-shim.mjs \
 *        --loader ./tests/helpers/ts-resolve-loader.mjs tests/semantic-guide-payload-contract.mjs
 *   (or run via: node tests/run-all-contracts.js --single=semantic-guide-payload-contract.mjs)
 */
...
async function testPayloadDelegatesToAdapter() {
    ...
    assert(srcCode.includes('getSearchContextSnapshot'), 'payload uses getSearchContextSnapshot (inlined)')
    assert(srcCode.includes('getPoints'), 'payload uses getPoints from adapter')
    assert(srcCode.includes('getResultContextMap'), 'payload uses getResultContextMap from adapter')
...
async function testAdapterCoversAllStateFields() {
    ...
    assert(adapterSrc.includes('semanticResultContextByLeadId'), 'adapter accesses semanticResultContextByLeadId')

    // Adapter must access state.currentSearchSummary
    assert(adapterSrc.includes('currentSearchSummary'), 'adapter accesses currentSearchSummary')

    // Adapter must access state.currentView
    assert(adapterSrc.includes('currentView'), 'adapter accesses currentView')

    // Adapter must access state.points
    assert(adapterSrc.includes('state.points'), 'adapter accesses state.points')
...
async function testAdapterSnapshotStructure() {
    ...
    assert('currentSearchSummary' in snap, 'snapshot has currentSearchSummary field')
    assert('currentView' in snap, 'snapshot has currentView field')
...
async function testBuildResultWithProvidedSnapshot() {
    ...
    const fakeContextMap = new Map([
        [
            'LI_001',
            { city: 'Austin', public_note: 'Great biz', public_detail: 'Details A', address: '123 A St', naics: '1234' }
        ],
        [
            'LI_002',
            { city: 'Boston', public_note: 'Old biz', public_detail: 'Details B', address: '456 B Ave', naics: '5678' }
        ]
    ])
...
async function testPayloadHelpersHonorExplicitSummary() {
    ...
    const { getSemanticGuidePayloadResults, getSemanticGuideAnchorPoint } =
        await import('../src/lib/journey/semantic-guide-payload.ts')
