...
const LIVE_URL = process.env.LIVE_SEMANTIC_URL ||
...
try {
  ...
  assert(snap.threadNodes === EXPECTED.threadNodes, `expected ${EXPECTED.threadNodes} thread nodes, got ${snap.threadNodes}`);
  assert(snap.edgeCount === EXPECTED.edgeCount, `expected ${EXPECTED.edgeCount} semantic edges, got ${snap.edgeCount}`);
  assert(snap.semanticThreadsStatus === 'ready', `semanticThreadsStatus should be ready, got ${snap.semanticThreadsStatus}`);
