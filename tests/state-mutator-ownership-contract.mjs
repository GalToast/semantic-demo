...
const OWNED_FIELDS = ['currentView', 'semanticLaneState', 'loadingPhaseKey', 'semanticThreadsStatus']
...
const offenders = []
const directWritePattern = new RegExp(`\\bstate\\.(${OWNED_FIELDS.join('|')})\\s*=(?!=)`, 'g')

const ALLOWED_DIRECT_WRITE_FILES = new Set(['src/lib/state/mutators.ts', 'src/lib/state/app.svelte.ts'])
