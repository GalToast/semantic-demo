...
function compareSceneBand(beforeBuffer, afterBuffer) {
    ...
    for (let y = y0; y < y1; y += 1) {
        for (let x = x0; x < x1; x += 1) {
            ...
            const delta =
                Math.abs(before.raw[beforeIndex] - after.raw[afterIndex]) +
...
test.describe('focus semantic Line2 shader ownership', () => {
    test('semantic focus line uses its own runtime shader uniforms', async ({ page }) => {
        ...
        page.on('console', (msg) => {
            if (
                msg.type() === 'error' &&
                /WebGLProgram: Shader Error|VALIDATE_STATUS false|shader is not compiled/i.test(msg.text())
            ) {
        ...
        await page.evaluate(() => {
            ...
            const setTrailDepth = actions?.setTrailDepth
        ...
        const before = await page.evaluate(() => {
            ...
            return {
                ...
                pointsShaderIsSeparate:
                    shader !== (window.__APP_STATE__ ?? window.__TEST_STATE__).pointsMaterial?.userData?.shader,
                uniformNames: Object.keys(shader.uniforms || {})
            }
        })

        expect(before.threadSource, 'focus path must be driven by semantic neighbors, not geometric fallback').toBe(
            'semantic'
        )
        ...
        expect(
            before.focusThreadSegments,
            'semantic focus cue probe should report rendered line segments'
        ).toBeGreaterThan(0)
        expect(before.denseBundleUniform, 'denseBundleMode uniform should match focus line density state').toBe(
            before.overlayNodeCount >= 6 ? 1 : 0
        )
        expect(Boolean(before.denseBundleMode), 'focus line should record dense bundle state from overlay count').toBe(
            before.overlayNodeCount >= 6
        )
        if (Number.isFinite(before.nextIndex)) {
            expect(
                before.nextCueSegments,
                'semantic focus cue probe should report next-cue segments when a next stop is available'
            ).toBeGreaterThan(0)
        } else {
            expect(
                before.nextCueSegments,
                'semantic focus cue probe should not fabricate next-cue segments without an available next stop'
            ).toBe(0)
        }
        expect(before.parentKind, 'focus line should record the active parent owner').toBe(
            before.hasMyceliumGroup ? 'mycelium' : 'scene'
        )
        ...
        expect(
            before.materialShaderIsOwnObject,
            'assertion must target focusSemanticLines.material.userData.shader'
        ).toBe(true)
        expect(before.pointsShaderIsSeparate, 'focus line shader must not be confused with pointsMaterial shader').toBe(
            true
        )
        ...
        await page.evaluate(() => {
            const currentTime = (window.__APP_STATE__ ?? window.__TEST_STATE__).focusSemanticLines.material.userData
        ...
        if (before.reducedMotion) {
            ...
            expect(
                after.time,
                'updateFocusSemanticOverlayPositions should advance focus line shader time'
        ...
        expect(
            visualDelta.changedPixels,
            'hiding focusSemanticLines should materially change rendered scene pixels'
        ).toBeGreaterThan(160)
        expect(
            visualDelta.strongPixels,
            'focusSemanticLines should contribute strong visible pixels, not only invisible state'
        ).toBeGreaterThan(24)
        expect(
            visualDelta.meanDelta,
            'focusSemanticLines should have measurable visual contrast in the scene band'
        ...
        expect(
            fallbackParent.builtInScene,
            'focus line should fall back to scene ownership while mycelium group is not ready'
        ).toBe(true)
        expect(fallbackParent.parentKind, 'fallback focus line should record scene parent ownership').toBe('scene')
        expect(
            fallbackParent.removedFromScene,
            'refreshFocusSemanticOverlay should remove fallback-owned scene lines before rebuilding'
        ).toBe(true)
        expect(fallbackParent.restoredParentKind, 'focus line should use the available owner after rebuild').toBe(
            fallbackParent.expectedRestoredParentKind
        )
