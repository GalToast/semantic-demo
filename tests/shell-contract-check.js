...
requireIncludes(
    ...
    'Svelte source shell owns the Pandora stylesheet'
)
...
requireExcludes(
    ...
    sourceHtml,
    'dist/bundle.js',
    'Svelte production runtime must not depend on the legacy esbuild bundle'
)
...
requireIncludes(
    ...
    'deploy docs must explain the shell contract'
)
requireIncludes(
    'ARCHITECTURE.md',
    architectureDoc,
    legacyShell,
    'architecture docs still document the legacy shell reference'
)
...
try {
    ...
    requireIncludes(
        ...
        'build:svelte',
        'production build must use the Svelte/Vite entry'
    )
    requireIncludes(
        'package.json',
        packageJson.scripts?.['check:shell'] || '',
        'shell-contract-check.js',
        'package script must expose the guard'
    )
} catch (error) {
