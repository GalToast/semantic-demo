...
const { buildFocusedPocketStagedPositions, buildFocusedSemanticPocket } =
...
function testPersonalityTypes() {
    ...
    assert(denseHub.type === 'DENSE_HUB' || denseHub.type === 'STANDARD', `DENSE_HUB or fallback: got ${denseHub.type}`)
...
function testPlacementGeometry() {
    ...
    assert(
        support.radius !== primary.radius || support.zOffset !== primary.zOffset,
...
function testBuildFocusedPocketStagedPositions() {
    ...
    const nonAnchorRoles = [...result.roles.entries()].filter(([i]) => i !== 0).map(([, v]) => v)
...
function testFocusThreadCurvePoint() {
    ...
    assert(pt0.x !== pt1.x || pt0.y !== pt1.y || pt0.z !== pt1.z, 'curve points at t=0 and t=1 differ')
