...
async function openAppPage(page, viewport, options = {}) {
  ...
  try {
    ...
    throw new Error(`Semantic Explorer navigation failed: target=${targetUrl} current=${page.url()} error=${message}`, { cause: error });
  }
  if (response && response.status() >= 400) {
    throw new Error(`Semantic Explorer navigation returned HTTP ${response.status()}: target=${targetUrl} response=${response.url()}`);
  }
  const hasAppIdentity = await page.waitForFunction(() => (
  ...
  if (!hasAppIdentity) {
    ...
    throw new Error(`Semantic Explorer app identity missing after navigation: target=${targetUrl} status=${response?.status() ?? 'no-response'} diagnostics=${JSON.stringify(diagnostics)}`);
  }
}
  ...
  const result = await page.evaluate(() => {
    ...
    const focusedIndex = Number.isFinite(state.navState?.focusedIndex)
      ...
      : Number.isFinite(state.focusedNode)
        ? state.focusedNode
        : null
    const targetIndex = focusedIndex !== null
      ? focusedIndex
    ...
    if (typeof actions.setSurface === 'function') {
      ...
      throw new Error('short-landscape parity bridge unavailable: expected writeNavStateMirror or switchView+setSurface')
    }
...
export async function openApp(page, viewport) {
  ...
  await page.waitForFunction(() => {
    ...
    return Array.isArray(appState.points) &&
