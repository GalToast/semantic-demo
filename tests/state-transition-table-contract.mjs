...
function assertEq(actual, expected, label) {
  if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new Error(`ASSERTION FAILED: ${label} - got '${JSON.stringify(actual)}', want '${JSON.stringify(expected)}'`);
}
...
function extractExportedFunction(src, name) {
  ...
  const directRe = new RegExp(`export\\s+function\\s+${name}\\s*\\([^)]*\\)(?:\\s*:\\s*\\S[^{]*)?\\s*\\{[\\s\\S]*?\\n\\}`);
...
for (const f of navFields) {
  assert(stateSrc.includes(`'${f}'`) || stateSrc.includes(`"${f}"`) || stateSrc.includes(` ${f}:`),
...
assert(stateSrc.includes('trailDepth === 2') || stateSrc.includes('trailDepth ===2') || stateSrc.includes('get semanticDiveMode'),
...
const lifecycleExports = [
  'resetExplorationFocus', 'resetExperienceState', 'returnToOverview',
  'setSemanticDiveMode', 'setTrailDepth', 'setMyceliumMode', 'resetNodePositions'
];
...
const hasGate = /nextDepth\s*===\s*2\s*&&\s*prevDepth\s*<\s*2/.test(lifecycleSrc) ||
...
const modeDesc = extractExportedObject(lifecycleSrc, 'MODE_DESCRIPTIONS') ||
...
for (const k of modeKeys) {
  assert(modeDesc.includes(`'${k}'`) || modeDesc.includes(`"${k}"`) || modeDesc.includes(`${k}:`),
...
const storyDesc = extractExportedObject(lifecycleSrc, 'STORY_DESCRIPTIONS') ||
...
for (const k of storyKeys) {
  assert(storyDesc.includes(`'${k}'`) || storyDesc.includes(`"${k}"`) || storyDesc.includes(`${k}:`),
...
assert(extractExportedFunction(lifecycleSrc, 'refreshCompositionState'), 'lifecycle.js must export refreshCompositionState');
console.log('  PASS');
...
assert(extractExportedFunction(lifecycleSrc, 'executeJourneyCompassAction'), 'lifecycle.js must export executeJourneyCompassAction');
console.log('  PASS');
...
assert(!/window\.dispatchNavTransition\s*=/.test(lifecycleSrc), 'window.dispatchNavTransition compatibility bridge must be retired');
console.log('  PASS');
...
assert(
  /case\s+NAV_TRANSITION_ACTIONS\.RESTORE_EXPLORATION_HISTORY\s*:|case\s+['"]RESTORE_EXPLORATION_HISTORY['"]\s*:/.test(navigationStateSrc),
