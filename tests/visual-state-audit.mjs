    ...
    await page
        .waitForFunction(
            (mustUseWebgl) => {
                ...
                const sceneRevealInactive =
                    state.sceneRevealActive !== true && document.body.dataset.sceneReveal !== 'active'

                const segmentCount =
                    (state.scenePerformanceDiagnostics?.myceliumCoreSegments || 0) +
                ...
                const threadsReady = state.myceliumGroup?.visible === true && segmentCount > 0 && threadOpacity >= 0.001

                return sceneRevealInactive && pointCount > 0 && pointOpacity >= 0.01 && threadsReady
            },
            requireWebgl,
            { timeout: 8000 }
        )
...
async function captureState(page, name) {
    ...
    const data = await page.evaluate((stateName) => {
        ...
        const boxFor = (selector) => {
            ...
            const topElement = rect.width > 0 && rect.height > 0 ? document.elementFromPoint(centerX, centerY) : null
            const describeElement = (el) => {
                if (!el) return null
                if (el.id) return `#${el.id}`
                if (el.classList?.length)
                    return `${el.tagName.toLowerCase()}.${Array.from(el.classList).slice(0, 3).join('.')}`
        ...
        const journeyActions = ['btn-journey-primary', 'btn-journey-secondary', 'btn-journey-tertiary']
            .map((id) => {
        ...
        const mapStrip = (() => {
            ...
            const result = {
                ...
                childClasses: Array.from(el.children)
                    .map((c) => c.className)
                    .join('|'),
        ...
        return {
            ...
            clusterLabelDiagnostics: (() => {
                ...
                const webglSprites = (() => {
                    ...
                    scene.traverse((node) => {
                        ...
                        const isClusterLabelSprite =
                            node?.isSprite === true && image?.width === 512 && image?.height === 128
                        ...
                        sprites.push({
                            ...
                            inViewport:
                                x >= -32 && x <= window.innerWidth + 32 && y >= -32 && y <= window.innerHeight + 32
                        })
                    })
                    const oversized = sprites.filter(
                        (sprite) =>
                            sprite.inViewport &&
                            (sprite.projectedWidth > window.innerWidth * 0.52 ||
                                sprite.projectedHeight > window.innerHeight * 0.13)
                    )
                    return {
                        available: true,
                ...
                return {
                    ...
                    offscreen: visibleLabels.filter(
                        (label) =>
                            label.x < -8 ||
                            label.y < -8 ||
            ...
            modeGridDiagnostics: (() => {
                ...
                return {
                    ...
                    names: chips.map(
                        (chip) => chip.querySelector('.mode-name')?.textContent?.trim() || chip.textContent.trim()
                    ),
            ...
            routeEvidence: (() => {
                ...
                const detail =
                    evidence.detail || document.body.dataset.visualRouteDetail || 'loaded from URL/query params'
...
async function enterFocusFromSearch(page, targetIndexOverride = null) {
    ...
    await page.evaluate((overrideIndex) => {
        ...
        const targetIndex = Number.isFinite(forcedIndex)
            ? forcedIndex
    ...
    await markVisualRouteEvidence(
        page,
        'constructed-surface',
        'focus surface dataset and focus-stage visibility shaped for visual audit'
    )
    await page
        .waitForFunction(() => new Promise((r) => requestAnimationFrame(() => r(true))), { timeout: 3000 })
        .catch(() => {})
    ...
    const waitForFocusedResult = () =>
        page.waitForFunction(
            () => {
                ...
                const focusStage = document.querySelector('#focus-stage')
                const focusStageVisible =
                    !!focusStage && !focusStage.hidden && getComputedStyle(focusStage).display !== 'none'
                return (
                    hasFocusedState &&
...
async function enterSemanticDiveViaVisibleControl(page) {
    const visibleEntryCandidate = () => {
        ...
        const candidates = [
            [
                'journey compass enter-inside action',
                document.querySelector('button[data-journey-action="enter-inside"]')
            ],
            ['#btn-focus-dive', document.querySelector('#btn-focus-dive')],
            [
                'visible Step Inside button',
                [...document.querySelectorAll('button')].find((button) =>
                    /step inside/i.test(`${button.textContent || ''} ${button.getAttribute('aria-label') || ''}`)
                )
            ]
        ]
    ...
    const clickTarget = await page
        .waitForFunction(visibleEntryCandidate, undefined, { timeout: 12000 })
...
async function enterMapFocusSearchByRealRoute(page) {
    ...
    await page.waitForFunction(
        () => {
            return (
                document.body.dataset.activeView === 'map' && document.body.dataset.panelSurface === 'map-focus-search'
            )
        },
        undefined,
        { timeout: 15000 }
        ...
        if (box && box.width > 0 && box.height > 0) {
            ...
            if (clicked) {
                ...
                await markVisualRouteEvidence(
                    page,
                    'real-click',
                    'mouse-clicked active thread inspector surface center'
                )
            }
        } else {
            await markVisualRouteEvidence(
                page,
                'real-click',
                'active thread inspector surface already active without clickable bounding box'
            )
        }
    }
...
async function enterSemanticDive(page) {
    ...
    if (await diveButton.isVisible().catch(() => false)) {
        const clicked = await diveButton
            .click({ timeout: 5000 })
            .then(() => true)
            .catch(() => false)
        ...
        if (await textButton.isVisible().catch(() => false)) {
            const clicked = await textButton
                .click({ timeout: 5000 })
                .then(() => true)
                .catch(() => false)
    ...
    await page
        .waitForFunction(() => document.body.dataset.panelSurface === 'semantic-dive', undefined, { timeout: 15000 })
        .catch(() => {})
    await page
    ...
    await page
        .waitForFunction(
            () => {
                ...
                const focused =
                    Number.isFinite(appState.focusedNode) || Number.isFinite(appState.navState?.focusedIndex)
                return (
                    focused &&
                    document.body.dataset.panelSurface === 'focus' &&
                    card &&
...
async function applyModeGridVisibleState(page) {
    await page.evaluate(() => {
        ...
        document
            .querySelectorAll(
                '#btn-launch, .stats-row, .stat-caption, .search-label, .search-input-wrapper, .search-hint, .semantic-lane-assist, .search-trail-cue'
            )
            .forEach((element) => {
...
async function run() {
    ...
    try {
        ...
        if (
            ...
            try {
                const mobilePage = await createAuditPage(browser, {
                    viewport: mobile,
                    deviceScaleFactor: 2,
                    isMobile: true
                })
                ...
                if (wantsAny(['03-mobile-focus-first-result', '04-mobile-field-node-active'])) {
                    ...
                    if (wantsState('04-mobile-field-node-active')) {
                        ...
                        await markVisualRouteEvidence(
                            mobilePage,
                            'constructed-surface',
                            'visual audit field-node focus-search fixture'
                        )
                ...
                if (wantsState('06-mobile-filters-open')) {
                    ...
                    const clicked = await mobilePage
                        .locator('#filters-section summary')
                        .click({ timeout: 5000 })
                        .then(() => true)
                        .catch(() => false)
                    if (clicked)
                        await markVisualRouteEvidence(
                            mobilePage,
                            'real-click',
                            'clicked filters summary in visual audit'
                        )
              ...
              <div class="search-error-state" role="alert">
                ...
                <div class="search-error-actions">
                  ...
                  <button class="search-error-dismiss-btn" type="button">Dismiss</button>
                    ...
                    await markVisualRouteEvidence(
                        mobilePage,
                        'constructed-surface',
                        'visual audit search error fixture'
                    )
                ...
                if (wantsState('11-mobile-selected-card-map-trail')) {
                    ...
                    await markVisualRouteEvidence(
                        mobilePage,
                        'constructed-surface',
                        'visual audit map trail dataset fixture'
                    )
                ...
                if (wantsState('21-mobile-route-trace-visible')) {
                    ...
                    await mobilePage.evaluate(async () => {
                        ...
                        const t2 =
                            (window.__APP_STATE__ ?? window.__TEST_STATE__)?.routeTraceLines?.material?.uniforms?.time
                                ?.value ?? null
            ...
            try {
                ...
                if (wantsState('11-desktop-selected-card-map-trail')) {
                    ...
                    await markVisualRouteEvidence(
                        desktopPage,
                        'constructed-surface',
                        'visual audit desktop map trail dataset fixture'
                    )
            ...
            try {
                ...
                if (wantsState('13-desktop-filters-open')) {
                    ...
                    await markVisualRouteEvidence(
                        desktopPage,
                        'constructed-surface',
                        'desktop filters are mobile-only; capture hidden desktop diagnostic without clicking summary'
                    )
                ...
                <div class="search-error-actions">
                  ...
                  <button class="search-error-dismiss-btn" type="button">Dismiss</button>
                    ...
                    await markVisualRouteEvidence(
                        desktopPage,
                        'constructed-surface',
                        'visual audit desktop search error fixture'
                    )
        ...
        if (wantsState('13-mobile-reduced-motion')) {
            const browser = await chromium.launch(launchOptions)
            try {
                const reducedPage = await createAuditPage(browser, {
                    viewport: mobile,
                    deviceScaleFactor: 2,
                    isMobile: true
                })
        ...
        if (wantsState('15-mobile-semantic-dive')) {
            const browser = await chromium.launch(launchOptions)
            try {
                const divePage = await createAuditPage(browser, {
                    viewport: mobile,
                    deviceScaleFactor: 2,
                    isMobile: true
                })
                await divePage.goto(withParams(targetUrl, { view: 'galaxy', q: 'coffee', anchor: '519' }), {
                    waitUntil: 'commit',
                    timeout: 10000
                })
        ...
        if (wantsState('23-mobile-short-landscape')) {
            const browser = await chromium.launch(launchOptions)
            try {
                const slPage = await createAuditPage(browser, {
    ...
    await fs.writeFile(
        path.join(outDir, 'route-evidence-summary.json'),
        `${JSON.stringify(routeEvidenceSummary, null, 2)}\n`,
        'utf8'
    )
    ...
    const isRendered = (b) =>
        b && b.display !== 'none' && b.visibility !== 'hidden' && Number(b.opacity) > 0.05 && b.pointerEvents !== 'none'
    ...
    const assertMapTrailTitleOnly = (state) => {
        ...
        if (!shouldBeVisible) {
            const isHidden = !strip.exists || !stripBox || stripBox.width === 0 || stripBox.height === 0
            if (isHidden) pass(state.name, 'map-strip:hidden-when-not-owner')
            else
                fail(
                    state.name,
                    'map-strip:hidden-when-not-owner',
        ...
        if (strip.buttonCount === 0) pass(state.name, 'map-strip:no-trail-strip-buttons')
            ...
            fail(
                state.name,
                'map-strip:no-trail-strip-buttons',
        ...
        if (strip.childCount === 1 && strip.titleCount === 1) pass(state.name, 'map-strip:exactly-one-title-child')
            ...
            fail(
                state.name,
                'map-strip:exactly-one-title-child',
        ...
        if (strip.titleText && strip.titleText.length > 0) pass(state.name, 'map-strip:title-has-content')
            ...
            fail(
                state.name,
                'map-strip:title-has-content',
                `map-strip-title should have non-empty text, got "${strip.titleText}"`
            )
        ...
        if (strip.titleAttr === strip.titleText) pass(state.name, 'map-strip:title-attr-matches-text')
            ...
            fail(
                state.name,
                'map-strip:title-attr-matches-text',
        ...
        if (strip.ariaLabel === strip.titleText) pass(state.name, 'map-strip:aria-label-matches-text')
            ...
            fail(
                state.name,
                'map-strip:aria-label-matches-text',
    ...
    const isRenderedAction = (action) => action?.action && isRendered(action.rect) && !action.hidden && !action.disabled
    ...
    for (const state of summary) {
        ...
        if (unexpected.length === 0) {
            ...
            fail(state.name, 'surface-overlap-matrix:no-unexpected-overlap', JSON.stringify(unexpected.slice(0, 5)))
        }
    }
    ...
    for (const state of summary) {
        ...
        if (constructedSurfaceStates.has(state.name)) {
            if (evidence.proofLane === 'constructed-surface') {
                ...
                fail(
                    state.name,
                    'route-evidence:constructed-surface-labeled',
                    `expected constructed-surface proof lane, got ${evidence.proofLane || 'missing'}`
                )
            }
        }
    }
    ...
    for (const state of summary) {
        const visibleCompactActions = (state.journeyActions || []).filter(
            (action) => isRenderedAction(action) && action.compactLabel
        )
    ...
    for (const state of summary.filter(isMobileState)) {
        ...
        if (!allowsLoadingOverlay) {
            if (state.bodyDataset?.loadingOverlay === 'active' || isVisible(loadingOverlay)) {
                fail(
                    state.name,
                    'visual-settle:loading-overlay-hidden',
                    'loading overlay is still active over a non-loading visual state'
                )
            } else {
        ...
        if (isRendered(compass)) {
            if (!allowsLoadingOverlay && String(compass.topElement || '').startsWith('#loading-overlay')) {
                fail(
                    state.name,
                    'visual-settle:.journey-compass-not-covered',
                    '.journey-compass center is covered by the loading overlay'
                )
            } else {
            ...
            for (const selector of lowerSurfaces) {
                ...
                if (!allowsLoadingOverlay && String(targetBox.topElement || '').startsWith('#loading-overlay')) {
                    fail(
                        state.name,
                        `visual-settle:${selector}-not-covered`,
                        `${selector} center is covered by the loading overlay`
                    )
                } else {
                ...
                if (rectsOverlap(compass, targetBox, 4)) {
                    fail(
                        state.name,
                        `surface-overlap:.journey-compass:${selector}`,
                        '.journey-compass overlaps lower panel surface'
                    )
                } else {
    ...
    if (shouldAssert('17-mobile-thread-inspector')) {
        ...
        const focusStageCard = requireRendered(
            '17-mobile-thread-inspector',
            'thread-inspector:focus-stage-card-visible',
            '.focus-stage-card'
        )
        const inspector = requireRendered(
            '17-mobile-thread-inspector',
            'thread-inspector:panel-visible',
            '#focus-thread-inspector'
        )
        const title = requireRendered(
            '17-mobile-thread-inspector',
            'thread-inspector:title-visible',
            '#focus-thread-inspector-title'
        )
        const copy = requireRendered(
            '17-mobile-thread-inspector',
            'thread-inspector:copy-visible',
            '#focus-thread-inspector-copy'
        )
        const meta = requireRendered(
            '17-mobile-thread-inspector',
            'thread-inspector:meta-visible',
            '#focus-thread-inspector-meta'
        )
        const pinBtn = requireRendered('17-mobile-thread-inspector', 'thread-inspector:pin-visible', '#btn-thread-pin')
        const followBtn = requireRendered(
            '17-mobile-thread-inspector',
            'thread-inspector:follow-visible',
            '#btn-thread-follow'
        )
        const clearBtn = requireRendered(
            '17-mobile-thread-inspector',
            'thread-inspector:clear-visible',
            '#btn-thread-clear'
        )
        ...
        if (
            inspectorState?.bodyDataset?.threadInspectSurface &&
            inspectorState.bodyDataset.threadInspectSurface !== 'idle'
        ) {
            pass('17-mobile-thread-inspector', 'thread-inspector:surface-state')
        } else {
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:surface-state',
                `expected active threadInspectSurface, got "${inspectorState?.bodyDataset?.threadInspectSurface || ''}"`
            )
        }
        ...
        if (inspector && withinViewport(inspector, viewport)) {
            ...
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:within-viewport',
                '#focus-thread-inspector extends outside mobile viewport'
            )
        }
        ...
        if (focusStage && withinViewport(focusStage, viewport)) {
            ...
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:focus-stage-within-viewport',
                '#focus-stage extends outside mobile viewport'
            )
        }
        if (focusStageCard && withinViewport(focusStageCard, viewport)) {
            ...
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:focus-stage-card-within-viewport',
                '.focus-stage-card extends outside mobile viewport'
            )
        }
        if (inspector?.centerTopInside) {
            ...
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:not-occluded',
                `inspector center is covered by ${inspector.topElement || 'nothing'}`
            )
        }
        if (!isRendered(searchContainer)) {
            ...
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:search-container-hidden',
                `.search-container should not duplicate focus context in active preview, got ${JSON.stringify(searchContainer)}`
            )
        }
        if (!isRendered(diveButton)) {
            ...
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:step-inside-hidden',
                `.focus-stage-dive-btn should not compete with thread actions, got ${JSON.stringify(diveButton)}`
            )
        }
        if (isVisible(nearbyStops) && nearbyStops.height < 40) {
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:nearby-stops-not-squeezed',
                `.focus-stage-neighbors is squeezed to ${Math.round(nearbyStops.height)}px`
            )
        } else {
        ...
        if (title?.text?.includes(' -> ')) {
            ...
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:title-copy',
                'thread inspector title does not include a real relationship arrow'
            )
        }
        if ((copy?.text || '').length >= 24) {
            ...
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:body-copy',
                'thread inspector body copy is empty or too short'
            )
        }
        if ((meta?.text || '').toLowerCase().includes('relationship')) {
            ...
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:meta-copy',
                'thread inspector meta does not include relationship source text'
            )
        }
        ...
        if (strandDiagnostics.active === true) {
            ...
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:strand-active',
                `inspectedStrandDiagnostics.active=${strandDiagnostics.active}`
            )
        }
        if ((strandDiagnostics.segmentCount || 0) > 0) {
            ...
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:strand-segments',
                `segmentCount=${strandDiagnostics.segmentCount || 0}`
            )
        }
        if ((strandDiagnostics.braidCount || 0) > 0) {
            ...
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:strand-braids',
                `braidCount=${strandDiagnostics.braidCount || 0}`
            )
        }
        if ((strandDiagnostics.endpointCount || 0) >= 2) {
            ...
            fail(
                '17-mobile-thread-inspector',
                'thread-inspector:strand-endpoints',
                `endpointCount=${strandDiagnostics.endpointCount || 0}`
            )
        }
        ...
        for (const [label, targetBox] of [
            ...
            if (touchTargetOk(targetBox)) {
                ...
                fail(
                    '17-mobile-thread-inspector',
                    `thread-inspector:${label}-touch-target`,
                    `${label} button is ${Math.round(targetBox.width)}x${Math.round(targetBox.height)}px`
                )
            }
        }
    }
    ...
    if (shouldAssert('21-mobile-route-trace-visible')) {
        ...
        if (diagnostics.active === true) {
            ...
            fail(
                '21-mobile-route-trace-visible',
                'route-trace:diagnostics-active',
                `routeTraceDiagnostics.active=${diagnostics.active}; reason=${diagnostics.reason || 'none'}`
            )
        }
        ...
        if ((diagnostics.segmentCount || diagnostics.lineSegmentCount || 0) > 0) {
            ...
            fail(
                '21-mobile-route-trace-visible',
                'route-trace:segment-count',
                `segmentCount=${diagnostics.segmentCount || 0}, lineSegmentCount=${diagnostics.lineSegmentCount || 0}`
            )
        }
        ...
        if (routeState?.bodyDataset?.routeMotion && routeState.bodyDataset.routeMotion !== 'inactive') {
            ...
            fail(
                '21-mobile-route-trace-visible',
                'route-trace:route-motion-active',
                `routeMotion=${routeState?.bodyDataset?.routeMotion || ''}`
            )
        }
    ...
    if (shouldAssert('18-mobile-loading-overlay')) {
        ...
        const progress = requireRendered(
            '18-mobile-loading-overlay',
            'loading-overlay:progress-visible',
            '.loading-progress'
        )
        const progressBar = requireRendered(
            '18-mobile-loading-overlay',
            'loading-overlay:progress-bar-visible',
            '#loading-progress-bar'
        )
        const phaseRow = requireRendered(
            '18-mobile-loading-overlay',
            'loading-overlay:phase-row-visible',
            '#loading-phase-row'
        )
        const phaseChip = requireRendered(
            '18-mobile-loading-overlay',
            'loading-overlay:phase-chip-visible',
            '.loading-phase-chip'
        )
        ...
        if (shell && withinViewport(shell, viewport)) {
            ...
            fail(
                '18-mobile-loading-overlay',
                'loading-overlay:shell-within-viewport',
                '.loading-shell extends outside mobile viewport'
            )
        }
        if (Number.parseInt(overlay?.zIndex || '0', 10) >= 999) {
            ...
            fail(
                '18-mobile-loading-overlay',
                'loading-overlay:overlay-layer',
                `expected overlay z-index >= 999, got ${overlay.zIndex || 'missing'}`
            )
        }
        if ((loadingState?.loadingOverlayDiagnostics?.phaseChipsCount || 0) >= 4) {
            ...
            fail(
                '18-mobile-loading-overlay',
                'loading-overlay:phase-chip-count',
                `expected >=4 phase chips, got ${loadingState?.loadingOverlayDiagnostics?.phaseChipsCount || 0}`
            )
        }
        if ((loadingState?.loadingOverlayDiagnostics?.activePhaseCount || 0) === 1) {
            ...
            fail(
                '18-mobile-loading-overlay',
                'loading-overlay:single-active-phase',
                `expected one active phase, got ${loadingState?.loadingOverlayDiagnostics?.activePhaseCount || 0}`
            )
        }
        if (title?.text?.includes('Growing the mycelium')) {
            ...
            fail(
                '18-mobile-loading-overlay',
                'loading-overlay:title-copy',
                'loading title does not include expected copy'
            )
        }
        if (note?.text?.includes('Montgomery County business records')) {
            ...
            fail(
                '18-mobile-loading-overlay',
                'loading-overlay:note-copy',
                'loading note does not include expected county-records copy'
            )
        }
        if (foot?.text?.includes('Semantic scene')) {
            ...
            fail(
                '18-mobile-loading-overlay',
                'loading-overlay:foot-copy',
                'loading foot does not include expected fixture copy'
            )
        }
        for (const [label, targetBox] of [
            ['kicker', kicker],
            ...
            if (targetBox && targetBox.width > 0 && targetBox.height > 0) {
                ...
                fail(
                    '18-mobile-loading-overlay',
                    `loading-overlay:${label}-has-area`,
                    `${label} has no measurable area`
                )
            }
        }
    }
    ...
    if (shouldAssert('19-mobile-compass-rail')) {
        ...
        if (compass && withinViewport(compass, viewport)) {
            ...
            fail(
                '19-mobile-compass-rail',
                'compass-rail:compass-within-viewport',
                '.journey-compass extends outside mobile viewport'
            )
        }
        if (rail && withinViewport(rail, viewport)) {
            ...
            fail(
                '19-mobile-compass-rail',
                'compass-rail:rail-within-viewport',
                '.journey-compass-rail extends outside mobile viewport'
            )
        }
        if (rail?.pointerEvents === 'none') {
            ...
            fail(
                '19-mobile-compass-rail',
                'compass-rail:not-occluded',
                `rail center is covered by ${rail.topElement || 'nothing'}`
            )
        }
        ...
        if ((diagnostics.stepsCount || 0) >= 4) {
            ...
            fail(
                '19-mobile-compass-rail',
                'compass-rail:step-count',
                `expected >=4 steps, got ${diagnostics.stepsCount || 0}`
            )
        }
        if (diagnostics.visibleStepsCount === diagnostics.stepsCount && diagnostics.stepsCount >= 4) {
            ...
            fail(
                '19-mobile-compass-rail',
                'compass-rail:steps-visible',
                `visible ${diagnostics.visibleStepsCount || 0} of ${diagnostics.stepsCount || 0} steps`
            )
        }
        if (!diagnostics.railOverflow) {
            ...
            fail(
                '19-mobile-compass-rail',
                'compass-rail:no-rail-overflow',
                'journey compass rail has horizontal overflow'
            )
        }
        if (
            (diagnostics.clippedStepsCount || 0) === 0 &&
            !diagnostics.kickerClipped &&
            !diagnostics.titleClipped &&
            !diagnostics.noteClipped
        ) {
            pass('19-mobile-compass-rail', 'compass-rail:no-text-clipping')
        } else {
            fail(
                '19-mobile-compass-rail',
                'compass-rail:no-text-clipping',
                `clipped steps=${diagnostics.clippedStepsCount || 0}, kicker=${Boolean(diagnostics.kickerClipped)}, title=${Boolean(diagnostics.titleClipped)}, note=${Boolean(diagnostics.noteClipped)}`
            )
        }
        if ((diagnostics.currentStepsCount || 0) === 1) {
            ...
            fail(
                '19-mobile-compass-rail',
                'compass-rail:single-current-step',
                `expected one current step, got ${diagnostics.currentStepsCount || 0}`
            )
        }
        const smallInteractiveTargets = (diagnostics.smallTouchTargets || []).filter(
        ...
        if (smallInteractiveTargets.length === 0) {
            ...
            fail(
                '19-mobile-compass-rail',
                'compass-rail:interactive-touch-targets',
                `small targets: ${smallInteractiveTargets.map((target) => `${target.text}:${Math.round(target.width)}x${Math.round(target.height)}`).join(', ')}`
            )
        }
        ...
        if (!isRendered(infoPanel)) {
            ...
            fail(
                '19-mobile-compass-rail',
                'compass-rail:map-idle-info-panel-hidden',
                `#info-panel should not own the map-idle overview, got ${JSON.stringify(infoPanel)}`
            )
        }
    ...
    if (shouldAssert('01-mobile-idle')) {
        ...
        const panelArea = infoPanel
            ? (infoPanel.width * infoPanel.height) / Math.max(1, viewport.width * viewport.height)
            : 0
        ...
        if (infoPanel && panelArea <= 0.42) {
            ...
            fail(
                '01-mobile-idle',
                'mobile-idle:overview-panel-density',
                `#info-panel area ratio ${panelArea.toFixed(3)} is too large for idle overview`
            )
        }
        if (searchContainer && withinViewport(searchContainer, viewport)) {
            ...
            fail(
                '01-mobile-idle',
                'mobile-idle:search-container-within-viewport',
                `.search-container should stay inside idle sheet, got ${JSON.stringify(searchContainer)}`
            )
        }
        if (!isRendered(launchButton)) {
            ...
            fail(
                '01-mobile-idle',
                'mobile-idle:launch-button-hidden',
                `#btn-launch competes with starter chips and search, got ${JSON.stringify(launchButton)}`
            )
        }
        if (!isRendered(demoStarters) && !isRendered(starterChip)) {
            ...
            fail(
                '01-mobile-idle',
                'mobile-idle:starter-ctas-hidden',
                `starter CTAs should not compete with search, demo=${JSON.stringify(demoStarters)} chip=${JSON.stringify(starterChip)}`
            )
        }
    }
    ...
    if (shouldAssert('20-mobile-mode-grid-visible')) {
        ...
        const activeChip = requireVisible(
            '20-mobile-mode-grid-visible',
            'mode-grid:active-chip-visible',
            '.mode-chip.active'
        )
        ...
        if (grid && withinViewport(grid, viewport)) {
            ...
            fail(
                '20-mobile-mode-grid-visible',
                'mode-grid:within-viewport',
                '#mode-grid extends outside mobile viewport'
            )
        }
        if (grid?.centerTopInside || grid?.pointerEvents === 'none') {
            ...
            fail(
                '20-mobile-mode-grid-visible',
                'mode-grid:occlusion',
                `mode grid center is covered by ${grid.topElement || 'nothing'}`
            )
        }
        if (!isVisible(box(modeState, '.demo-starter-chip'))) {
            ...
            fail(
                '20-mobile-mode-grid-visible',
                'mode-grid:demo-starters-hidden',
                'demo starter chips overlap the visible mode grid'
            )
        }
        ...
        if ((diagnostics.chipsCount || 0) >= 4) {
            ...
            fail(
                '20-mobile-mode-grid-visible',
                'mode-grid:chip-count',
                `expected >=4 mode chips, got ${diagnostics.chipsCount || 0}`
            )
        }
        if (diagnostics.visibleChipsCount === diagnostics.chipsCount && diagnostics.chipsCount >= 4) {
            ...
            fail(
                '20-mobile-mode-grid-visible',
                'mode-grid:chips-visible',
                `visible ${diagnostics.visibleChipsCount || 0} of ${diagnostics.chipsCount || 0} chips`
            )
        }
        ...
        if ((diagnostics.clippedChipsCount || 0) === 0) {
            ...
            fail(
                '20-mobile-mode-grid-visible',
                'mode-grid:no-chip-clipping',
                `${diagnostics.clippedChipsCount} mode chip labels are clipped`
            )
        }
        if ((diagnostics.activeChipsCount || 0) === 1 && diagnostics.activeChipAriaPressed === 'true') {
            ...
            fail(
                '20-mobile-mode-grid-visible',
                'mode-grid:active-chip-state',
                `active chips=${diagnostics.activeChipsCount || 0}, aria=${diagnostics.activeChipAriaPressed || 'missing'}`
            )
        }
        if ((diagnostics.smallTouchTargets || []).length === 0) {
            ...
            fail(
                '20-mobile-mode-grid-visible',
                'mode-grid:touch-targets',
                `small targets: ${diagnostics.smallTouchTargets.map((target) => `${target.text}:${Math.round(target.width)}x${Math.round(target.height)}`).join(', ')}`
            )
        }
        ...
        if (
            chip?.text?.length &&
            activeChip?.text?.includes('County View') &&
            name?.text?.length &&
            names.length >= 4
        ) {
    ...
    if (shouldAssert('25-mobile-search-no-results')) {
        ...
        if (noResultsState?.bodyDataset?.panelSurface === 'search') {
            ...
            fail(
                '25-mobile-search-no-results',
                'no-results:panel-surface-search',
                `expected search, got ${noResultsState?.bodyDataset?.panelSurface || 'none'}`
            )
        }
        if (isRendered(searchContainer)) {
            ...
            fail(
                '25-mobile-search-no-results',
                'no-results:search-container-visible',
                '.search-container should render for empty search'
            )
        }
        if (searchContainer?.className?.includes('results-rendered')) {
            ...
            fail(
                '25-mobile-search-no-results',
                'no-results:container-results-rendered',
                `.search-container should be terminal/results-rendered, got ${searchContainer?.className || 'missing'}`
            )
        }
        if (!searchContainer?.className?.includes('searching')) {
            ...
            fail(
                '25-mobile-search-no-results',
                'no-results:container-not-searching',
                '.search-container should not keep searching class'
            )
        }
        if (searchResults?.className?.includes('active')) {
            ...
            fail(
                '25-mobile-search-no-results',
                'no-results:search-results-active',
                `#search-results should be active, got ${searchResults?.className || 'missing'}`
            )
        }
        ...
        if (emptyTitle?.text?.trim() === 'No direct matches found') {
            ...
            fail(
                '25-mobile-search-no-results',
                'no-results:empty-title-copy',
                `unexpected empty title: ${emptyTitle?.text || 'missing'}`
            )
        }
        ...
        if (isRendered(suggestionChip)) {
            ...
            fail(
                '25-mobile-search-no-results',
                'no-results:suggestion-chip-visible',
                'expected at least one visible suggestion chip'
            )
        }
        if (
            isRendered(searchResults) &&
            isRendered(infoPanel) &&
            searchResults.y + searchResults.height <= infoPanel.y + infoPanel.height + 1
        ) {
            pass('25-mobile-search-no-results', 'no-results:results-within-panel')
        } else {
            fail(
                '25-mobile-search-no-results',
                'no-results:results-within-panel',
                `search results ${JSON.stringify(searchResults)} vs panel ${JSON.stringify(infoPanel)}`
            )
        }
        if (isRendered(searchResults) && searchResults.overflowY === 'auto') {
            ...
            fail(
                '25-mobile-search-no-results',
                'no-results:results-scroll-owner',
                `#search-results should own overflow-y:auto, got ${JSON.stringify(searchResults)}`
            )
        }
        if (!isRendered(shareToggle)) {
            ...
            fail(
                '25-mobile-search-no-results',
                'no-results:share-toggle-hidden',
                `.share-toggle should stay hidden, got ${JSON.stringify(shareToggle)}`
            )
        }
        if (!isRendered(controls)) {
            ...
            fail(
                '25-mobile-search-no-results',
                'no-results:controls-hidden',
                `.controls should stay hidden, got ${JSON.stringify(controls)}`
            )
        }
        if (viewport?.width <= 390 && (noResultsState?.scroll?.overflowX ?? 0) <= 1) {
            ...
            fail(
                '25-mobile-search-no-results',
                'no-results:no-horizontal-overflow',
                `viewport overflow: ${JSON.stringify(noResultsState?.scroll)}`
            )
        }
    }
    ...
    for (const state of summary.filter((entry) => entry?.sceneLuminance)) {
        ...
        const name = state.name
        const isFocusOrDive =
            (name.includes('focus') || name.includes('selected-card') || name.includes('dive')) &&
            !name.includes('field-node')
        ...
        if (scene.p95 > maxP95) {
            fail(state.name, 'scene-luminance:p95', `p95 luminance ${scene.p95} exceeds ${maxP95} in scene region`)
        } else {
    ...
    const labelBudgetFor = (state, diagnostics) => {
        const mode =
            diagnostics?.mode ||
            (state.name.includes('dive')
                ? 'inside'
                : state.name.includes('focus') || state.name.includes('selected-card')
                  ? 'focus'
                  : state.name.includes('search') || state.name.includes('route-trace')
                    ? 'search'
                    : 'overview')
    ...
    for (const state of summary.filter((entry) => clusterLabelStates.has(entry.name))) {
        ...
        if (labels.length <= maxLabels) {
            ...
            fail(
                state.name,
                'cluster-labels:budget',
                `visible labels ${labels.length} exceeds ${maxLabels}: ${labels.map((label) => label.text).join(', ')}`
            )
        }
        ...
        if ((diagnostics.offscreen || []).length === 0) {
            ...
            fail(
                state.name,
                'cluster-labels:inside-viewport',
                JSON.stringify((diagnostics.offscreen || []).slice(0, 4))
            )
        }
        ...
        if (reducedCard) {
            ...
            if (reducedCard.clusterRgb !== '78 205 196') {
                fail(
                    ...
                    'reduced-motion:selected-card:cluster-rgb',
            ...
            if (reducedCard.borderRadius !== '12px') {
                fail(
                    '12-desktop-reduced-motion',
                    'reduced-motion:selected-card:border-radius',
    ...
    if (shouldAssert('24-mobile-map-focus-search')) {
        ...
        const myceliumAction = box(
            mapFocusSearchState,
            '.map-trail-strip .trail-strip-btn[data-journey-action="open-mycelium"]'
        )
        const resetAction = box(
            mapFocusSearchState,
            '.map-trail-strip .trail-strip-btn[data-journey-action="county-overview"]'
        )
        const searchAction = box(
            mapFocusSearchState,
            '.map-trail-strip .trail-strip-btn[data-journey-action="focus-search"]'
        )
        ...
        const isNonInteractiveTitleRendered = (targetBox) =>
            targetBox &&
        ...
        if (
            isVisibleBox(actionRow) ||
            isVisibleBox(trailControls) ||
            isVisibleBox(trailContext) ||
            isVisibleBox(filtersSection)
        ) {
            fail(
                '24-mobile-map-focus-search',
    ...
    if (shouldAssert('16-desktop-info-panel-populated')) {
        ...
        if (!isRendered(modeGrid)) {
            ...
            fail(
                '16-desktop-info-panel-populated',
                'info-panel-populated:mode-grid-hidden',
                'mode grid should be hidden in populated focus panel state'
            )
        }
        ...
        if (isVisible(compassNote)) {
            ...
            fail(
                '16-desktop-info-panel-populated',
                'info-panel-populated:compass-note-visible',
        ...
        if (focusStageCard?.text && !/Business Name|What they do/.test(focusStageCard.text)) {
            ...
            fail(
                '16-desktop-info-panel-populated',
                'info-panel-populated:focus-stage-populated-text',
                'focus-stage card does not include populated business copy'
            )
        }
    ...
    if (shouldAssert('03-mobile-focus-first-result')) {
        ...
        if (panelSurface === 'focus' || panelSurface === 'focus-search') {
            ...
            fail(
                '03-mobile-focus-first-result',
                'mobile-focus:panel-surface-focus',
                `expected focus/focus-search, got ${panelSurface || 'none'}`
            )
        }
        if (isVisible(focusStage)) {
            ...
            fail(
                '03-mobile-focus-first-result',
                'mobile-focus:focus-stage-visible',
                '#focus-stage should render after entering focus'
            )
        }
        if (isRendered(focusCard)) {
            ...
            fail(
                '03-mobile-focus-first-result',
                'mobile-focus:focus-card-visible',
                '.focus-stage-card should render after entering focus'
            )
        }
        ...
        if (!isRendered(svelteCompassRail)) {
            ...
            fail(
                '03-mobile-focus-first-result',
                'mobile-focus:svelte-compass-rail-hidden',
                `#compass-rail should not compete with compact focus chrome, got ${JSON.stringify(svelteCompassRail)}`
            )
        }
        ...
        if (!isRendered(searchContainer)) {
            ...
            fail(
                '03-mobile-focus-first-result',
                'mobile-focus:search-container-hidden',
                `.search-container should hand off to the focus stage, got ${JSON.stringify(searchContainer)}`
            )
        }
        ...
        if (focusState?.bodyDataset?.threadInspectSurface === 'idle' && isVisible(threadInspector)) {
            fail(
                '03-mobile-focus-first-result',
                'mobile-focus:idle-thread-preview-hidden',
                '#focus-thread-inspector should not render before a neighbor preview exists'
            )
        } else {
        ...
        if (!isRendered(selectedCard)) {
            ...
            fail(
                '03-mobile-focus-first-result',
                'mobile-focus:legacy-selected-card-hidden',
                `.selected-card should not compete with #focus-stage on mobile focus, got ${JSON.stringify(selectedCard)}`
            )
        }
        ...
        if (!isRendered(networkKey)) {
            ...
            fail(
                '03-mobile-focus-first-result',
                'mobile-focus:passive-network-key-hidden',
                `#canvas-color-legend should collapse once focus-stage owns the mobile cockpit, got ${JSON.stringify(networkKey)}`
            )
        }
        ...
        if (!isRendered(focusWhat)) {
            ...
            fail(
                '03-mobile-focus-first-result',
                'mobile-focus:instructional-copy-hidden',
                `.focus-stage-what should not insert instructional copy into the selected-business cockpit, got ${JSON.stringify(focusWhat)}`
            )
        }
        ...
        if (instructionalFragments.length === 0) {
            ...
            fail(
                '03-mobile-focus-first-result',
                'mobile-focus:journey-instructions-hidden',
                `journey instruction fragments should not compete inside the selected-business cockpit: ${JSON.stringify(instructionalFragments)}`
            )
        }
        ...
        if (!isRendered(diveButton)) {
            ...
            fail(
                '03-mobile-focus-first-result',
                'mobile-focus:primary-action-after-neighbor',
                `primary action should sit below the neighbor lane when a neighbor is visible: action=${JSON.stringify(diveButton)} neighbor=${JSON.stringify(firstNeighbor)}`
            )
        }
        if (isRendered(diveButton) && isRendered(firstNeighbor)) {
            if (!rectsOverlap(diveButton, firstNeighbor, 2)) {
                ...
                fail(
                    '03-mobile-focus-first-result',
                    'mobile-focus:primary-action-neighbor-no-overlap',
                    `primary action must not overlap the visible neighbor pill: action=${JSON.stringify(diveButton)} neighbor=${JSON.stringify(firstNeighbor)}`
                )
            }
        }
        ...
        const overlappingNeighbor = visibleNeighbors.find(
            (neighbor) => isRendered(diveButton) && rectsOverlap(diveButton, neighbor, 2)
        )
    ...
    if (shouldAssert('04-mobile-field-node-active')) {
        ...
        if (focusPanelMode === 'field-node') {
            ...
            fail(
                '04-mobile-field-node-active',
                'field-node:focus-panel-mode',
                `expected field-node, got ${focusPanelMode || 'none'}`
            )
        }
        if (panelSurface === 'focus-search' || panelSurface === 'focus') {
            ...
            fail(
                '04-mobile-field-node-active',
                'field-node:panel-surface-focus',
                `expected focus/focus-search, got ${panelSurface || 'none'}`
            )
        }
        if (isRendered(compass)) {
        ...
        if (isVisible(focusStage)) {
            ...
            fail(
                '04-mobile-field-node-active',
                'field-node:focus-stage-visible',
                '#focus-stage should render in field-node mode'
            )
        }
        ...
        if (!isRendered(networkKey)) {
            ...
            fail(
                '04-mobile-field-node-active',
                'field-node:passive-network-key-hidden',
                `#canvas-color-legend should not compete with the field-node cockpit, got ${JSON.stringify(networkKey)}`
            )
        }
    }
    ...
    if (shouldAssert('09-mobile-map-empty-state')) {
        ...
        if (
            isVisible(emptyBox) &&
            emptyBox.height <= 236 &&
            emptyBox.width <= Math.min(300, viewportFor(emptyState).width - 48)
        ) {
            pass('09-mobile-map-empty-state', 'map-empty:compact-card-proportion')
        } else if (isVisible(emptyBox)) {
            fail(
                '09-mobile-map-empty-state',
                'map-empty:compact-card-proportion',
                `.map-empty-state should stay compact, got ${JSON.stringify(emptyBox)}`
            )
        }
        if (isRendered(viewToggle) && withinViewport(viewToggle, viewportFor(emptyState))) {
            ...
            fail(
                '09-mobile-map-empty-state',
                'map-empty:view-toggle-within-viewport',
                `.view-toggle should stay inside map-idle viewport, got ${JSON.stringify(viewToggle)}`
            )
        }
        ...
        if (
            emptyState?.bodyDataset?.panelSurface === 'map-idle' &&
            emptyState?.bodyDataset?.journeyCompassDensity === 'hidden'
        ) {
            if (!isRendered(compass)) {
                ...
                fail(
                    '09-mobile-map-empty-state',
                    'map-empty:hidden-density-compass-suppressed',
                    `density=hidden should suppress .journey-compass, got ${JSON.stringify(compass)}`
                )
            }
        }
        if (emptyState?.bodyDataset?.panelSurface === 'map-idle') {
            if (utilityChrome.length === 0) {
                ...
                fail(
                    '09-mobile-map-empty-state',
                    'map-empty:utility-chrome-suppressed',
                    `map-idle should not show standalone utility chrome: ${JSON.stringify(utilityChrome)}`
                )
            }
        }
    }
    ...
    if (shouldAssert('14-desktop-search-error')) {
        const _errorState = requireState('14-desktop-search-error')
        for (const selector of [
            '.search-error-state',
            '.search-error-kicker',
            '.search-error-retry-btn',
    ...
    if (shouldAssert('22-mobile-semantic-dive-320')) {
        ...
        if (dive320State.scroll.overflowX > 0) {
            fail(
                '22-mobile-semantic-dive-320',
                'no-overflow-x',
                `horizontal overflow ${dive320State.scroll.overflowX}px`
            )
        } else {
        ...
        if (dive320State.bodyDataset?.panelSurface === 'semantic-dive') {
            ...
            fail(
                '22-mobile-semantic-dive-320',
                'semantic-dive-320:panel-surface',
        ...
        if (dive320State.bodyDataset?.semanticDive === 'active') {
            ...
            fail(
                '22-mobile-semantic-dive-320',
                'semantic-dive-320:semantic-dive-active',
        ...
        if (focusStage && withinViewport(focusStage, viewport)) {
            ...
            fail(
                '22-mobile-semantic-dive-320',
                'semantic-dive-320:focus-stage-within-viewport',
    ...
    if (shouldAssert('23-mobile-short-landscape')) {
        ...
        if (focusStage && withinViewport(focusStage, slViewport)) {
            ...
            fail(
                '23-mobile-short-landscape',
                'short-landscape:focus-stage-within-viewport',
        ...
        if (diveButton && isRendered(diveButton) && withinViewport(diveButton, slViewport)) {
            ...
            fail(
                '23-mobile-short-landscape',
                'short-landscape:dive-button-within-viewport',
        ...
        if (!isRendered(neighborRail) || withinViewport(neighborRail, slViewport)) {
            ...
            fail(
                '23-mobile-short-landscape',
                'short-landscape:neighbor-rail-not-escaping',
        ...
        if (infoPanel && withinViewport(infoPanel, slViewport)) {
            ...
            fail(
                '23-mobile-short-landscape',
                'short-landscape:info-panel-within-viewport',
        ...
        if (isRendered(viewToggle) && withinViewport(viewToggle, slViewport)) {
            ...
            fail(
                '23-mobile-short-landscape',
                'short-landscape:view-toggle-within-viewport',
        ...
        for (const selector of [
            ...
            if (isRendered(chrome)) {
                fail(
                    '23-mobile-short-landscape',
                    `short-landscape:utility-chrome-hidden:${selector}`,
        ...
        if (isRendered(networkKey)) {
            fail(
                '23-mobile-short-landscape',
                'short-landscape:network-key-hidden',
        ...
        for (const selector of ['#btn-legend', '#btn-keyboard-help']) {
            ...
            if (isRendered(chrome) && chrome.width >= 44 && chrome.height >= 44 && withinViewport(chrome, slViewport)) {
                ...
                fail(
                    '23-mobile-short-landscape',
                    `short-landscape:utility-chrome-tappable:${selector}`,
        ...
        if (isRendered(compassNote)) {
            fail(
                '23-mobile-short-landscape',
                'short-landscape:compass-note-hidden',
