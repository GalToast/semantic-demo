...
console.log(`Viewport: ${SHORT_LANDSCAPE.width}x${SHORT_LANDSCAPE.height} (short landscape)\n`);
...
console.log('[TEST] Canopy HUD — journey-compass bounding rect at 667x375');
...
//
// At short-landscape (667x375), the focus-stage CSS may apply opacity:0 or
// visibility:hidden to hide it rather than letting it overflow. If the element
// is effectively hidden by the short-landscape CSS, we skip the overflow test
// (it is correctly handled by the CSS). Only assert if it is visibly rendered.

console.log('[TEST] Focus Stage — #focus-stage bounding rect at 667x375');
...
if (!fsIsVisible) {
  ...
  // (CSS hides it instead of letting it overflow), so we skip the overflow test.
...
console.log('[TEST] Info Panel — #info-panel bounding rect at 667x375');
...
// Only check overlap when BOTH are visibly rendered.

console.log('[TEST] No overlap — journey-compass vs #focus-stage at 667x375');
...
if (overlapInfo) {
  ...
  // Vertical: stage should start at or below compass bottom (no overlap below)
  const verticalOverlap = stageTop < compassBottom;
  // Horizontal: horizontal overlap only if both spans overlap on x-axis
...
console.log('[TEST] No overflow — all visible key elements at 667x375');
