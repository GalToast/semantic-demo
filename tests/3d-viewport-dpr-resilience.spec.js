...
async function openPage(browser, { width, height, deviceScaleFactor = 1 }) {
    ...
    await page.goto(`${BASE_URL}/dist/svelte/index.html?view=galaxy&nodemo=1`, { waitUntil: 'domcontentloaded' })
...
function assertViewportHealth(diag, label) {
    ...
    expect(
        Math.abs(diag.cameraAspect - diag.canvasCSS.width / diag.canvasCSS.height),
        `${label}: camera aspect should match canvas`
    ).toBeLessThan(0.04)
    expect(diag.backing.width, `${label}: backing width should not undershoot CSS width`).toBeGreaterThanOrEqual(
        Math.floor(diag.canvasCSS.width)
    )
    expect(diag.backing.height, `${label}: backing height should not undershoot CSS height`).toBeGreaterThanOrEqual(
        Math.floor(diag.canvasCSS.height)
    )
    expect(diag.finite, `${label}: should project a useful node sample`).toBeGreaterThan(12)
    expect(
        diag.insideRatio,
        `${label}: most sampled nodes should be in front/in-bounds; diag=${JSON.stringify(diag)}`
...
test.describe('3D viewport and DPR resilience', () => {
    ...
    for (const scenario of cases) {
        test(`${scenario.label}: canvas, camera, and projection stay coherent`, async ({ browser }) => {
            ...
            try {
                ...
                expect(
                    Math.abs(diag.devicePixelRatio - scenario.deviceScaleFactor),
                    `${scenario.label}: browser context DPR should match`
                ).toBeLessThan(0.05)
                ...
                if (scenario.deviceScaleFactor > 1) {
                    ...
                    expect(
                        diag.backing.width,
                        `${scenario.label}: backing width should reflect high-DPI context`
                    ).toBeGreaterThanOrEqual(Math.floor(minWidth))
                    expect(
                        diag.backing.height,
                        `${scenario.label}: backing height should reflect high-DPI context`
                    ).toBeGreaterThanOrEqual(Math.floor(minHeight))
                }
            } finally {
    ...
    test('resize: camera aspect follows canvas after desktop-to-mobile resize', async ({ browser }) => {
        ...
        try {
            ...
            expect(after.canvasCSS.width, 'canvas should follow resized viewport width').toBeLessThan(
                before.canvasCSS.width
            )
            expect(after.cameraAspect, 'camera aspect should change after resize').not.toBeCloseTo(
                before.cameraAspect,
                2
            )
        } finally {
