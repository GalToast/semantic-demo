 ...
 *
 * Run via:
 *   npx playwright test tests/3d-focus-ghost-graph-visibility.spec.js --browser=chromium --headed
...
async function probeGhostGraph(page) {
    return page.evaluate(
        ({ baseOpacity, sceneOpacityScale, focusOpacityScale }) => {
    ...
    return page.evaluate(
        (config) => {
            ...
            const expectedOpacity = isFocused
                ? config.baseOpacity *
                  config.sceneOpacityScale *
                  (trailDepth >= 2 ? config.semanticDiveOpacityScale : config.focusOpacityScale)
...
test.describe('3d-focus-ghost-graph-visibility', () => {
    ...
    test('desktop: whole-graph point cloud remains legible during focus mode', async ({ page }) => {
        ...
        await page.waitForFunction(
            () => Number.isFinite((window.__APP_STATE__ ?? window.__TEST_STATE__ ?? {}).focusedNode),
            { timeout: 15000 }
        )
        await page
    ...
    test('desktop: whole-graph point cloud remains visible in semantic-dive mode', async ({ page }) => {
        ...
        expect(afterDive2.currentOpacity, 'point cloud remains visible but quieter in semantic-dive').toBeGreaterThan(
            0.08
        )
    ...
    test('desktop: out-of-pocket nodes have finite in-canvas screen coordinates during focus mode', async ({
        page
    }) => {
        test.setTimeout(FOCUS_GHOST_TIMEOUT_MS)
        await openApp(page, { width: 1440, height: 900 })
        ...
        expect(
            snap.outOfPocketInCanvasCount,
            'at least some out-of-pocket nodes must project into the canvas'
        ).toBeGreaterThan(0)
    ...
    test('desktop: out-of-pocket in-canvas nodes are reachable (not blocked by overlays)', async ({ page }) => {
        ...
        expect(
            reachableCount,
            `at least one out-of-pocket ghost node must be canvas-reachable; got ${reachableCount}/${snap.outOfPocketInCanvasSample.length}`
        ).toBeGreaterThan(0)
    })
    ...
    test('short-landscape: ghost graph is present and in-canvas at 844×390', async ({ page }) => {
        ...
        expect(['focus', 'trail'], 'short-landscape focused traversal should be in focus/trail mode').toContain(
            snap.mode
        )
        expect(snap.pocketSize, 'pocket must have at least 1 node at short-landscape').toBeGreaterThan(0)
        expect(
            snap.outOfPocketInCanvasCount,
            'at least some out-of-pocket nodes must be in canvas at short-landscape'
        ).toBeGreaterThan(0)
        ...
        expect(
            err < 30, // looser tolerance for smaller viewport
