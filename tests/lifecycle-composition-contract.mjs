...
const { refreshCompositionState } = await import('../src/lib/stores/lifecycle.ts')
const { setSemanticLaneUiState } = await import('../src/lib/orchestration/lifecycle.ts')
...
// applyCompositionState (refreshCompositionState) has been migrated to read from Svelte stores.
...
assert(
    ...
    'focused record plus degraded search intent owns focus-search context'
)
assert(
    document.body.dataset.panelSurface === 'focus-search',
    'focused record plus search intent owns the focus-search panel surface'
)
...
assert(
    lanePill.getAttribute('aria-label') === 'Search is recovering in the background.',
    'lane pill keeps accessible compact status'
)
