...
function sourceFiles() {
    ...
    return [
        ...
        return (
            rel !== path.join('tests', 'demo-camera-retirement-contract.mjs') &&
...
test('no source file imports from demo-camera.js specifically', () => {
    ...
    for (const file of sourceFiles()) {
        ...
        for (const match of importMatches) {
            ...
            assert(
                match.includes(activeModule),
                `${path.relative(ROOT, file)} imports from retired ${retiredModule}: ${match}`
            )
        }
    }
})
...
test('demo-controller.js does not reference demo-camera', () => {
    ...
    if (!fs.existsSync(p)) {
        ok('demo-controller.js absent (already retired)')
        return
    }
