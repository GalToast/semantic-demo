...
const BRIDGE_ALLOWLIST = new Set(['src/lib/engine'])

/** Current anti-pattern import count — workers SHOULD reduce this over time */
const APPROVED_ANTIPATTERN_COUNT = 17 // W15 Wave E: event-bindings.ts canonical port adds 13 legacy binding imports (bindings/* still in js/modules/), will resolve in W16 death-bridge sweep
...
function isInAllowlist(file: string): boolean {
    ...
    return Array.from(BRIDGE_ALLOWLIST).some((allowed) => relDir === allowed || relDir.startsWith(allowed + '/'))
}
...
describe('Svelte-bridge import contract (S7)', () => {
    ...
    it('no direct js/ imports in non-engine files (the contract)', () => {
        ...
        if (violations.length > 0) {
            const summary = violations.map((v) => `  ${v.file}  →  ${v.importPath}`).join('\n')
