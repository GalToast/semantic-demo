...
try {
  ...
  manifestEntries = Object.values(groups)
    .flatMap((g) => ((g as { contracts?: unknown[] } | undefined)?.contracts ?? []).map(String))
} catch {
...
test('every contract-ish file on disk is registered in the manifest', () => {
  const missing = onDisk.filter((f) => !manifestEntries.some((e) => {
    if (e === f) return true
