 ...
 * - D1: `supportsCapableWebGL()` is the probe behind the (default-OFF)
 *   `VITE_S5_AUTO_ENTER_3D` flag. Tests inject a `ProbeEnv` seam — no global
 *   stubbing (happy-dom globals are non-redefinable in vitest here).
 */
...
const SRC = readFileSync(
    resolve(__dirname, '../../src/components/Placeholder2D.svelte'),
    'utf8',
)

const {
    supportsCapableWebGL,
    S5_AUTO_ENTER_AFTER,
} = await import(
...
describe('S5 capability probe (S5-D1)', () => {
    it('auto-enter flag is OFF by default (opt-in, waits for phone-farm smoke)', () => {
        expect(S5_AUTO_ENTER_AFTER).toBe(false)
    })
