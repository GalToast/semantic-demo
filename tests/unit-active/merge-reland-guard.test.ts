...
describe('merge-reland guard', () => {
  it('recent merge commits do not resurrect stale file content from the overwritten side', () => {
    ...
    for (const { sha, subject } of merges) {
      ...
      const conflictCandidates = runGit(`diff-tree --no-commit-id -r --name-only ${firstParent} ${secondParent} -- src tests scripts`)
        .split('\n')
        .filter(Boolean)
      ...
      if (stale.length > 0) {
        findings.push(`${sha.slice(0, 7)} ${subject.slice(0, 60)} :: merge took the STALE parent's content on ${stale.join(', ')}`)
      }
    }
