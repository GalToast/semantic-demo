...
const INFO_PANEL_CONDITIONAL_CLASSES = ['selected-card-empty']
...
function testInfoPanelOwnsAllSurfaceIds() {
    ...
    for (const cls of INFO_PANEL_CONDITIONAL_CLASSES) {
        assert(src.includes(`class:${cls}=`), `InfoPanel.svelte must use conditional class ${cls}`)
    }
...
function testRetiredComponentsRemoved() {
    for (const rel of RETIRED_COMPONENT_PATHS) {
        assert(!exists(rel), `retired component ${rel} must not be restored`)
    }
...
function testInfoPanelStateOwnsContentDescriptors() {
    ...
    const requiredSurfaces = ['idle', 'focus', 'focus-search', 'search', 'semantic-dive']
    for (const surface of requiredSurfaces) {
        ...
        assert(hasDescriptor, `info-panel-state.ts must have content descriptor for surface "${surface}"`)
    }

    // Must have selectionSuppressed for search surface
    assert(/selectionSuppressed/.test(src), 'info-panel-state.ts must define selectionSuppressed for search surfaces')
    ...
    assert(/panelVisible/.test(src), 'info-panel-state.ts must define panelVisible for map surfaces')
...
function testViewModelExposesContract() {
    ...
    for (const field of requiredFields) {
        assert(new RegExp(`\\b${field}\\b`).test(src), `view-model must expose ${field} in SelectedBusinessProps`)
    }

    // Must handle null point (empty state)
    assert(/if\s*\(\s*!point\s*\)/.test(src), 'view-model must handle null point for empty state')
...
function testHtmlShellNoStaleSlots() {
    ...
    assert(!html.includes('info-panel-chrome-island'), 'HTML shell must not expose obsolete info-panel chrome slot')
    assert(!html.includes('legend-panel-chrome-island'), 'HTML shell must not expose obsolete legend-panel chrome slot')
...
function testInfoPanelReadsParityAttrs() {
    ...
    assert(/panelOpen/.test(src), 'InfoPanel.svelte must derive panelOpen from surface state')

    // Must derive isEmpty from selectedRecord
    assert(/isEmpty/.test(src), 'InfoPanel.svelte must derive isEmpty from selectedRecord presence')
...
function testIdleSurfaceClearsSelectedRecord() {
    ...
    assert(
        /let\s+selectedRecord\s*=\s*\$derived\.by\s*\(\s*\(\)\s*=>\s*\{[\s\S]*?if\s*\(\s*effectiveSurface\s*===\s*['"]idle['"]\s*\)\s*return\s+null\s*;/.test(
            src
        ),
