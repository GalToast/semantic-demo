...
async function expectOverviewReset(page, label) {
    await expect
        .poll(
            ...
            { message: `${label}: overview reset state`, timeout: 15000 }
        )
        .toEqual({
            navMode: 'overview',
            focusedNode: null,
...
test('semantic-dive reset also clears focus-panel overlap', async ({ page }) => {
    ...
    // Wait for overview reset
    await page.waitForFunction(
        () => window.__TEST_STATE__?.navState?.mode === 'overview' && window.__TEST_STATE__?.semanticDiveMode === false,
        { timeout: 15000 }
    )
    ...
    expect(
        focusStageCleared || focusPanelCleared,
        'focus stage or panel must be cleared after semantic-dive reset'
    ).toBe(true)
})
