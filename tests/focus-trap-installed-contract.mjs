...
try {
    ...
    assert(
        !/if\s*\(\s*(typeof\s+window[^)]*|import\.meta\.env[^)]*|document\.readyState[^)]*)[^)]*\)\s*\{[\s\S]{0,400}bindFocusTrapObserver\s*\(/.test(mainSrc),
    ...
    assert(
        /beforeunload|disposeAppListeners|import\.meta\.hot\.dispose|disposeFocusTrapBindings\s*\(\s*\)\s*\{/.test(surrounding),
    ...
    assert(
        /\.observe\s*\(\s*document\.body\s*,\s*\{[\s\S]*?attributeFilter\s*:\s*\[\s*['"]data-panel-surface['"]\s*\]/.test(bindingsSrc),
