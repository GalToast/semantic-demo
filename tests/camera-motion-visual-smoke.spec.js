...
async function runCoffeeFocusFlow(page, { requireCorridor = true } = {}) {
    ...
    await page.waitForFunction(
        () => {
            const state = window.__APP_STATE__ ?? window.__TEST_STATE__ ?? {}
            return (
                Array.isArray(state.currentSearchSummary?.resultIndices) &&
    ...
    await page
        .waitForFunction(() => (window.__APP_STATE__ ?? window.__TEST_STATE__)?.semanticDiveMode === true, null, {
            timeout: 8000
        })
        .catch(() => {})
    await page
    ...
    const inside = await page.evaluate(() => {
        ...
        const rects = Array.from(
            document.querySelectorAll('#btn-focus-dive, #btn-inside-next, #btn-inside-county')
        ).map((el) => {
