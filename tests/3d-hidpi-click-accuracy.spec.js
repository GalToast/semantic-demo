...
test.describe('3D HiDPI click accuracy (deviceScaleFactor=2)', () => {
    test('desktop DPR=2: click on projected canvas node focuses a valid node with correct identity', async ({
        browser
    }) => {
        ...
        try {
            ...
            expect(isValidNodeIndex(after.focusedNode, after.pointCount), 'DPR=2 click must focus a valid node').toBe(
                true
            )
            ...
            expect(
                pickDist,
                `DPR=2 pick distance=${Math.round(pickDist)}px must be within tolerance`
            ).toBeLessThanOrEqual(64)

            // Focused node identity must match the hovered node that was clicked
            expect(after.focusedNode, 'focusedNode must equal the resolved hover index at DPR=2').toBe(
    ...
    test('desktop DPR=2: label content is legible after focus via click', async ({ browser }) => {
        ...
        try {
            ...
            expect(
                legibility.panelHasContent || legibility.hasLabel,
                'DPR=2 focus state should produce non-empty label/panel content'
            ).toBe(true)
    ...
    test('mobile portrait DPR=2: tap on projected canvas node focuses a valid node', async ({ browser }) => {
        ...
        try {
            ...
            expect(
                candidates.length,
                'mobile DPR=2 must expose at least one projected canvas candidate'
            ).toBeGreaterThan(0)
            ...
            expect(
                ...
                'mobile DPR=2 tap must focus a valid node'
            ).toBe(true)
    ...
    test('short landscape DPR=2: projected node tap yields valid focus', async ({ browser }) => {
        ...
        try {
            ...
            expect(
                isValidNodeIndex(after.focusedNode, after.pointCount),
                'short-landscape DPR=2 tap must focus a valid node'
            ).toBe(true)
    ...
    test('desktop DPR=2: click-away does not create garbage focus state', async ({ browser }) => {
        ...
        try {
            ...
            expect(noFocus || validFocus, 'DPR=2 away-click must leave focusedNode null or valid, not garbage').toBe(
                true
            )
        } finally {
            if (context) await context.close().catch(() => {})
    ...
    test('desktop DPR=2: canvas backing store reflects DPR=2 (not DPR=1)', async ({ browser }) => {
        ...
        try {
            ...
            expect(
                backingRatioW,
                `DPR=2 backing width ratio should be ~2, got ${backingRatioW.toFixed(2)}`
            ).toBeGreaterThan(1.8)
            expect(
                backingRatioH,
                `DPR=2 backing height ratio should be ~2, got ${backingRatioH.toFixed(2)}`
            ).toBeGreaterThan(1.8)
        } finally {
            if (context) await context.close().catch(() => {})
        }
