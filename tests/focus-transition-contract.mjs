    ...
    return page.evaluate(() => {
        ...
        if (typeof s.buildSemanticMyceliumEdges === 'function') {
            ...
            if (edges) {
                ...
                passes.push(
                    `buildSemanticMyceliumEdges: core=${corePairs.length} wispy=${wispyPairs.length} bridge=${bridgePairs.length}`
                )
...
async function auditMyceliumDensity(page) {
    return page.evaluate(() => {
        ...
        const scores = candidates.map((c) => c.score ?? c.semanticScore ?? 0).filter((v) => Number.isFinite(v))
