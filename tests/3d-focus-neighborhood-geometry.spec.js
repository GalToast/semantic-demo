...
async function probeNeighborhood(page) {
    return page.evaluate(() => {
        ...
        const roles = nav.focusPocketRoleByIndex ? Object.fromEntries(nav.focusPocketRoleByIndex) : {}
...
test.describe('focus-neighborhood geometry', () => {
    test('desktop: non-anchor pocket nodes have finite screen coordinates in unobstructed canvas', async ({ page }) => {
        ...
        expect(
            snap.reachableCount,
            `at least one non-anchor neighbor must be on-screen and reachable, got ${snap.reachableCount} of ${projected.length}`
        ).toBeGreaterThan(0)

        const onCanvasProjected = projected.filter((n) => n.hasScreen && n.inCanvas)
        expect(
            onCanvasProjected.length,
            `at least one non-anchor neighbor must project inside the canvas; projected=${JSON.stringify(projected)}`
        ).toBeGreaterThan(0)
    ...
    test('desktop: non-anchor neighbors are role-distinguishable from anchor (anchor=anchor, neighbor=primary/support/halo)', async ({
        page
    }) => {
        test.setTimeout(FOCUS_NEIGHBORHOOD_TEST_TIMEOUT_MS)
        await openApp(page, { width: 1440, height: 900 })
        ...
        for (const [idx, role] of Object.entries(snap.roles)) {
            ...
            expect(validRoles.has(role), `neighbor idx=${idx} must have role primary/support/halo, got "${role}"`).toBe(
                true
            )
        }
    ...
    test('desktop: non-anchor neighbors have minimum screen distance from anchor (not co-located)', async ({
        page
    }) => {
        test.setTimeout(FOCUS_NEIGHBORHOOD_TEST_TIMEOUT_MS)
        await openApp(page, { width: 1440, height: 900 })
        ...
        expect(
            separatedCount,
            `at least one non-anchor neighbor must be ≥${MIN_DISTANCE_PX}px from anchor, got ${separatedCount}/${nonAnchorProjected.length} separated`
        ).toBeGreaterThan(0)
    })
    ...
    test('desktop: anchor/primary/support/halo pocket nodes have mathematically distinct spore scales', async ({
        page
    }) => {
        test.setTimeout(FOCUS_NEIGHBORHOOD_TEST_TIMEOUT_MS)
        await openApp(page, { width: 1440, height: 900 })
        ...
        expect(
            uniqueScales.length,
            `pocket must expose at least 2 distinct scale values across roles; got ${uniqueScales.join(', ')}`
        ).toBeGreaterThan(1)
    ...
    test('tablet: non-anchor neighbors have minimum screen distance from anchor at 1024x768', async ({ page }) => {
        ...
        const nonAnchorProjected = snap.projected.filter((n) => n.idx !== snap.focusedIndex && n.hasScreen)
        expect(
            nonAnchorProjected.length,
            'tablet must have at least one non-anchor projected neighbor'
        ).toBeGreaterThan(0)
        ...
        expect(
            separatedCount,
            `tablet: at least one non-anchor neighbor must be ≥${MIN_DISTANCE_PX}px from anchor, got ${separatedCount}/${nonAnchorProjected.length}`
        ).toBeGreaterThan(0)
        ...
        expect(
            unobstructed.length,
            `tablet: non-anchor neighbors must not be occluded by UI overlays; onCanvas=${onCanvasProjected.length}`
        ).toBeGreaterThan(0)
    })
    ...
    test('mobile-portrait: non-anchor neighbors are visible and not occluded at 390x844', async ({ page }) => {
        ...
        expect(
            onScreen.length,
            'at least one non-anchor neighbor must be on-screen at mobile-portrait'
        ).toBeGreaterThan(0)
        ...
        expect(
            unobstructed.length,
            `mobile-portrait: non-anchor neighbors must not be occluded; onScreen=${onScreen.length}, inCanvas=${inCanvas.length}`
        ).toBeGreaterThan(0)
        ...
        expect(
            separatedCount,
            `mobile-portrait: at least one neighbor must be ≥${MIN_DISTANCE_PX}px from anchor`
        ).toBeGreaterThan(0)
    })
    ...
    test('mobile-portrait: spore scale differentiation holds at 390x844', async ({ page }) => {
        ...
        expect(
            uniqueScales.length,
            `mobile-portrait pocket must expose ≥2 distinct scales; got ${uniqueScales.length}`
        ).toBeGreaterThan(1)
    })
    ...
    test('short-landscape: spore scale differentiation holds at 844x390', async ({ page }) => {
        ...
        expect(
            uniqueScales.length,
            `short-landscape pocket must expose ≥2 distinct scales; got ${uniqueScales.length}`
        ).toBeGreaterThan(1)
    })
})
