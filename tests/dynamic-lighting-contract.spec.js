...
async function openApp(page, viewport = { width: 1440, height: 900 }) {
    ...
    await page
        .waitForFunction(() => Boolean(window.__TEST_STATE__?.myceliumCoreLines?.material), { timeout: 10000 })
        .catch(() => {})
}
...
test.describe('dynamic-lighting: mycelium opacity responds to focus state', () => {
    ...
    test('overview opacities are at overview-mode values', async ({ page }) => {
        ...
        for (const [key, opacity] of available) {
            expect(opacity, `${key} opacity should be near overview value ${OVERVIEW_TARGETS[key]}`).toBeCloseTo(
                OVERVIEW_TARGETS[key],
                1
            )
        }
    ...
    test('opacities increase after entering focus mode (search click)', async ({ page }) => {
        ...
        expect(['focus', 'trail'], 'navMode should indicate an active focus/trail state after result click').toContain(
            focused.navMode
        )
        ...
        for (const [key, opacity] of focusedAvailable) {
            ...
            expect(opacity, `${key} opacity should reach the focus range`).toBeGreaterThanOrEqual(
                FOCUS_TARGETS[key] * 0.75
            )
        }
    })
