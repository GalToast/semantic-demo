...
assert(
    typeof lifecycle.resetExplorationFocus === 'function',
    'resetExplorationFocus must be exported from lifecycle.ts'
)
assert(typeof lifecycle.resetExperienceState === 'function', 'resetExperienceState must be exported from lifecycle.ts')
assert(
    typeof lifecycle.resetStateBeforeUrlRestore === 'function',
    'resetStateBeforeUrlRestore must be exported from lifecycle.ts'
)
...
const focusOnNodeSource = readFileSync(
    join(PROJECT_ROOT, 'src', 'lib', 'engine', 'camera-choreography', 'cursor.ts'),
    'utf8'
)
const focusStoreSource = readFileSync(join(PROJECT_ROOT, 'src', 'lib', 'stores', 'focus.svelte.ts'), 'utf8')
assert(
...
const CANONICAL_WRITERS = {
    ...
    'navState.focusedIndex': new Set(['navigation-state.ts', 'lifecycle.ts', 'search-state.ts', 'micro-demo.ts']),
    ...
    'navState.trailNeighborIndices': new Set(['navigation-state.ts', 'lifecycle.ts', 'journey.ts', 'micro-demo.ts']),
    'navState.trailCursor': new Set(['navigation-state.ts', 'lifecycle.ts', 'journey.ts', 'micro-demo.ts']),
    ...
    'navState.trailSeedIndex': new Set(['navigation-state.ts', 'lifecycle.ts', 'journey.ts', 'micro-demo.ts']),
    'navState.threadCandidates': new Set(['navigation-state.ts', 'journey.ts']),
    'navState.threadReasonByIndex': new Set(['navigation-state.ts', 'journey.ts']),
    'navState.threadSource': new Set(['navigation-state.ts', 'journey.ts']),
    'navState.walkHistoryIndices': new Set(['navigation-state.ts', 'lifecycle.ts', 'micro-demo.ts']),
...
const SCAN_FIELDS = [
    ...
    'navState.explorationHistoryIndices',
    'navState.trailNeighborIndices',
    'navState.trailCursor',
    'navState.walkHistoryIndices',
    'navState.trailSeedIndex',
    'navState.threadCandidates',
    'navState.threadReasonByIndex',
...
for (const field of focusPocketFields) {
    ...
    try {
        const writers = scanWriters(modPath, field)
        const unexpected = writers.filter((w) => !canonicalSet.has(mod))
        fpViolations = fpViolations.concat(
            unexpected.map((w) => `lifecycle.js:${w.lineno} — '${field}': ${w.text} (type:${w.type})`)
        )
    } catch (e) {
        if (e.code === 'ENOENT') {
