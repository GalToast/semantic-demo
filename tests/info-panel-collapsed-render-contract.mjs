...
async function run() {
  ...
  try {
    ...
    check('bottom-anchored', info.bottomAnchored, `bottom=${info.bottom.toFixed(1)}px vs viewport=${info.viewportHeight}px`);
    check('mobile-sheet-geometry', info.edgeAnchored && info.bottomAnchored, 'panel uses mobile bottom-sheet geometry');
