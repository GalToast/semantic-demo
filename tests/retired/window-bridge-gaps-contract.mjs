...
const LIFECYCLE_PATH = resolveSource('', SEMDEMO_ROOT)
const VIEW_CONTROLLER_PATH = resolveSource('', SEMDEMO_ROOT)
...
const JOURNEY_SELECTED_CARD_PATH = resolveSource('', SEMDEMO_ROOT)
const UI_RENDERERS_PATH = resolveSource('src/lib/ui-renderers.ts', SEMDEMO_ROOT)
const FOCUS_STAGE_RENDERER_PATH = resolveSource('', SEMDEMO_ROOT)
const SCENE_REVEAL_PATH = resolveSource('src/lib/engine/scene-reveal.ts', SEMDEMO_ROOT)
const EVENT_BINDINGS_PATH = resolveSource('', SEMDEMO_ROOT)
...
function testGap1_getRouteLayerOrigin() {
    ...
    const hasLegacyGuard =
        /typeof\s+window\.getRouteLayerOrigin\s*===\s*['"]function['"]\s*\?\s*window\.getRouteLayerOrigin\(\)\s*:\s*['"]galaxy['"]/.test(
            src
        )
...
function testGap2_syncClusterSectionState() {
    ...
    assert(
        /import\s*\{\s*syncClusterSectionState\s*\}\s*from\s*['"](?:\.\/cluster-labels(\.ts)?|@lib\/ui\/cluster-labels)['"]\s?;?/.test(
            eventBindingsSrc
        ),
...
function testGap3b_applySearchGlowVisualState() {
    ...
    assert(
        /import\s+\{\s*publish,\s*EVENTS\s*\}\s+from\s+['"](?:\.\/event-bus\.(?:js|ts)|@lib\/orchestration\/event-bus)['"]/.test(
            pointColorSrc
        ),
        'journey-point-color.js must import publish and EVENTS from event-bus.ts'
    )
    assert(
        !/search-lifecycle-adapter/.test(pointColorSrc),
        'journey-point-color.js must not import retired search lifecycle adapter'
    )
    ...
    assert(
        !bareApplySearchGlowVisualState,
        'journey-point-color.js must not have bare window.applySearchGlowVisualState calls'
    )
...
function testGap4_updateSelectedCardHeading() {
    ...
    assert(
        /import\s*\{[\s\S]*updateSelectedCardHeading[\s\S]*\}\s*from\s*['"]\.\/focus-stage-renderer\.(?:js|ts)['"]/.test(
            selectedCardSrc
        ),
    ...
    assert(
        !/import\s*\{[\s\S]*updateSelectedCardHeading[\s\S]*\}\s*from\s*['"]\.\/ui-renderers\.(?:js|ts)['"]/.test(
            lifecycleSrc
        ),
...
function testGap5_focusOnNode() {
    ...
    assert(
        /publish\s*\(\s*EVENTS\.SEARCH_FOCUS_REQUESTED\s*,\s*\{[^}]*\bpoint\b[^}]*\bindex\b/.test(
            searchOrchestrationSrc
        ),
    ...
    assert(
        /focusOnNode\s*:\s*\(index:\s*number,\s*options\?:\s*Record<string,\s*unknown>\)\s*=>[\s\S]{0,180}focusOnNodeAction\(index,\s*options\)/.test(
            appSrc
        ),
