...
async function clusterStateProbe(page) {
    return page.evaluate(() => {
        ...
        return {
            ...
            activeClusterItems: Array.from(document.querySelectorAll('.cluster-item.active')).map((el) =>
                Number(el.dataset.cluster)
            )
        }
    })
}
...
test.describe('activeClusterFilter URL Restoration Race', () => {
    ...
    test('back/forward restores cluster= from URL, not stale state', async ({ page }) => {
        ...
        await page.waitForFunction(() => document.body.dataset.graphicsMode === 'webgl', { timeout: 20000 })
        await page.waitForFunction(() => window.__TEST_STATE__?.activeClusterFilter === 5, { timeout: 15000 })
        await page
    ...
    test('URL with cluster= overwrites an already-active in-memory cluster filter', async ({ page }) => {
        ...
        await page.waitForFunction(() => document.body.dataset.graphicsMode === 'webgl', { timeout: 20000 })
        await page.waitForFunction(() => window.__TEST_STATE__?.activeClusterFilter === 2, { timeout: 15000 })
        await page
            .waitForFunction(() => new Promise((r) => requestAnimationFrame(() => r(true))), { timeout: 5000 })
            .catch(() => {})
