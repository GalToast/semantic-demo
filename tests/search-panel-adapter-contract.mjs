...
const SEARCH_OWNER_FILES = ['state.ts', 'orchestration.ts', 'results-ui.ts']
...
assert(
    adapterSrc.includes("classList.toggle('has-expanded-results', resultsExpanded)"),
    'Adapter must own has-expanded-results container class'
)
console.log('  PASS')
...
for (const fileName of SEARCH_OWNER_FILES) {
    const src = readFileSync(path.join(SEARCH_SRC, fileName))
    assert(
        !src.includes('document.body.dataset.searchGlow'),
        `${fileName} must not set searchGlow directly; delegate through setSearchGlowState`
    )
}
console.log('  PASS')
...
assert(
    /import\s*\{[^}]*setSearchContainerState[^}]*setSearchGlowState[^}]*\}\s*from\s*['"][^'"]*(?:\.\/)?search-panel-adapter['"]/.test(
        resultsUiSrc
    ),
    'results-ui.ts must import setSearchContainerState and setSearchGlowState from search-panel-adapter'
)
assert(
    resultsUiSrc.includes('setSearchContainerState({ resultsExpanded: isExpanded })'),
    'results-ui.ts must delegate expanded-results container state'
)
