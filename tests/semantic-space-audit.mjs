...
const indexManifest = layoutManifest.index_dir && fs.existsSync(indexManifestPath)
  ? JSON.parse(fs.readFileSync(indexManifestPath, 'utf8'))
  : null;
const indexMetadata = layoutManifest.index_dir && fs.existsSync(indexMetadataPath)
...
const indexOrderMismatches = indexMetadata
  ? dataRows.reduce((count, row, index) => count + (String(row[7]) !== String(indexMetadata[index]?.lead_id) ? 1 : 0), 0)
  : dataRows.length;
const scriptEmbeddingHash = fs.existsSync(SCRIPT_EMBEDDINGS_PATH) ? hashFile(SCRIPT_EMBEDDINGS_PATH) : null;
const indexEmbeddingHash = layoutManifest.index_dir && fs.existsSync(indexEmbeddingsPath) ? hashFile(indexEmbeddingsPath) : null;
...
assert(summary.positionsOutsideUnitCube === 0, `data.dat coordinates must stay inside [0,1]^3; rows outside unit cube: ${summary.positionsOutsideUnitCube}`);
if (gzipDataRows) {
  assert(gzipDataRows.length === dataRows.length, `data.dat.gz rows must match data.dat rows: ${gzipDataRows.length} != ${dataRows.length}`);
  assert(gzipCoordinateMismatches === 0, `data.dat.gz coordinates must match data.dat coordinates; mismatched rows: ${gzipCoordinateMismatches}`);
}
...
assert(indexMetadata.length === dataRows.length, `index metadata rows must match data.dat: ${indexMetadata.length} != ${dataRows.length}`);
assert(Number(indexManifest.count) === dataRows.length, `index manifest count must match data.dat: ${indexManifest.count} != ${dataRows.length}`);
assert(Number(indexManifest.dimensions) === 1024, `index dimensions must stay at 1024, got ${indexManifest.dimensions}`);
assert(indexOrderMismatches === 0, `data.dat lead order must match index metadata order; mismatches: ${indexOrderMismatches}`);
assert(
...
assert(Number(threadIndexManifest.count) === dataRows.length, `thread source index count must match data.dat: ${threadIndexManifest.count} != ${dataRows.length}`);
...
assert(summary.neighborhoodPreservation.recallAt48 >= 0.11, `semantic recall@48 too low: ${summary.neighborhoodPreservation.recallAt48}`);
assert(summary.neighborhoodPreservation.anyAt24 >= 0.45, `semantic any@24 too low: ${summary.neighborhoodPreservation.anyAt24}`);
