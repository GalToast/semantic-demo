...
async function snapshotState(page) {
    return page.evaluate(() => {
        ...
        return {
            ...
            // Derived from state
            hasFocus:
                (s.focusedNode !== null && s.focusedNode !== undefined) ||
...
function assertInvariants(snap) {
    ...
    if (snap.semanticDiveMode !== expectedDiveMode) {
        errors.push(
            `semanticDiveMode=${snap.semanticDiveMode} but trailDepth=${snap.trailDepth} — expected ${expectedDiveMode}`
        )
    }
    ...
    if (snap.semanticDive !== expectedDiveAttr) {
        errors.push(
            `dataset.semanticDive=${snap.semanticDive} but diveUiVisible=${diveUiVisible} — expected ${expectedDiveAttr}`
        )
    }
...
test.describe('3D semantic state transition integrity', () => {
    ...
    test('semantic dive transition: Step Inside keeps state consistent', async ({ page }) => {
        ...
        await page
            .waitForFunction(() => (window.__APP_STATE__ ?? window.__TEST_STATE__)?.semanticDiveMode === true, {
                timeout: 10000
            })
            .catch(() => {})
    ...
    test('map trail transition: switching to map view keeps state consistent', async ({ page }) => {
        ...
        expect(preMapSnap.panelSurface, 'panelSurface should be semantic-dive before switching to map').toBe(
            'semantic-dive'
        )
        ...
        await page
            .waitForFunction(() => document.body?.dataset?.activeView === 'map', { timeout: 10000 })
            .catch(() => {})
    ...
    test('escape-from-dive: Escape from trailDepth=2 returns to overview with all invariants intact', async ({
        page
    }) => {
        test.setTimeout(180000)
        ...
        expect(snap.panelSurface, 'panelSurface must not be semantic-dive after Escape from dive').not.toBe(
            'semantic-dive'
        )
    ...
    test('dive without focus node is silently ignored', async ({ page }) => {
        ...
        const snap = await snapshotState(page)
        // window.__navActions__?.setSemanticDiveMode(true) when canDive=false is a no-op (canDive requires currentView===galaxy AND hasFocus)
