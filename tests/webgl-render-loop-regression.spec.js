...
async function waitForSceneReady(page) {
    ...
    await page.waitForFunction(() => document.querySelector('#canvas-container')?.classList.contains('canvas-ready'), {
        timeout: 20_000
    })
...
async function canvasPixelVariance(page) {
    ...
    if ((await canvas.count()) === 0) return { hasPixels: false, reason: 'no canvas element' }
    ...
    return { hasPixels: litPixels > 0, nonZeroCount: litPixels, totalSamples: png.width * png.height }
}
...
test.describe('WebGL Render Loop Regression', () => {
    test('canvas is visible, canvas-ready class present, and render loop draws geometry', async ({ page }) => {
        ...
        expect(
            pixelCheck.hasPixels,
            `canvas must have non-blank pixels (reason: ${pixelCheck.reason || `${pixelCheck.nonZeroCount}/${pixelCheck.totalSamples}`})`
        ).toBe(true)
    ...
    test('canvas-ready class appears before loading overlay disappears', async ({ page }) => {
        ...
        expect(
            overlayHiddenMs - canvasReadyMs,
            'overlay should hide soon after canvas-ready (max 6s)'
