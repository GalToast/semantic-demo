    ...
    const result = await page.evaluate(
        async ({ stateName }) => {
            ...
            if (!expectsRouteControls) {
                if (visible(routeControls) && !allowsIdleRouteControls)
                    failures.push({
                        check: 'idle-route-controls-hidden',
                        selector: '[aria-label="Journey route controls"]'
                    })
                else passes.push({ check: 'idle-route-controls-hidden' })
            } else if (!visible(primary))
                failures.push({ check: 'primary-action-visible', selector: '#btn-journey-primary' })
            else {
                ...
                if (isMobile && !touchTarget(primary))
                    failures.push({ check: 'primary-action-touch-target', rect: primary.getBoundingClientRect() })
                else passes.push({ check: 'primary-action-touch-target' })
                if (clipped(primary))
                    failures.push({
                        check: 'primary-action-text-clipping',
                ...
                if (
                    ...
                    failures.push({
                        check: 'primary-action-compact-label',
                        detail: 'font-size is zero without a pseudo-label'
                    })
                } else {
            ...
            if (isMobile && visible(sheet)) {
                ...
                if (Math.abs(window.innerHeight - rect.bottom) > 1 || rect.top < -1)
                    failures.push({ check: 'sheet-anchoring', rect })
                else passes.push({ check: 'sheet-anchoring' })
                if (handleWidth < 24 || handleHeight < 3 || before.content === 'none')
                    failures.push({
                        check: 'sheet-handle-affordance',
                        width: handleWidth,
                        height: handleHeight,
            ...
            for (const toggle of toggles) {
                if (!toggle.getAttribute('aria-controls')) {
                    failures.push({
                        check: 'expanded-control-target',
                        selector: toggle.id || toggle.className || toggle.tagName
                    })
                }
            }
...
try {
    ...
    await reducedPage.goto(withParams(targetUrl, { view: 'galaxy', q: 'coffee', anchor: '519' }), {
    ...
    await fs.promises.writeFile(
        path.join(outDir, 'mobile-reduced-motion.json'),
