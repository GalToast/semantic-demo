...
svelteTests.push(async function testSveltePhaseOrder() {
    if (!svelteLoadingSource) return skip('LoadingOverlay.svelte not readable');
    const hasOrder = svelteLoadingSource.includes("'records'")
...
svelteTests.push(async function testSveltePhaseProgressMapping() {
    ...
    const hasProgressMap = svelteLoadingSource.includes('progress:')
...
svelteTests.push(async function testSvelteLoadingStateAttribute() {
    ...
    if (!hasStateAttr) throw new Error('LoadingOverlay must set data-loading-state="active"');
...
svelteTests.push(async function testSvelteAppRendersLoadingOverlay() {
    ...
    const hasImport = svelteAppSource.includes("LoadingOverlay")
        && svelteAppSource.includes("@components/LoadingOverlay.svelte");
...
legacyTests.push(async function testSceneReadyDispatch() {
    ...
    if (hasStringLiteral) throw new Error('hideLoadingOverlay must not dispatch CustomEvent with raw string literal');
