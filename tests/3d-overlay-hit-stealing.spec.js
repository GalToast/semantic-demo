...
async function findReachableNodeCoordinate(page) {
    ...
    for (const pass of passes) {
        ...
        for (const candidate of candidates) {
            ...
            if (
                stack.some((item) => item.isCanvas) &&
                !stack.some((item) => item.isInteractiveOverlay && item.pointerEvents !== 'none')
            ) {
...
test.describe('3D overlay hit ownership', () => {
    test('desktop: reachable node coordinate includes canvas and excludes interactive overlays', async ({ page }) => {
        ...
        expect(
            target.stack.some((item) => item.isCanvas),
            `canvas should be present in hit stack: ${JSON.stringify(target.stack)}`
        ).toBe(true)
        expect(
            target.stack.some((item) => item.isInteractiveOverlay && item.pointerEvents !== 'none'),
            `interactive overlay should not cover reachable node: ${JSON.stringify(target.stack)}`
        ).toBe(false)
    })
    ...
    test('desktop: overlay clicks do not create canvas pick evidence', async ({ page }) => {
        ...
        expect(
            ...
            'overlay clicks must leave focusedNode null or valid'
        ).toBe(true)
    ...
    test('mobile portrait: overlay click does not leak into canvas focus', async ({ page }) => {
        ...
        expect(
            isValidNodeIndex(after.focusedNode, after.pointCount),
            'mobile overlay click must leave focusedNode null or valid'
        ).toBe(true)
    ...
    test('short landscape: reachable node still avoids overlay hit-stealing', async ({ page }) => {
        ...
        expect(
            target.stack.some((item) => item.isCanvas),
            `canvas should remain in short-landscape hit stack: ${JSON.stringify(target?.stack)}`
        ).toBe(true)
    })
    ...
    test('short landscape: focus pocket nodes are not consumed by overlay hit-stealing', async ({ page }) => {
        ...
        expect(
            ...
            `short-landscape pocket must retain reachable nodes, got ${pocket.reachableCount}`
        ).toBeGreaterThan(0)
    })

    test('mobile-portrait: focus neighborhood nodes are not consumed by overlay hit-stealing at 390x844', async ({
        page
    }) => {
        test.setTimeout(90000)
        await openApp(page, { width: 390, height: 844 })
        ...
        expect(
            pocket.reachableCount,
            `mobile-portrait pocket must retain reachable nodes, got ${pocket.reachableCount}`
        ).toBeGreaterThan(0)
    })

    test('mobile-portrait: clicking near a focus-neighborhood node does not create spurious canvas focus pick at 390x844', async ({
        page
    }) => {
        test.setTimeout(90000)
        await openApp(page, { width: 390, height: 844 })
        ...
        for (const point of neighborhoodScreenPoints.slice(0, 3)) {
            ...
            const onOverlay = centers.some((c) => Math.abs(c.x - clickX) < 30 && Math.abs(c.y - clickY) < 30)
        ...
        const after = await probe(page)
        expect(
            after.lastCanvasNodeFocusPick,
            'clicking near neighborhood nodes should not create spurious canvas focus pick'
        ).toBeNull()
        expect(
