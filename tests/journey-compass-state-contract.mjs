...
const { getFocusedJourneyPoint, getJourneyCompassState } = await import('../src/lib/journey/compass-state.ts')
...
assert.equal(
    compassState.primaryAction?.action,
    'enter-inside',
    'search anchor focus exposes enter-inside as primary action'
)
assert.equal(
    compassState.secondaryAction?.action,
    'open-map',
    'search anchor focus exposes open-map as secondary action'
)
