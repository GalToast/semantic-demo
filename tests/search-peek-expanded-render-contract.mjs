    ...
    return new Promise((resolvePromise) => {
        const server = createServer((req, res) => {
            ...
            try {
                ...
                const ct =
                    extname(fp) === '.css' ? 'text/css' : extname(fp) === '.ts' ? 'application/javascript' : 'text/html'
    ...
    await page
        .waitForFunction(
            () => {
                ...
                return (
                    Boolean(document.querySelector('.search-container') && document.querySelector('#search-results')) ||
...
async function assertPeekMode(page, ctx) {
    ...
    if (info.resultsPresent) ctx.pass('peek', 'dom:search-results')
        ...
        ctx.fail('peek', 'dom:search-results', 'missing #search-results')
        return info
    }
    ...
    if (info.panelSurface === 'search' && info.panelSurfaceDetail === 'peek') {
        ...
        ctx.fail(
            'peek',
            'state:panel-surface-detail',
            `expected search/peek, got ${info.panelSurface || 'missing'}/${info.panelSurfaceDetail || 'missing'}`
        )
    }
    ...
    if (info.countDisplay === 'none') ctx.pass('peek', 'visibility:count-hidden-in-peek')
        ...
        ctx.fail(
            'peek',
            'visibility:count-hidden-in-peek',
            `.search-results-count display="${info.countDisplay}" should be "none"`
        )
    ...
    if (info.whatDisplay === 'none') ctx.pass('peek', 'visibility:result-what-hidden-in-peek')
        ...
        ctx.fail(
            'peek',
            'visibility:result-what-hidden-in-peek',
            `.search-result-what display="${info.whatDisplay}" should be "none"`
        )
    ...
    if (info.metaDisplay === 'none') ctx.pass('peek', 'visibility:result-meta-hidden-in-peek')
        ...
        ctx.fail(
            'peek',
            'visibility:result-meta-hidden-in-peek',
            `.search-result-meta display="${info.metaDisplay}" should be "none"`
        )
    ...
    if (info.contextDisplay === 'none') ctx.pass('peek', 'visibility:result-context-hidden-in-peek')
        ...
        ctx.fail(
            'peek',
            'visibility:result-context-hidden-in-peek',
            `.search-result-context display="${info.contextDisplay}" should be "none"`
        )
    ...
    if (info.barDisplay === 'none') ctx.pass('peek', 'visibility:result-bar-hidden-in-peek')
        ...
        ctx.fail(
            'peek',
            'visibility:result-bar-hidden-in-peek',
            `.search-result-bar display="${info.barDisplay}" should be "none"`
        )

    const allNonFirstHidden = info.nonFirstDisplays.every((display) => display === 'none')
    if (allNonFirstHidden)
        ctx.pass(
            'peek',
            'visibility:non-first-results-hidden-in-peek',
            'non-first items are hidden in collapsed peek mode'
        )
    else {
        const details = info.nonFirstDisplays
            .map((display, i) => `${display} (overflow: ${info.nonFirstOverflow[i]})`)
            .join(', ')
        ctx.fail(
            'peek',
            'visibility:non-first-results-hidden-in-peek',
            `non-first item displays: [${details}], body classes: [${info.bodyClasses.join(', ')}]`
        )
    }
...
async function assertExpandedMode(page, ctx) {
    ...
    if (info.resultsPresent) ctx.pass('expanded', 'dom:search-results')
        ...
        ctx.fail('expanded', 'dom:search-results', 'missing #search-results')
        return info
    }
    ...
    if (info.countDisplay !== 'none') ctx.pass('expanded', 'visibility:count-visible')
        ...
        ctx.fail(
            ...
            `.search-results-count display="${info.countDisplay}" should not be "none"`
        )
    ...
    if (info.whatDisplay !== 'none') ctx.pass('expanded', 'visibility:result-what-visible')
        ...
        ctx.fail(
            'expanded',
            'visibility:result-what-visible',
            `.search-result-what display="${info.whatDisplay}" should not be "none"`
        )
    ...
    if (info.metaDisplay !== 'none') ctx.pass('expanded', 'visibility:result-meta-visible')
        ...
        ctx.fail(
            'expanded',
            'visibility:result-meta-visible',
            `.search-result-meta display="${info.metaDisplay}" should not be "none"`
        )
    ...
    if (info.contextDisplay !== 'none') ctx.pass('expanded', 'visibility:result-context-visible')
        ...
        ctx.fail(
            'expanded',
            'visibility:result-context-visible',
            `.search-result-context display="${info.contextDisplay}" should not be "none"`
        )
    ...
    if (info.barDisplay !== 'none') ctx.pass('expanded', 'visibility:result-bar-visible')
        ...
        ctx.fail(
            'expanded',
            'visibility:result-bar-visible',
            `.search-result-bar display="${info.barDisplay}" should not be "none"`
        )

    if (info.visibleItemsCount >= 2)
        ctx.pass('expanded', 'visibility:multiple-results-visible', `${info.visibleItemsCount} results visible`)
    else
        ctx.fail(
            'expanded',
            'visibility:multiple-results-visible',
            `only ${info.visibleItemsCount} result(s) visible — expected ≥2`
        )
    ...
    await page.evaluate(() => {
        ...
        <div class="search-error-state" role="alert">
          ...
          <div class="search-error-actions">
            ...
            if (container)
                container.classList.add('has-query', 'results-rendered', 'has-expanded-results', 'search-degraded')
        }
    })
    await page
    ...
    const info = await page.evaluate(() => {
        ...
        return {
            ...
            errorStateVisible: errorState
                ? getComputedStyle(errorState).display !== 'none' &&
                  getComputedStyle(errorState).visibility !== 'hidden'
                : null,
...
async function main() {
    ...
    try {
        ...
        const itemCount = await page.evaluate(() => document.querySelectorAll('.search-result-item').length)
