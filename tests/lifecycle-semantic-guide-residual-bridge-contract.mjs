...
function testLifecycleDoesNotImportUpdateLegendGuideStateFromSemanticGuide() {
  ...
  assert(!badImport, 'lifecycle must NOT import updateLegendGuideState from any source (legend-panel.svelte.ts owns it)');
...
function testCloseLegendGuideOwnership() {
  ...
  assert(
    ...
    'legend-panel.svelte.ts must export closeLegendGuide'
  );
  ...
  assert(
    ...
    'closeLegendGuide delegates to closeLegendPanel'
  );
...
function testDeletedKernelsNotExist() {
  ...
  assert(
    !fs.existsSync(DELETED_LEGEND_UI),
    'js/modules/legend-ui.ts must be deleted'
  );
  assert(
    !fs.existsSync(DELETED_SEMANTIC_GUIDE),
    'js/modules/semantic-guide.ts must be deleted'
  );
