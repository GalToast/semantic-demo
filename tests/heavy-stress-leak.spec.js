...
test.describe('Heavy stress & GPU leak auditor', () => {
  ...
  test('Runs repeated search, view navigation, and WebGL context restoration cycles with 0 errors', async ({ page }) => {
    ...
    for (let i = 0; i < 5; i += 1) {
      ...
      await page.evaluate(() => {
        const s = window.__APP_STATE__ ?? window.__TEST_STATE__;
        if (s) {
          s.withMutation(() => { s.currentView = 'map'; });
        }
      });
      ...
      await page.evaluate(() => {
        const s = window.__APP_STATE__ ?? window.__TEST_STATE__;
        if (s) {
          s.withMutation(() => { s.currentView = 'galaxy'; });
        }
      });
      await page.waitForTimeout(200);
    }
    ...
    for (let i = 0; i < 2; i += 1) {
      const lostTriggered = await page.evaluate(() => {
        return typeof window.__THREE_APP__?.simulateWebGLContextLoss === 'function' &&
