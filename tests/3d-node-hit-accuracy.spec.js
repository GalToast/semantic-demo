...
test.describe('3D canvas node hit accuracy', () => {
    test('desktop: clicking a real hoverable canvas node focuses a valid node', async ({ page }) => {
        ...
        expect(
            ...
            'focusedNode must be valid after real canvas node click'
        ).toBe(true)
        expect(after.navMode, 'canvas node click should enter focus mode').toBe('focus')
        expect(
            after.lastCanvasNodePick || after.lastCanvasNodeFocusPick,
            'click should record canvas pick evidence'
        ).not.toBeNull()
    })
    ...
    test('desktop: click pick evidence stays near the pointer coordinate', async ({ page }) => {
        ...
        expect(
            pickDist,
            `pick should stay near clicked coordinate, distance=${Math.round(pickDist)}px`
    ...
    test('mobile portrait: tapping a real hoverable canvas node focuses a valid node', async ({ page }) => {
        ...
        const { after } = await clickResolvedNode(page)
        expect(isValidNodeIndex(after.focusedNode, after.pointCount), 'mobile canvas tap must focus a valid node').toBe(
            true
        )
    ...
    test('short landscape: tapping a real hoverable canvas node focuses a valid node', async ({ page }) => {
        ...
        expect(
            isValidNodeIndex(after.focusedNode, after.pointCount),
            'short-landscape canvas tap must focus a valid node'
        ).toBe(true)
    })
    ...
    test('desktop: edge-region click yields a valid pick within tolerance', async ({ page }) => {
        ...
        const after = await probe(page)
        expect(isValidNodeIndex(after.focusedNode, after.pointCount), 'edge-region click must focus a valid node').toBe(
            true
        )
        ...
        expect(
            pickDist,
            `edge-region pick should stay near click coordinate, distance=${Math.round(pickDist)}px`
        ).toBeLessThanOrEqual(64)
    })
    ...
    test('mobile portrait: edge-region tap yields valid pick within tolerance', async ({ page }) => {
        ...
        const after = await probe(page)
        expect(isValidNodeIndex(after.focusedNode, after.pointCount), 'mobile edge tap must focus a valid node').toBe(
            true
        )
    ...
    test('short landscape: edge-region click yields valid pick within tolerance', async ({ page }) => {
        ...
        expect(
            isValidNodeIndex(after.focusedNode, after.pointCount),
            'short-landscape edge click must focus a valid node'
        ).toBe(true)
