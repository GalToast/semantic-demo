...
async function main() {
    ...
    try {
        ...
        // Step 1: App.svelte must mount into #app
        console.log('Waiting for App.svelte to mount into #app...')
        ...
        // Step 3: Force panelSurface to expose all sections/controls
        console.log('Setting panelSurface to map-focus-search to expose all controls...')
        await page.evaluate(() => {
        ...
        // Verify elements exist in DOM after panelSurface change
        const elsExist = await page.evaluate(() => ({
        ...
        assert(elsExist.focusStage, 'focus-stage exists in DOM')
        assert(elsExist.cameraControls, 'camera-controls exists in DOM')

        // Step 4: Verify mounted component structures
        console.log('Verifying Svelte component structures...')

        // Step 5: Verify store-to-DOM reactivity (Filters)
        console.log('Verifying store reactivity in Filter component...')
        ...
        assert(chipBefore === 'true', 'Filters: "All" chip starts with aria-pressed=true')
        ...
        assert(chipAfter.allPressed === 'false', 'Filters: "All" chip aria-pressed=false after click')
        assert(chipAfter.activePressed === 'true', 'Filters: "Active" chip aria-pressed=true after click')
        ...
        const svelteErrors = consoleMessages.filter(
            (m) =>
                m.type === 'error' &&
