...
function setFocusViaSelectedPoint(index) {
  state.selectedPoint = state.points?.[index] ?? { lead_id: `fake-${index}`, name: `Biz ${index}`, cluster: 1 };
...
function setSearchIntent(query = 'coffee') {
  state.currentSearchSummary = { query, visibleMatches: 5 };
...
resetState();
setFocusViaSelectedPoint(4);
state.trailDepth = 1; // < 2 means semanticDiveMode=false
...
assert(ds('panelSurface') !== 'search' && ds('panelSurface') !== 'focus',
...
for (const s of GALAXY_STATES) {
  ...
  assert(ps !== 'undefined' && ps !== 'null' && ps !== '',
    `5A[${s.label}]: panelSurface is not a nullish string`);
...
const CANONICAL_STATES = [
  ...
  { label: 'focus-search',     currentView: 'galaxy', setup: () => { setFocusViaSelectedPoint(2); setSearchIntent(); } },
  { label: 'semantic-dive',   currentView: 'galaxy', setup: () => { setFocusViaSelectedPoint(2); state.trailDepth = 2; } },
  { label: 'map-focus-search',currentView: 'map',   setup: () => { setFocusViaSelectedPoint(2); setSearchIntent(); } },
