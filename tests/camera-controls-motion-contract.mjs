...
assert(
  document.body.dataset.focusTransition === 'idle',
  'setFocusTransitionMode: only-invalid-chars collapse to idle'
);
...
assert(document.body.dataset.cameraAssist === 'arriving', 'startFocusCameraAssist: body.dataset.cameraAssist is arriving');
assert(document.body.dataset.cameraAssistReason === 'focus', 'startFocusCameraAssist: body.dataset.cameraAssistReason set');

// focusCameraAssistIsActive: true while within window
globalThis._now = 5500;
assert(
  focusCameraAssistIsActive(5500) === true,
  'focusCameraAssistIsActive: true when now < until'
);

// focusCameraAssistIsActive: false once expiry passes — triggers release
globalThis._now = 6000;
assert(
  focusCameraAssistIsActive(6000) === false,
  'focusCameraAssistIsActive: false after expiry'
);
assert(
  state.focusCameraAssistActive === false,
  'focusCameraAssistIsActive: expiry auto-releases active flag'
);
assert(
