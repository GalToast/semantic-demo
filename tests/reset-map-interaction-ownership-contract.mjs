...
function assertOverview(label, snap) {
  ...
  assert(snap.bodyDataset.activeView === 'galaxy', `${label}: activeView should be galaxy, got ${snap.bodyDataset.activeView}`);
  ...
  if (idleInfoPanel) {
    assert(idleInfoPanel.bottom >= snap.viewport.height - 2,
...
async function searchAndFocusFirstResult(page) {
  ...
  assert(focused.independentDrawers.length === 1 && focused.independentDrawers[0].name === 'focusStage',
...
async function enterSemanticDive(page) {
  ...
  await page.waitForFunction(() => {
    const state = window.__APP_STATE__ || window.__TEST_STATE__ || {};
    return state.semanticDiveMode === true &&
      state.trailDepth === 2 &&
...
async function runMapTransitionScenario(browser) {
  ...
  assert(map.bodyDataset.panelSurface === 'map-focus-search',
    `map transition: panelSurface should be map-focus-search, got ${map.bodyDataset.panelSurface}`);
  assert(map.bodyDataset.mapContext === 'focus-search',
    `map transition: mapContext should be focus-search, got ${map.bodyDataset.mapContext}`);
  assert(map.bodyDataset.semanticDive === 'inactive', `map transition: semanticDive dataset should be inactive, got ${map.bodyDataset.semanticDive}`);
  assert(map.semanticDiveMode === true && map.trailDepth === 2,
  ...
  assert(map.boxes.mapContainer?.visible, `map transition: map container should be visible, got ${JSON.stringify(map.boxes.mapContainer)}`);
  assert(!map.independentDrawers.some((drawer) => drawer.name === 'focusStage'),
    `map transition: focusStage should not remain a primary drawer, got ${JSON.stringify(map.independentDrawers)}`);
  assert(map.independentDrawers.length <= 1,
    `map transition: map-focus-search should expose at most one primary drawer, got ${JSON.stringify(map.independentDrawers)}`);
  assert(!map.independentDrawers.length || map.independentDrawers[0]?.name === 'infoPanel',
    `map transition: only infoPanel may become drawer-sized in map-focus-search, got ${JSON.stringify(map.independentDrawers)}`);
  assert(!map.independentDrawers.some((drawer) => drawer.name === 'searchResults'),
  ...
  if (map.boxes.searchResults?.visible) {
    assert(map.boxes.searchResults.bottom <= map.viewport.height,
      `map transition: visible search results should not overflow viewport, got ${JSON.stringify(map.boxes.searchResults)}`);
  }
  assert(!map.boxes.searchContainer?.visible,
    `map transition: search chrome should not occlude the selected map drawer, got ${JSON.stringify(map.boxes.searchContainer)}`);
  assert(map.boxes.infoPanel?.visible, `map transition: infoPanel should be visible, got ${JSON.stringify(map.boxes.infoPanel)}`);
  assert(map.boxes.infoPanel.height <= Math.min(340, Math.round(map.viewport.height * 0.4)),
  ...
  assert(map.boxes.infoPanel.bottom <= map.viewport.height + 1,
