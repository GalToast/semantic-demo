...
const surfaceMode = (mode) =>
    `(?:body\\.surface-${mode}|\\[data-panel-surface=['"]${mode}['"]\\])`
...
function run() {
  ...
  assert(
    new RegExp(
      `${surfaceDetail('none')}[\\s\\S]*${searchActive}`
  ...
  assert(
    new RegExp(
      `${surfaceMode('search')}[^,{]*${surfaceDetail('peek')}[\\s\\S]*?${searchActive}`
    ).test(mobilePremiumSrc),
  ...
  assert(
    new RegExp(
      `${surfaceMode('focus-search')}[^,{]*${surfaceDetail('peek')}[\\s\\S]*?${infoPanelRef}`
    ).test(mobilePremiumSrc),
  ...
  assert(
    new RegExp(
      `${surfaceDetail('peek')}[\\s\\S]*?\\.search-result-name`
    ).test(mobilePremiumSrc),
