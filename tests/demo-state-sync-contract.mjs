...
if (hasSvelte) {
    ...
    test('demo.ts uses DEMO_LIFETIME_KEY of moco_mycelium_demo_v1', () => {
        assert(
            /DEMO_LIFETIME_KEY\s*=\s*['"]moco_mycelium_demo_v1['"]/.test(svelteStoreSource),
    ...
    test('demo.ts has isDemoSuppressedThisSession guard using sessionStorage', () => {
        assert(
            /export\s+function\s+isDemoSuppressedThisSession/.test(svelteStoreSource),
            'demo.ts must export isDemoSuppressedThisSession'
        )
        assert(
            /sessionStorage\.getItem\(DEMO_SESSION_KEY\)/.test(svelteStoreSource),
    ...
    test('demo.ts has hasDemoBeenSeen guard using localStorage', () => {
        assert(/export\s+function\s+hasDemoBeenSeen/.test(svelteStoreSource), 'demo.ts must export hasDemoBeenSeen')
        assert(
            /localStorage\.getItem\(DEMO_LIFETIME_KEY\)/.test(svelteStoreSource),
    ...
    test('demo.ts checks for demo=force parameter in shouldRunDemo', () => {
        ...
        assert(
            forceMatches.length >= 1,
            `expected at least one demo=force parameter check, found ${forceMatches.length}`
        )
    })
    ...
    test('demo.ts markDemoCompleted writes to localStorage', () => {
        assert(/export\s+function\s+markDemoCompleted/.test(svelteStoreSource), 'demo.ts must export markDemoCompleted')
        assert(
            /localStorage\.setItem\(\s*DEMO_LIFETIME_KEY/.test(svelteStoreSource),
    ...
    test('demo.ts markDemoSessionSkipped writes to sessionStorage', () => {
        assert(
            /export\s+function\s+markDemoSessionSkipped/.test(svelteStoreSource),
            'demo.ts must export markDemoSessionSkipped'
        )
        assert(
            /sessionStorage\.setItem\(DEMO_SESSION_KEY/.test(svelteStoreSource),
    ...
    test('DemoChoreography.svelte stores demo completion in localStorage', () => {
        assert(
            /localStorage\.setItem\(['"]moco_mycelium_demo_v1['"]/.test(svelteComponentSource) ||
    ...
    test('DemoChoreography.svelte stores session skip in sessionStorage', () => {
        assert(
            /sessionStorage\.setItem\(['"]moco_mycelium_demo_session_v1['"]/.test(svelteComponentSource) ||
    ...
    test('DemoChoreography.svelte delegates eligibility and storage key ownership to demo store helpers', () => {
        assert(
            /shouldRunDemo\(\)/.test(svelteComponentSource),
    ...
    test('demo state is decoupled from window globals', () => {
        assert(!/window\.__demoState/.test(svelteStoreSource), 'demo.ts must not expose state on window.__demoState')
        assert(
            !/window\.isMicroDemoRunning/.test(svelteStoreSource),
            'demo.ts must not expose isMicroDemoRunning on window'
        )
        assert(!/window\.cancelMicroDemo/.test(svelteStoreSource), 'demo.ts must not expose cancelMicroDemo on window')
    })
} else {
    // ── Legacy Path Checks ──────────────────────────────────────────────────
    ...
    test('micro-demo.js uses SESSION_STORAGE_KEY of moco_mycelium_demo_session_v1', () => {
        assert(
            /SESSION_STORAGE_KEY\s*=\s*['"]moco_mycelium_demo_session_v1['"]/.test(microDemoSource),
            'micro-demo.js must define SESSION_STORAGE_KEY as moco_mycelium_demo_session_v1'
        )
    })
    ...
    test('micro-demo.js checks for demo=force parameter in shouldRunMicroDemo and startMicroDemo', () => {
        ...
        assert(
            forceMatches.length >= 2,
            `expected at least two demo=force parameter checks, found ${forceMatches.length}`
        )
    })

    test('micro-demo.js uses setInfoPanelOpen(false) at T=7200ms instead of raw slide-in-left manipulation', () => {
        assert(
            !/classList\.remove\(['\"]slide-in-left['\"]\)/.test(microDemoSource),
            'micro-demo.js must not directly remove slide-in-left class'
        )
        assert(
            /setInfoPanelOpen\(\s*false\s*\)/.test(microDemoSource),
            'micro-demo.js must call setInfoPanelOpen(false) at T=7200ms'
        )
    })

    test('micro-demo.js calls setInfoPanelOpen(true) via __demoReset (called from _resetAppState)', () => {
        assert(
            /_resetAppState\b[\s\S]*?__demoReset\(\)/.test(microDemoSource),
            '_resetAppState must call __demoReset()'
        )
        assert(
            /function __demoReset\(\)[\s\S]*?setInfoPanelOpen\(\s*true\s*\)/.test(microDemoSource),
            '__demoReset must call setInfoPanelOpen(true)'
        )
        assert(
            !microDemoSource.includes('window.setInfoPanelOpen'),
            'micro-demo.js must use the named setInfoPanelOpen import, not window.setInfoPanelOpen'
        )
    })

    test('event-bindings.js defines revealSelectedBusinessCard', () => {
        assert(
            /export\s+function\s+revealSelectedBusinessCard/.test(eventBindingsSource),
            'event-bindings.js must export revealSelectedBusinessCard'
        )
        assert(
            /revealSelectedBusinessCard\(\)\s*\{[\s\S]*?setInfoPanelOpen\(\s*true\s*\)/.test(eventBindingsSource),
            'revealSelectedBusinessCard must call setInfoPanelOpen(true)'
        )
    })
}
