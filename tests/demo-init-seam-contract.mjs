...
const svelteComponentSource = hasSvelte
  ? fs.readFileSync(svelteDemoComponentPath, 'utf8')
  : null;
...
if (hasSvelte) {
  ...
  test('App.svelte renders DemoChoreography with force/suppress props', () => {
    const appSource = fs.readFileSync(path.join(ROOT, 'src/App.svelte'), 'utf8');
    assert(/<DemoChoreography\s+force=\{forceDemo\}\s+suppress=\{noDemo\}/.test(appSource)
  ...
  test('demo store defines SHOWCASE_POOL', () => {
    assert(/const\s+SHOWCASE_POOL\s*:\s*readonly\s+number\[\]/.test(svelteStoreSource)
  ...
  test('demo store defines all valid state machine phases', () => {
    const requiredPhases = ['IDLE', 'GLIDING', 'ARRIVED', 'CARD_VISIBLE', 'PULLBACK', 'WIDE_VIEW', 'RETURNING', 'COMPLETE', 'CANCELLED'];
    for (const phase of requiredPhases) {
      assert(svelteStoreSource.includes(`'${phase}'`) || svelteStoreSource.includes(`"${phase}"`),
        `demo.ts must define phase '${phase}'`);
    }
  });

  test('demo store uses Map-based timer tracking (bug fix)', () => {
    assert(/const\s+timers\s*=\s*new\s+Map/.test(svelteStoreSource),
      'demo.ts must use Map<string, number> for timer tracking');
    assert(/export\s+function\s+cancelAllDemoTimers/.test(svelteStoreSource),
      'demo.ts must export cancelAllDemoTimers');
  });

  test('demo store defines findDemoNode with validation', () => {
    assert(/export\s+function\s+findDemoNode/.test(svelteStoreSource),
      'demo.ts must export findDemoNode');
    assert(/status\s*===\s*['"]disqualified['"]/.test(svelteStoreSource),
  ...
  test('DemoChoreography.svelte cleans up timers on destroy', () => {
    assert(/onDestroy[\s\S]*?cancelAllDemoTimers/.test(svelteComponentSource)
  ...
  test('DemoChoreography.svelte renders dismiss button', () => {
    assert(/demo-dismiss/.test(svelteComponentSource)
      || /Dismiss demo/.test(svelteComponentSource),
  ...
  test('App does not contain micro-demo state machine logic', () => {
    const appSource = fs.readFileSync(path.join(ROOT, 'src/App.svelte'), 'utf8');
    assert(!/GLIDING|ARRIVED|CARD_VISIBLE|PULLBACK|WIDE_VIEW|RETURNING/.test(appSource),
