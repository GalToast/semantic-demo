...
function run() {
    ...
    assertContains(
        mobilePremiumSrc,
        /@media\s*\(max-width:\s*900px\)\s*and\s*\(max-height:\s*430px\)\s*and\s*\(orientation:\s*landscape\)[\s\S]*body:is\(\.surface-focus,\s*\.surface-focus-search,\s*\.surface-semantic-dive\)[\s\S]*:is\([\s\S]*\.share-toggle[\s\S]*\.controls[\s\S]*\.panel-toggle[\s\S]*\.time-display[\s\S]*\)\s*{[\s\S]*display:\s*none;[\s\S]*visibility:\s*hidden;[\s\S]*pointer-events:\s*none;/,
    ...
    assertContains(
        mobilePremiumSrc,
        /@media\s*\(max-width:\s*900px\)\s*and\s*\(max-height:\s*430px\)\s*and\s*\(orientation:\s*landscape\)[\s\S]*body:is\(\.surface-focus,\s*\.surface-focus-search,\s*\.surface-semantic-dive\)[\s\S]*#btn-share-view\s*{[\s\S]*display:\s*none;[\s\S]*visibility:\s*hidden;[\s\S]*pointer-events:\s*none;/,
        'mobile premium split must suppress the share button separately so #btn-share-view does not raise .controls specificity'
    )
    assertContains(
        mobilePremiumSrc,
        /@media\s*\(max-width:\s*900px\)\s*and\s*\(max-height:\s*430px\)\s*and\s*\(orientation:\s*landscape\)[\s\S]*body:is\(\.surface-focus,\s*\.surface-focus-search,\s*\.surface-semantic-dive\)[\s\S]*:is\(#btn-legend,\s*#btn-keyboard-help\)\s*{[\s\S]*display:\s*flex;[\s\S]*visibility:\s*visible;[\s\S]*pointer-events:\s*auto;/,
