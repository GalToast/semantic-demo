...
- **Splash dismissal on deep-links (PR-B2):** `parseUrlParams()` in `src/main.ts` returns `isDeepLink: true` for `?anchor=N`, `?view=map`, or `?q=...` (length >= 2). When deep-link AND desktop (`renderKind !== 'placeholder2d'`), `engineReady.signalReady()` fires immediately at boot so the user sees their target state instead of being forced through the "Click Explore" gate. Mobile 2D placeholder keeps its normal splash/CTA flow. `?story=` is intentionally NOT a deep-link — story prompts fire post-splash via DemoChoreography. If you add a new deep-link-shaped URL param, extend `parseUrlParams().isDeepLink` and document the addition here.
