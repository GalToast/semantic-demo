...
- `src/components/DemoChoreography.svelte` + `src/lib/stores/demo.svelte.ts` (10-phase) is the canonical demo. The legacy micro-demo engine (`src/lib/engine/demo-choreography.ts`, `src/lib/demo/choreography.ts`) was fully deleted in the 2026-08-14 dead-code round-2 sweep — `clearDemoTimers`/`isMicroDemoRunning` no longer exist anywhere. What survives in `src/lib/demo/` is canonical-demo support: `demo-script.ts` (10-step `DEMO_SCRIPT` + phase driver), `guards.ts` (`guardReducedMotion`, `notifyDemoUnableToStart`), `camera.ts` (`cancelOverviewCameraAnimation`). Eligibility lives in `shouldRunDemo()` (checks `?demo=force`, `?nodemo`, isDeepLink, seen/session state, placeholder surface, reduced-motion). Replay dispatches `demo-replay-requested` → DemoChoreography re-runs attemptStart after sceneReady (prevents stacked veils, M15).
...
- **Asymmetric `$derived` gate widening** (W53, `671af64c`): widening a parent gate (`use-surface-composition.svelte.ts` `focusActive` via `isFocusSurfaceActive`) MUST mirror the same predicates into child gates gating DOM (`JourneyChrome.svelte` `chromeHasFocus` via same helper). `focusActive` and `chromeHasFocus` are single-sourced through `isFocusSurfaceActive` — both are thin `$derived` wrappers in their respective `.svelte.ts` files. Asymmetric gating causes silent 30s e2e timeouts. `check:svelte`/lint won't catch it.
