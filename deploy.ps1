...
Invoke-Step @("scp", "-P", $Port, "dist/svelte/index.html", "${Target}vector-explorer-polished.html")
...
Invoke-Step @(
    "ssh", "-p", $Port, $SshTarget,
    "find '$RemoteDir' -maxdepth 1 -type d -exec chmod 755 {} \; && find '$RemoteDir/assets' '$RemoteDir/css' '$RemoteDir/js' '$RemoteDir/scripts' -type d -exec chmod 755 {} \; 2>/dev/null || true && find '$RemoteDir/assets' '$RemoteDir/css' '$RemoteDir/js' '$RemoteDir/scripts' -type f -exec chmod 644 {} \; 2>/dev/null || true && chmod 644 '$RemoteDir/index.html' '$RemoteDir/data.dat' '$RemoteDir/data.dat.gz' '$RemoteDir/semantic_threads.dat' '$RemoteDir/semantic_threads_ui.dat' '$RemoteDir/semantic_space_layout_manifest.json' '$RemoteDir/semantic-demo.css' '$RemoteDir/vector-explorer-polished.html' '$RemoteDir/vector-explorer-pandora.css' '$RemoteDir/.htaccess' 2>/dev/null || true"
)
