...
foreach ($Artifact in $DataDirArtifacts) {
    $BuiltArtifact = "dist/svelte/data/$Artifact"
    if (Test-Path $BuiltArtifact) {
        Invoke-Step @("scp", "-P", $Port, $BuiltArtifact, "$Target" + "data/$Artifact")
    } else {
        Write-Warning "Optional data twin missing: $BuiltArtifact"
    }
