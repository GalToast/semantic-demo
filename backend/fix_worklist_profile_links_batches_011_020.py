...
# Worklists in-scope for the "batches 011-020" pass.
WORKLIST_FILE_RE = re.compile(
    r"^registered-entities-batch-0(1[1-9]|20)(?:-[^-]+)?-worklist.*\.md$",
    re.IGNORECASE,
)
...
def load_registered_entities_index_map() -> dict[int, str]:
    ...
    This is grounded truth for where a lead lives after normalization/moves.
    """
...
def iter_in_scope_worklists() -> list[Path]:
    ...
    for p in sorted(BATCHES_ROOT.glob("*.md")):
        if WORKLIST_FILE_RE.match(p.name):
            out.append(p)
    return out
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fix stale worklist profile links for registered-entities batches 011-020 by rewriting links to the "
            "canonical LeadID profile path from leads/index.csv."
        )
    )
    ...
    parser.add_argument(
        "--out",
        default="",
        help="Report path (default: reports/fix-worklist-profile-links-batches-011-020-<date>.md)",
    )
    ...
    report_path = Path(args.out) if args.out else Path("reports") / (
        f"fix-worklist-profile-links-batches-011-020-{date.today().isoformat()}.md"
    )
    report_path.parent.mkdir(parents=True, exist_ok=True)

    lines: list[str] = []
    lines.append("# Fix Worklist Profile Links (Batches 011-020)")
    ...
    if not all_changes:
        ...
        for c in all_changes[:200]:
            lines.append(f"- {c.worklist.as_posix()} | LeadID {c.lead_id} | {c.old} -> {c.new}")
