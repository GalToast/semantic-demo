...
export function buildSelectedBusinessProps(
    ...
    const interestingNote =
        adapter && typeof adapter.getInterestingBusinessNote === 'function'
            ? adapter.getInterestingBusinessNote(point)
            : ''
    const matchNarrative =
        adapter && typeof adapter.buildSelectedMatchNarrative === 'function'
    ...
    if (Number.isFinite(point.lat) && Number.isFinite(point.lng)) {
        neighborhoodText = `${point.city || 'Montgomery County'}`
    }
    ...
    return {
        ...
        facts,
        sensitivityBadges,
        mapText: neighborhoodText || COPY.selectedEmptyMap || 'No location data',
        threadText,
        isPopulated: true
    }
}
