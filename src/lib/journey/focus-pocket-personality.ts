...
import { getFocusConstellationViewportProfile, seededUnit } from '@lib/journey/focus-pocket-geometry'
...
export function getNeighborhoodPersonality(index: number): NeighborhoodPersonality {
    ...
    const cities = new Set(
        primaryCandidates.map((c: SemanticCandidate) => normalizeCityForFilter((state.points as any)[c.index]?.city))
    )
