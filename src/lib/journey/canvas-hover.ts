...
import { showCanvasHoverPreview, hideCanvasHoverPreview } from './canvas-hover-preview'
...
export function clearCanvasFieldHover(
    ...
    _canvasFieldHoverClearTimer = canvasInteractionAdapter.setTimer(clear, CANVAS_FIELD_HOVER_CLEAR_DELAY_MS)
}
...
export function setCanvasFieldHover(candidate: HoverCandidate | null, canvas: HTMLCanvasElement | null): void {
    ...
    if (prev && Number.isFinite(prev.index)) {
        ...
        if (moved > STABLE_HOVER_STICKY_PX) {
            appState.stableCanvasHover = candidate
        } else {
    ...
    appState.lastCanvasNodeHover = stableCandidate
    showCanvasHoverPreview(stableCandidate.index, stableCandidate.screenX, stableCandidate.screenY)
}
