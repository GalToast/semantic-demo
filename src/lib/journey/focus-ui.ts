...
export function updateFocusNeighborRail(): void {
    ...
    candidates.forEach((candidate: any, order: number) => {
        ...
        const point = (Number.isFinite(candidate.index) && candidate.index >= 0 && candidate.index < points.length)
        ...
        const focusPoint = (Number.isFinite(focusIdx) && focusIdx! >= 0 && focusIdx! < points.length) ? (points[focusIdx!] as any) : null;
...
function updateWalkBreadcrumb(hasFocus: boolean = false): void {
    ...
    const points = getPoints()!;
    const history = ((getNavState()?.walkHistoryIndices || []) as number[])
...
export function updateTraversalUi(): void {
    ...
    const currentCandidate = nav.mode === 'trail' && nav.trailCursor >= 0 && neighborCount > 0
        ...
        : null;
    const sourceLabel = nav.threadSource === 'semantic'
        ? 'semantic thread'
    ...
    const nextWalkReason = nextWalkCandidate ? summarizeNeighborReason(nextWalkCandidate, nextWalkPoint as any, currentFocusPoint as any) : '';
    if (focusRouteEl) focusRouteEl.dataset.state = neighborCount ? (nav.mode === 'trail' ? 'walking' : 'ready') : 'empty';

    if (coldDegradedNoRail) {
        const queryLabel = (getSemanticLaneSnapshot() as any)?.query ? `"${(getSemanticLaneSnapshot() as any).query}"` : 'this semantic trail';
    ...
    if ((getTrailDepth() ?? 0) >= 1 && (nav.walkHistoryIndices || []).length >= 0) {
        ...
        const pocketNote = nav.threadSource === 'semantic' && focusPocketMeta?.active
