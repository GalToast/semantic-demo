...
export function syncSemanticDiveUi(): void {
    ...
    const nextExploreCandidate =
        active && currentFocusIndex !== null
    ...
    if (insideControls) {
        if (active) {
            ...
            insideControls.dataset.nextState = isExploring ? 'walking' : hasNextCandidate ? 'available' : 'complete'
        } else {
    ...
    if (insideNextButton) {
        ...
        insideNextButton.textContent = isExploring ? 'Following...' : hasNextCandidate ? 'Next Stop' : 'Trail Complete'
    ...
    if (label)
        label.textContent = isTransitioning ? 'Entering...' : active ? 'Inside Neighborhood' : 'Explore Neighborhood'
    if (copy) {
