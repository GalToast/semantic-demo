...
export function getThreadInspectionState(index: number | null = appState.inspectedThreadIndex, options: ThreadInspectionOptions = {}): ThreadInspectionState | null {
    ...
    const candidate = Number.isFinite(index)
        ? (appState.navState.threadCandidates as any[])?.find((item: any) => item && (typeof item === 'number' ? item === index : item.index === index))
        : null;
    const candidateIndex = candidate && typeof candidate === 'object' ? candidate.index : (typeof candidate === 'number' ? candidate : null);
    const point = candidateIndex !== null && candidateIndex >= 0 && candidateIndex < pts.length ? pts[candidateIndex] : null;
    ...
    const relationshipTitle = active && relationshipRole
        ...
        : '';
    const role = active ? (appState.navState.focusPocketRoleByIndex instanceof Map ? appState.navState.focusPocketRoleByIndex.get(candidateIndex!) : undefined) || candidateObj.role || 'trail' : '';
    const source = active
    ...
    const meta = active
        ...
        : 'Preview connection';
    const rawSurface = pinned ? 'pinned' : (options.surface || (typeof document !== 'undefined' ? document.body.dataset.threadInspectSurface : null) || null);
    ...
    if (!inspector.dataset.pointerGuardBound) {
        ...
        const pointerEnter = (): void => {
            ...
            if (clearTimerId) {
                ...
                withStateMutation(() => { (legacyState as any).canvasThreadInspectionClearTimer = null; });
                appState.canvasThreadInspectionClearTimer = null;
            }
        };
        const pointerLeave = (): void => {
            appState.threadInspectorPointerInside = false;
            if (typeof document !== 'undefined' && document.body.dataset.threadInspectSurface === 'canvas' && appState.pinnedThreadIndex === null) {
...
export function exploreThreadNeighbor(index: number, options: ThreadInspectionOptions = {}): { targetIndex: number; fromIndex: number | null; reason: string } | null {
    ...
    const fromIndex = Number.isFinite(options.fromIndex)
        ...
        : getFocusedIndex() !== null
          ? getFocusedIndex()
          : null;
    const candidate = (appState.navState.threadCandidates as any[])?.find((item: any) => item && (typeof item === 'number' ? item === index : item.index === index));
    ...
    dispatchNavTransition(NAV_TRANSITION_ACTIONS.WALK_TO, { index, fromIndex: fromIndex ?? undefined, appendHistory: !options.restoreHistory } as any);
    renderThreadInspection(index, { force: true, surface: options.surface || 'explore' });
    withStateMutation(() => { (legacyState.navState as any).lastTraversalReason = reason; });
