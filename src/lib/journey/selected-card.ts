 ...
 *
 * Ported from: js/modules/journey-selected-card.ts
 *
...
import { focusStore, setSelectedBusiness as setStoreSelectedBusiness } from '@lib/stores/focus.svelte.ts';
import { getActiveClusterFilter, getActiveFilters } from '@lib/stores/filter.svelte';
...
export function initJourneySelectedCard(deps: Record<string, unknown> = {}): void {
    ...
    const sync = (): void => {
        ...
        const selectedPoint = focusedIndex !== null && focusedIndex >= 0 && focusedIndex < records.length
            ? records[focusedIndex]
            : null;
        updateSelectedBusiness(selectedPoint || null, { skipHydrate: true });
    };
...
export function syncFocusStage(point: any): void {
    const points = getBusinessRecords();
    ...
    const cleanupFocusStageTrap = (): void => {
        ...
        if (getPreviouslyFocusedFocusStage()) {
            try {
                ...
                // Focus restore failure is non-critical
            }
    ...
    const focusedNode = getFocusedIndex();
    ...
    const activeClusterFilter = getActiveClusterFilter();
    const activeClusterFilterIndex = activeClusterFilter !== null ? Number(activeClusterFilter) : null;
    const isFilteredOut = effectiveIndex !== null
        && effectiveIndex >= 0
        && !isPointVisible(effectiveIndex, points, Number.isFinite(activeClusterFilterIndex) ? activeClusterFilterIndex : null, getActiveFilters());
...
export function updateSelectedBusiness(point: any, options: UpdateSelectedBusinessOptions = {}): void {
    ...
    setStoreSelectedBusiness(point || null);
    if (legacySelectedPointStore && typeof legacySelectedPointStore.set === 'function') {
        try { legacySelectedPointStore.set(point || null); } catch { /* ignore */ }
    }
