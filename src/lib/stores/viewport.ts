...
export const isCompactLandscape = derived(viewport, ($v) =>
  $v.width <= MOBILE_BREAKPOINT && $v.height <= COMPACT_LANDSCAPE_MAX_HEIGHT
);

/** Ultra-compact portrait: max-width 430px, height 741-860px. */
export const isUltraCompactPortrait = derived(viewport, ($v) =>
  $v.width <= ULTRA_COMPACT_MAX_WIDTH &&
  $v.height >= ULTRA_COMPACT_MIN_HEIGHT &&
  $v.height <= ULTRA_COMPACT_MAX_HEIGHT
);
