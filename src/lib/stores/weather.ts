/**
 * @lib/stores/weather.ts — Weather data store
 *
...
export const weatherData = writable<WeatherData>({ ...INITIAL_WEATHER });

// ── Derived ───────────────────────────────────────────────────────────────────

export const weatherTemperature = derived(weatherData, ($w) => $w.temperature);
export const weatherCondition = derived(weatherData, ($w) => $w.condition);
export const weatherLabel = derived(weatherData, ($w) => $w.label);
export const weatherForecast = derived(weatherData, ($w) => $w.forecast);
export const hasWeather = derived(weatherData, ($w) => $w.updatedAt > 0);
