 ...
 * Retryable = transient AND not permanent.
 * Errors that match neither category are retried conservatively
 * (better to retry a permanent failure once than to silently drop a transient one).
 */
export function isRetryableError(err: unknown): boolean {
    return isTransientError(err) || !isPermanentError(err)
}
...
/**
 * A provider configuration for use in a failover chain.
 * Each provider can have its own retry options and an identifier
 * used for logging.
 */
export interface ProviderConfig<TProviderMeta = string> {
    /** Human-readable label for the provider (e.g. "search-api", "local-index") */
    name: string
    /** Provider-specific metadata (e.g. a URL, an index name, a config key). */
    meta: TProviderMeta
    /** Optional per-provider retry overrides. Falls back to the callWithFailover defaults. */
    retryOptions?: RetryOptions
}

/**
 * A failover chain: an ordered list of provider configs tried in priority order.
 */
export type ProviderFailoverChain<TProviderMeta = string> = readonly ProviderConfig<TProviderMeta>[]

/**
 * Call a function across a failover chain of providers with per-provider retry.
 *
 * For each provider in the chain (in order):
 * 1. Retry the provider's `executor` with exponential backoff + jitter.
 * 2. If the provider succeeds, return the result immediately.
 * 3. If the provider exhausts retries on a transient error, log the
 *    failure and try the next provider.
 * 4. Permanent errors (400, 401, 403, 404, 422) are NOT retried and NOT
 *    failed over — they are re-thrown immediately.
 *
 * @param chain - Ordered list of provider configs
 * @param executor - Async function that takes a ProviderConfig and returns a result.
 *   Should throw on failure; permanent errors it throws will propagate immediately.
 * @param options - Default retry options for all providers (can be overridden per-provider)
 * @returns The result from the first successful provider
 * @throws The last error if all providers fail on transient errors,
 *   or the permanent error immediately if one is encountered.
 */
export async function callWithFailover<T, TProviderMeta = string>(
    chain: ProviderFailoverChain<TProviderMeta>,
    ...
    for (let i = 0; i < chain.length; i++) {
        ...
        try {
            ...
            // Permanent errors are never retried or failed over.
            if (isPermanentError(err)) {
                debugWarn(
                    `[failover] ${label}: provider "${provider.name}" permanent error, not failing over:`,
                    error.message
                )
                throw error
            }

            // Transient error: record and try next provider.
            lastTransientError = error
            debugWarn(
                `[failover] ${label}: provider "${provider.name}" failed after retries, ` +
                    `trying next provider (${i + 2}/${chain.length}): ${error.message}`
            )
        }
    }

    // All providers exhausted.
    throw lastTransientError ?? new Error(`${label}: all ${chain.length} providers failed`)
}
