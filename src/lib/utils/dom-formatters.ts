...
/**
 * Map a raw threadSource string to a composable, user-friendly noun.
 *
 * The engine stores `'semantic'` when relationship data backs the connections
 * and `'geometric-fallback'` (or any other string) when it falls back to
 * geographic proximity. Returns a noun ('similarity' / 'location') meant to
 * be composed as "matched by {label}" — see JourneyChrome.svelte and
 * focus-ui.ts. Raw values are forbidden jargon per docs/ux-copy-rules.md.
 */
export function formatThreadSourceLabel(source: string | null | undefined): string {
    if (source === 'semantic') return 'similarity'
    return 'location'
}
