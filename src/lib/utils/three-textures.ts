/**
 * @lib/utils/three-textures.ts — Canvas-based dynamic texture generation for WebGL
 *
...
/**
 * Creates a spore point-sprite texture (soft radial glow with inner core).
 * Used for the base node appearance in the semantic field.
 */
export function createSporeTexture(): CanvasTexture {
...
/**
 * Creates a focus-ring texture (concentric ring glow).
 * Used when a node is focused/selected.
 */
export function createFocusRingTexture(): CanvasTexture {
  const canvas = document.createElement('canvas');
  canvas.width = 96;
  canvas.height = 96;
  const ctx = canvas.getContext('2d')!;
...
/**
 * Creates a focus-next-cue texture (arrow pointer with ring).
 * Used to indicate the next trail stop during journey walking.
 */
export function createFocusNextCueTexture(): CanvasTexture {
  const canvas = document.createElement('canvas');
  canvas.width = 128;
  canvas.height = 128;
  const ctx = canvas.getContext('2d')!;
