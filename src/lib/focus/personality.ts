...
import { getBusinessRecords, getPointIndexByLeadId, getSemanticNeighborMap } from '@lib/data-store'
...
export type PersonalityType = 'STANDARD' | 'DEEP_DIVE' | 'DENSE_HUB' | 'BRIDGE_NODE' | 'EDGE_NODE' | 'TIGHT_CLUSTER'
...
export function getNeighborhoodPersonality(
    index: number,
    primaryCandidates: Array<{ index: number; semanticScore?: number; score?: number }> = getSemanticCandidateSlice(
        index,
        8
    ),
