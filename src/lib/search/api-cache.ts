...
export async function fetchSemanticSearchResults(
    ...
    const retryDelays =
        Array.isArray(options.retryDelaysMs) && options.retryDelaysMs.length
        ...
        try {
            ...
            if (detectStaticDevPHP(responseText) && allowsStaticDevFallback()) {
                if (shouldLogStaticDevFallback()) {
                    debugWarn(
                        '[semantic-search-api-cache] Detected raw PHP response. Assuming static dev server. Returning mock results.'
                    )
                }
                ...
                try {
                    ...
                    Object.defineProperty(jsonErr as object, 'correlationId', {
                        value: crypto.randomUUID(),
                        writable: false,
                        configurable: true
                    })
            ...
            if (!response.ok || !payload?.ok) {
                const err: Error & { correlationId?: string } = new Error(
                    payload?.error || 'Semantic search is unavailable right now.'
                )
            ...
            lastError =
                attemptTimedOut || (error as Error)?.name === 'AbortError'
