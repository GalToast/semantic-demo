...
export function resultMatchesNumericSearchQuery(
    result: { point?: Point; address?: string; publicNote?: string; publicDetail?: string; naics?: string } | null,
    query: unknown
): boolean {
    const digits = String(query || '').replace(/\D/g, '')
    if (!digits || !result?.point) return false
    const point = result.point
    const exactFields = [point.lead_id, point.phone, point.lat, point.lng].map((v) =>
...
function getMockRowTerms(row: ServiceResultRow): string[] {
    const text = normalizeMockSearchText(
        [row?.name, row?.naics, row?.public_note, row?.public_detail, row?.city].filter(Boolean).join(' ')
    )
...
function scoreMockPointForRow(point: Point, terms: string[]): number {
    ...
    const text = normalizeMockSearchText(
        [point.name, point.what, point.city, point.naics, point.trivia].filter(Boolean).join(' ')
    )
...
function getMockFallbackPointIndex(row: ServiceResultRow, order: number = 0): number | null {
    ...
    if (
        ...
        isPointVisible(hintedIndex, points, state.activeClusterFilter as number | null, state.activeFilters) &&
    ...
    points.forEach((point: Point, index: number) => {
        if (point && isPointVisible(index, points, state.activeClusterFilter as number | null, state.activeFilters)) {
...
export function mapSemanticSearchServiceResult(row: ServiceResultRow, order: number = 0): SearchResult | null {
    ...
    if (isMockRow) {
        ...
        rawPointIndex = state.pointIndexByLeadId.get(String(row.lead_id))
        ...
        if (
            !sourcePoint ||
            !isPointVisible(rawPointIndex, points, state.activeClusterFilter as number | null, state.activeFilters)
        )
            return null
