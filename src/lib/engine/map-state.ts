 ...
 */
import { state, withStateMutation, type Point } from '@legacy/state';
import { subscribeKeyed, EVENTS } from '@legacy/modules/event-bus';
import { pointHasGeocode, isPointVisible } from '@legacy/modules/utils/geo-data';
import { formatBusinessName } from '@lib/utils/dom-formatters';
import { showExperienceToast } from '@lib/ui/ui-feedback';
import { focusOnPoint } from '@lib/orchestration/lifecycle';
import { hideTooltip } from '@legacy/modules/tooltip';
...
export async function initMap(): Promise<void> {
    const mapState = state as unknown as {
        ...
        navState: { focusedIndex: number | null; walkHistoryIndices: number[]; trailNeighborIndices: number[]; mode: string };
    ...
    try {
        ...
        const L = window.L! as {
            ...
            polyline: (latLngs: Array<[number, number]>, options: Record<string, unknown>) => { addTo(layer: unknown): unknown };
        ...
        const ms = mapState as unknown as { mapInitialized: boolean; map: unknown; markersLayer: unknown; mapRouteLayer: unknown; pointMarkers: unknown[] };
...
export function centerMapOnRouteAnchor(): boolean {
    ...
    const routePoints = getMapRoutePoints();
    const routeLatLngs: Array<[number, number]> = routePoints.map(({ point }: { point: Point }) => [point.lat!, point.lng!]);
    const L = window.L! as {
        latLngBounds: (latLngs: Array<[number, number]>) => unknown;
    };
    if (routeLatLngs.length >= 2) {
        const bounds = L.latLngBounds(routeLatLngs);
        (state.map as { fitBounds: (b: unknown, opts: Record<string, unknown>) => void }).fitBounds(bounds as unknown as Parameters<typeof state.map extends infer T ? T extends { fitBounds: (...a: unknown[]) => unknown } ? T : never : never>[0], {
            animate: true,
            maxZoom: 15,
        ...
        (state.map as { setView: (latLng: [number, number], zoom: number, opts: Record<string, unknown>) => void }).setView([focusPoint!.lat!, focusPoint!.lng!], 15, { animate: true });
    }
    return true;
}
...
export function getRouteEmbodimentIndices(): number[] {
    ...
    const pushIndex = (index: number | null | undefined): void => {
        if (!Number.isFinite(index) || index === null || index === undefined || index < 0 || index >= state.points.length) return;
