...
function buildGeometricMyceliumEdges(clusterMembers: Map<number, number[]>, clusterCentroids: Map<number, { x: number; y: number; z: number }>): MyceliumEdgeSets | undefined {
        ...
        clusters
            ...
            .forEach(({ cluster: otherCluster, dist }) => {
                ...
                const a = (clusterMembers.get(cluster) || []).slice().sort((left, right) => {
                    ...
                    if (!lp || !rp) return 0;
                    return Math.hypot(lp.x - otherCentroid.x, lp.y - otherCentroid.y, lp.z - otherCentroid.z)
                ...
                const b = (clusterMembers.get(otherCluster) || []).slice().sort((left, right) => {
                    ...
                    const rp = state.nodePositions[right];
                    if (!lp || !rp) return 0;
                    return Math.hypot(lp.x - centroid.x, lp.y - centroid.y, lp.z - centroid.z)
...
function buildSemanticMyceliumEdges(): MyceliumEdgeSets | null {
    ...
    state.points.forEach((point: any, index: number) => {
        ...
        record?.neighbors?.forEach((neighbor: any) => {
            ...
            const bridgeLike = String(neighbor.threadType || '').toLowerCase().includes('bridge') || bridgeScore >= 0.62;
    ...
    return corePairs.length || wispyPairs.length || bridgePairs.length
        ? { corePairs, wispyPairs, bridgePairs }
        : null;
}
