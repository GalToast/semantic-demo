...
function _acquireWithMutation(ctx: BridgeContext): void {
    ...
    if (typeof window !== 'undefined' && (window as unknown as Record<string, unknown>).withStateMutation) {
        ctx._withMutation = (window as unknown as Record<string, unknown>).withStateMutation as <T>(fn: () => T) => T
        return
    }
...
function unbindEventBridge(ctx: BridgeContext): void {
    ...
    if (ctx._sceneReadyHandler) {
        window.removeEventListener('scene-ready', ctx._sceneReadyHandler as EventListener)
    ...
    return {
        ...
        async init(canvas: HTMLCanvasElement): Promise<void> {
            ...
            try {
                ...
                if (ctx._state) {
                    ...
                    if (Array.isArray(legacyGeometryState.nodePositions))
                        appState.nodePositions = legacyGeometryState.nodePositions
                    if (Array.isArray(legacyGeometryState.targetPositions))
                        appState.targetPositions = legacyGeometryState.targetPositions
                    if (Array.isArray(legacyGeometryState.originalPositions))
                        appState.originalPositions = legacyGeometryState.originalPositions
                }
                ...
                if (ctx._state?.points?.length && ctx._state?.nodePositions?.length) {
                    try {
                        _createMycelium()
                    } catch (threadErr) {
                ...
                try {
                    ...
                    ctx._removeCanvasInteraction = disposeCanvasNodeInteractionBindings
                ...
                if (ctx._state) {
                    ...
                    loadSemanticThreads({ reason: 'svelte-bridge-init' }).catch((err: unknown) => {
                        console.warn('[EngineBridge] TS semantic-threads background load failed:', err)
                    })
                }
        ...
        applyFilters(filters: ActiveFilters, _options: FilterOptions = {}): void {
            ...
            if (ctx._filterState) {
                ...
                ctx._state.filterColorVersion = (ctx._state.filterColorVersion ?? 0) + 1
            }
        ...
        getFocusedIndex(): number | null {
            return Number.isFinite(ctx._state?.focusedNode) ? (ctx._state?.focusedNode ?? null) : null
        }
    }
}
