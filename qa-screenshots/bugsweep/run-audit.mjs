...
const REPORT = path.resolve(process.cwd(), 'tmp', 'visual-ui-audit-3.md')
...
// Every structural check result, keyed by surface, for report emission
const allChk = []
...
async function dismissSplash(page) {
    ...
    await page.evaluate(() => {
        const cta = document.querySelector('[data-testid="splash-cta"], [data-testid="placeholder-cta"]')
...
async function structuralChecks(page, surfaceName) {
    ...
    allChk.push({ surface: surfaceName, result: results })
    return results
}
...
async function main() {
    ...
    try {
        ...
        // Surface 3: Search open (deep-link triggers search)
        console.log('Surface: search-coffee')
        await loadAndSettle(dp, BASE_URL + '/?nodemo=1&q=coffee')
        ...
        // Surface 4: Focus pocket (deep-link focuses a business)
        console.log('Surface: focus-pocket')
        await loadAndSettle(dp, BASE_URL + '/?nodemo=1&anchor=519')
    ...
    try {
        ...
        // Mobile Surface 3: Focus pocket (deep-link)
        console.log('Surface: mobile-focus-pocket')
        await loadAndSettle(mp, BASE_URL + '/?nodemo=1&anchor=519')
    ...
    report += '## Section 3: Structural Findings (per-surface)\n\n'
    for (const { surface, result } of allChk) {
        report += `### ${surface}\n\n`
        report += `- canvasNonBlank: ${result.canvasNonBlank}\n`
        report += `- overflow: horizontal=${result.overflow.horizontal} (scrollW ${result.overflow.scrollWidth} vs vp ${result.overflow.viewportW}), vertical=${result.overflow.vertical} (scrollH ${result.overflow.scrollHeight} vs vp ${result.overflow.viewportH})\n`
        if (result.clickEating.length) {
            report += `- CLICK-EATING (${result.clickEating.length}):\n`
            for (const c of result.clickEating) {
                report += `  - target ${c.target} (z=${c.targetZ}) blockedBy ${c.blockedBy} (z=${c.blockerZ}) rect=${JSON.stringify(c.targetRect)}\n`
            }
        } else {
            report += `- click-eating: none\n`
        }
        if (result.duplicateIds.length) {
            report += `- DUPLICATE-IDS: ${result.duplicateIds.map((d) => `${d.id}(x${d.count})`).join(', ')}\n`
        } else {
            report += `- duplicate-ids: none\n`
        }
        if (result.lowContrast.length) {
            report += `- LOW-CONTRAST: ${result.lowContrast.map((l) => `"${l.text}" ${l.tag} color=${l.color} bg=${l.bg} alpha=${l.alpha}`).join('; ')}\n`
        } else {
            report += `- low-contrast: none\n`
        }
        if (result.emptyInteractive.length) {
            report += `- EMPTY-INTERACTIVE: ${result.emptyInteractive.map((e) => `${e.tag}${e.id}${e.cls} ${JSON.stringify(e.rect)}`).join('; ')}\n`
        } else {
            report += `- empty-interactive: none\n`
        }
        if (result.panelOverflow) {
            const clipped = Object.entries(result.panelOverflow).filter(([, v]) => v.hClipped || v.vClipped)
            if (clipped.length) {
                report += `- PANEL-CLIPPED: ${clipped.map(([sel, v]) => `${sel} (scroll ${v.sw}x${v.sh} vs client ${v.cw}x${v.ch})`).join('; ')}\n`
            }
        }
        if (result.networkFails && result.networkFails.length) {
            report += `- NETWORK-FAILS: ${result.networkFails.map((n) => `${n.url} ${n.status}`).join('; ')}\n`
        }
        report += '\n'
    }
