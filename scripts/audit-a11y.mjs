...
function auditFile(filePath) {
    ...
    while ((tagMatch = tagRegex.exec(content)) !== null) {
        ...
        if (tagName === 'button') {
            ...
            if (closeIdx !== -1) {
                const innerText = content.substring(offset + fullTag.length, closeIdx).replace(/<[^>]*>/g, '').trim();
        ...
        const NON_SEMANTIC = new Set([
            ...
            'p', 'li', 'ul', 'ol', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'svg',
        ]);
        if (NON_SEMANTIC.has(tagName)) {
            const hasClick = /on(click|pointerdown|mousedown|keydown)\s*=/i.test(attrsText)
    ...
    while ((ariaMatch = ariaHiddenRegex.exec(content)) !== null) {
        ...
        if (/<(button|input|select|textarea)\b/i.test(innerContent)
...
if (jsonMode) {
    ...
    console.log(`  ${allFindings.length} findings across ${fileCount} file(s) — ` +
