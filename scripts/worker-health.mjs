...
//
// As of W56 the external-subagents server itself now surfaces retry_count,
// last_error, provider_health and thinking_* in external_subagent_poll.
// This script remains useful for offline log forensics and for detecting the
// "timeout with zero output" spawn-wedge that poll also cannot see.
import { readFileSync, existsSync, readdirSync } from 'node:fs'
import { join } from 'node:path'

function resolveLog(arg) {
    if (existsSync(arg)) {
        return { stdout: arg, stderr: arg.replace(/stdout\.log$/, 'stderr.log') }
    }
    if (arg.startsWith('ocw_') || /^ocw[/_]/.test(arg)) {
        ...
        if (existsSync(f)) return { stdout: f, stderr: join(dir, 'stderr.log') }
    }
    ...
    if (existsSync(root)) {
        for (const d of readdirSync(root)) {
            if (d.startsWith(arg) || d.includes(arg)) {
                ...
                if (existsSync(f)) return { stdout: f, stderr: join(root, d, 'stderr.log') }
            }
        }
    }
...
function main() {
    ...
    if (!arg) {
        console.error('usage: worker-health.mjs <worker-id-or-path>')
        process.exit(2)
    }
    const { stdout, stderr } = resolveLog(arg)
    const raw = readFileSync(stdout, 'utf8')
    const rawStderr = existsSync(stderr) ? readFileSync(stderr, 'utf8') : ''
    ...
    for (const line of lines) {
        ...
        if (j.type === 'auto_retry_start') {
            retries++
            lastRetryAttempt = j.attempt
            if ('__retryAfterHonored__' in j) retryAfterHonored = j.__retryAfterHonored__
        }
        // thinking: streamed deltas OR non-streamed thinking content blocks (assistant only)
        if (j.type === 'thinking_delta') thinking++
        else if (
            j.message &&
            j.message.role === 'assistant' &&
            Array.isArray(j.message.content) &&
            j.message.content.some((c) => c.type === 'thinking')
        )
            thinking++
        // text: streamed deltas OR non-streamed final text (assistant only; some providers return full text in one shot)
        if (j.type === 'text_delta') text++
        else if (
            j.message &&
            j.message.role === 'assistant' &&
            Array.isArray(j.message.content) &&
            j.message.content.some((c) => c.type === 'text' && c.text)
        )
            textNonStreamed++
    ...
    console.log('worker-health:', stdout)
    ...
    console.log('  --- provider health (now in poll, kept here for offline forensics) ---')
    ...
    console.log(
        '  thinking deltas:  ',
        thinking,
        thinking && !text && !textNonStreamed ? '  <<< reasoning-only (still thinking OR stuck)' : ''
    )
    console.log(
        '  text deltas:      ',
        text,
        textNonStreamed ? '(+' + textNonStreamed + ' non-streamed final blocks)' : ''
    )
    ...
    if (usage) {
        console.log(
            ...
            JSON.stringify({
                ...
                cost: usage.cost && usage.cost.total
            })
        )
    } else {
    ...
    const timeoutInStderr = /Pi RPC worker timed out after \d+s/i.test(rawStderr)
    const zeroAssistantOutput = !firstAssistantTs && !hasOutput && thinking === 0 && toolCalls === 0
    if (timeoutInStderr && zeroAssistantOutput && connErr === 0 && retries === 0)
        console.log(
            '  VERDICT: SPAWN-WEDGE — timed out with zero assistant output, zero connection errors, zero retries. Provider never returned any events. Try relaunching; if it repeats, restart the external-subagents MCP server (or the whole Pi session).'
        )
    else if (timeoutInStderr && (connErr > 0 || retries > 0))
        console.log(
            '  VERDICT: timed out while stuck in a connection-error retry loop. Check router/upstream, consider a different model/provider.'
        )
    else if (connErr > 0 && !hasOutput && thinking < 20)
        console.log(
            '  VERDICT: stuck in connection-error retry loop — NOT progressing. Check router/upstream, consider relaunch.'
        )
    else if (connErr > 0 && hasOutput)
        console.log('  VERDICT: hit connection errors but RECOVERED and produced output.')
    else if (connErr > 0 && !hasOutput)
        console.log('  VERDICT: connection errors, no output yet — retrying (check router/upstream).')
    else if (thinking > 0 && !hasOutput && !agentSettled)
        console.log('  VERDICT: reasoning, no visible output yet — likely thinking (verify with another poll).')
