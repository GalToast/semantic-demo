...
const OUT = 'dist/svelte/data/rows.bin'
...
for (const r of rows) {
    ...
    let o = 0
    for (let i = 0; i < 3; i++) {
        rec.writeFloatLE(Number(r[i]) || 0, o)
        o += 4
    }
    rec.writeUInt32LE(Number(r[3]) || 0, o)
    o += 4
    rec.writeUInt32LE(Number(r[7]) || 0, o)
    o += 4
    rec.writeFloatLE(Number(r[8]) || 0, o)
    o += 4
    rec.writeFloatLE(Number(r[9]) || 0, o)
    o += 4
    // string columns 4,5,6,10,11,12,13,14,15 interned (u32 idx each)
    for (const ci of [4, 5, 6, 10, 11, 12, 13, 14, 15]) {
        rec.writeUInt32LE(intern(String(r[ci] ?? '')), o)
        o += 4
    }
    chunks.push(rec)
}
...
console.log(
    `TDBR: ${binMB.toFixed(3)}MB vs raw ${rawMB.toFixed(2)}MB -> ${((1 - binMB / rawMB) * 100).toFixed(1)}% smaller | rows ${rows.length} | str ${strtab.length}`
)
