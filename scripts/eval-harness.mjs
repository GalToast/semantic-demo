...
function evaluateAssertion(a, record) {
    ...
    if (type === 'json_schema') {
        try {
            let parsed
            try {
                parsed = typeof record[target] === 'string' ? JSON.parse(record[target]) : record[target]
            } catch {
                parsed = record[target]
            }
...
function validateAssertionShape(assertions, prefix) {
    ...
    for (let j = 0; j < assertions.length; j++) {
        const a = assertions[j],
            al = `${prefix}.assertions[${j}]`
        if (!a || typeof a !== 'object' || !a.type) {
            errors.push(`${al}: missing type`)
            continue
        }
        if (!ASSERTION_TYPES.has(a.type)) errors.push(`${al}: unknown type "${a.type}"`)
        if ((a.type === 'exact_match' || a.type === 'includes') && a.value === undefined)
            errors.push(`${al}: missing value`)
        if ((a.type === 'file_exists' || a.type === 'json_schema' || a.type === 'llm_judge') && !a.target)
            errors.push(`${al}: missing target`)
...
function validateManifest(manifestPath) {
    ...
    const runs = Array.isArray(manifest)
        ? manifest
        : manifest?.runs && Array.isArray(manifest.runs)
          ? manifest.runs
          : null
    if (!runs) {
        console.error('manifest must be a JSON array or object with runs array')
        process.exit(1)
    }
...
function summarize() {
    ...
    for (const [model, group] of byModel) {
        const n = group.length,
            successes = group.filter((r) => r.success)
...
function evaluateRun(run, record) {
    ...
    if (run.timeout_seconds && record.elapsedMs > run.timeout_seconds * 1000)
        verdicts.push({
            pass: false,
            label: 'elapsedMs <= timeout_seconds',
            detail: `${record.elapsedMs}ms > ${run.timeout_seconds}s`
        })
...
function runMode(manifestPath, ciMode) {
    ...
    else if (manifest?.runs && Array.isArray(manifest.runs)) {
        runs = manifest.runs
        if (manifest.min_success !== undefined) minSuccess = Number(manifest.min_success)
    } else {
        console.error('manifest must be a JSON array or object with runs array')
        process.exit(1)
    }
    ...
    const results = runs.map((run) => {
        ...
        if (!record)
            return {
                id: run.id,
                pass: false,
                verdicts: [{ pass: false, label: 'record', detail: 'no matching log row found' }]
            }
    ...
    if (ciMode) {
        ...
        for (const row of loadLog()) {
            ...
            const s = modelStats.get(key)
            s.total++
            if (row.success) s.successes++
        }
        let modelFail = false
        for (const [model, stat] of modelStats) {
            ...
            if (rate < minSuccess) {
                modelFail = true
                console.log(
                    `  ✗ ${model} success rate ${(rate * 100).toFixed(1)}% < ${(minSuccess * 100).toFixed(0)}% threshold (${stat.successes}/${stat.total})`
                )
            }
        }
