...
const JOURNEY_NAVIGATION_OWNER_VALUES = ['journey-compass', 'map-trail-strip', 'map-controls', 'scene', 'inside-walk']
...
for (const relPath of CSS_FILES) {
    ...
    for (const value of PANEL_SURFACE_VALUES) {
        // Single-quoted
        const sqPattern = new RegExp(`body\\.is-active\\[data-panel-surface='${value}'\\]`, 'g')
        ...
        // Double-quoted
        const dqPattern = new RegExp(`body\\.is-active\\[data-panel-surface="${value}"\\]`, 'g')
    ...
    for (const value of ACTIVE_VIEW_VALUES) {
        const sqPattern = new RegExp(`body\\.is-active\\[data-active-view='${value}'\\]`, 'g')
        ...
        const dqPattern = new RegExp(`body\\.is-active\\[data-active-view="${value}"\\]`, 'g')
        const dqMatches = out.match(dqPattern)
        if (dqMatches) {
    ...
    for (const value of JOURNEY_NAVIGATION_OWNER_VALUES) {
        const sqPattern = new RegExp(`body\\.is-active\\[data-journey-navigation-owner='${value}'\\]`, 'g')
        ...
        const dqPattern = new RegExp(`body\\.is-active\\[data-journey-navigation-owner="${value}"\\]`, 'g')
        const dqMatches = out.match(dqPattern)
        if (dqMatches) {
...
console.log(
    `  TOTAL:                                       ${totalPanelSurface + totalPanelSurfacePrefix + totalActiveView + totalJourneyNav}`
)
