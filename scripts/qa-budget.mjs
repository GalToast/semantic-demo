...
async function findNewestBaseline() {
  ...
  try {
    ...
    matches.sort();
    newest = resolve(docsDir, matches[matches.length - 1]);
  } catch {
    return null;
  }
  return newest;
...
`);
  ...
  if (baselineMode === 'read' && baselineFile) {
    ...
    console.log(`Baseline JS total: ${kb(jSSize)} KB  |  Current JS total: ${kb(s.totalBytes)} KB  |  Delta: ${diff >= 0 ? '+' : ''}${kb(diff)} KB`);

    const mtChunks = modeTransitionChunks(s);
    let mtGrowth = false;
    for (const c of mtChunks) {
      const bs = baseMap.get(c.name) ?? 0;
      const d = c.size - bs;
