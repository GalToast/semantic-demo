...
function main(argv) {
    ...
    switch (action) {
        case 'set': {
            if (!cond) {
                console.error('set needs a condition')
                process.exit(2)
            }
        ...
        case 'step': {
            ...
            if (!st) {
                console.error('no goal state — set first (goal.mjs set "cond::cmd: ...")')
                process.exit(2)
            }
        ...
        case 'pause': {
            const st = readStateFile(stateFile)
            if (!st) {
                console.error('no goal')
                process.exit(2)
            }
            st.status = 'paused'
            writeStateFile(st, stateFile)
            console.log('PAUSED')
            break
        }
        case 'resume': {
