...
const WRONG_GUESSES = {
    'task-13': 'function dedupeLast(a){const s=new Set(),o=[];for(const x of a){if(s.has(x))continue;s.add(x);o.push(x)}return o}module.exports={dedupeLast};', // keep-FIRST (common default)
...
const FIXED_IMPLS = {
    'task-13': 'function dedupeLast(a){const s=new Set(),o=[];for(let i=a.length-1;i>=0;i--){if(s.has(a[i]))continue;s.add(a[i]);o.unshift(a[i])}return o}module.exports={dedupeLast};',
    'task-14': 'function wrap(i,n){return ((i%n)+n)%n}module.exports={wrap};',
    'task-15': 'function deepMerge(a,b){if(a==null)return b==null?{}:b;if(b==null)return a;if(Array.isArray(a)||Array.isArray(b)){return (Array.isArray(a)?a:[]).concat(Array.isArray(b)?b:[])}if(typeof a==="object"&&typeof b==="object"){const o={...a};for(const k of Object.keys(b))o[k]=deepMerge(a[k],b[k]);return o}return b}module.exports={deepMerge};'
}

function taskList() {
    return readdirSync(TASKS_DIR).filter((d) => d.startsWith('task-') && !d.startsWith('.')).sort()
}
...
for (const t of targets) {
    ...
    if (!trapTier) {
        ...
        if (buggy.pass) { fails++; console.log(`❌ ${t} BUGGY-PASSES (committed src must fail its test)`); rows.push({ task: t, buggy: 'BUGGY-PASSES' }) }
    ...
    rows.push({ task: t, buggy: buggy.pass ? 'BUGGY-PASSES' : 'buggy-fails', wrong: wrong.pass ? 'WRONG-PASSES' : 'wrong-fails', fixed: fixed.pass ? 'fixed-passes' : 'FIXED-FAILS' })
    if (!ok) { fails++; console.log(`❌ ${t}`) } else console.log(`✅ ${t}`)
}
if (fails > 0) {
    console.log(`\nGATE RED — ${fails} task(s) fail the semantic-trap contract:`)
    rows.filter((r) => r.buggy === 'BUGGY-PASSES' || r.wrong === 'WRONG-PASSES' || r.fixed === 'FIXED-FAILS').forEach((r) => console.log('  ', JSON.stringify(r)))
    process.exit(1)
}
