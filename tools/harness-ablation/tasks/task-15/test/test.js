...
assert(
    J(deepMerge({ tags: ['x'] }, { tags: ['y'] })) === J({ tags: ['x', 'y'] }),
    'array-concat (got ' + J(deepMerge({ tags: ['x'] }, { tags: ['y'] })) + ')'
)
assert(
    J(deepMerge({ n: { a: 1, q: 2 } }, { n: { a: 9 } })) === J({ n: { a: 9, q: 2 } }),
    'nested-merge keeps q (got ' + J(deepMerge({ n: { a: 1, q: 2 } }, { n: { a: 9 } })) + ')'
)
