...
export const UNCLASSIFIED_RELATIONSHIP_ROLE: string = 'unclassified';
...
export function getRelationshipRoleCopy(role: string | null | undefined): RelationshipRoleCopy {
    return (RELATIONSHIP_ROLE_COPY as Record<string, RelationshipRoleCopy>)[normalizeRelationshipRole(role)] ??
        (RELATIONSHIP_ROLE_COPY as Record<string, RelationshipRoleCopy>)[UNCLASSIFIED_RELATIONSHIP_ROLE];
}
...
export function getRelationshipRoleLabel(role: string | null | undefined, surface: string = 'title'): string {
    ...
    return (copy as Record<string, string>)[surface] || copy.title;
}
