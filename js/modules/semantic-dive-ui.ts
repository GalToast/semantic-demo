...
export function syncSemanticDiveUi(): void {
    ...
    const deadline = (state as any)._semanticDiveTransitionDeadline || 0;
    ...
    const hasWalked = ((getNavState() as any)?.explorationHistoryIndices || []).length > 1;

    if (insideControls) {
        if (active) {
            insideControls.hidden = false;
            insideControls.setAttribute('aria-hidden', 'false');
            (insideControls as any).inert = false;
            ...
            (insideControls as any).inert = true;
    ...
    if (diveButton) (diveButton as any).inert = !showDiveButton;
    ...
    if (copy) {
        (copy as HTMLElement).textContent = isTransitioning
