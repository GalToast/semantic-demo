...
export async function initEventListeners({
    ...
    try {
        ...
        if (typeof (searchResults as any)?.initSearchResultsSvelteIsland === 'function') (searchResults as any).initSearchResultsSvelteIsland();
        if (typeof (selectedDetails as any)?.initSelectedDetailsSvelteIsland === 'function') (selectedDetails as any).initSelectedDetailsSvelteIsland();
        if (typeof (filterChrome as any)?.initFilterChromeSvelteIsland === 'function') (filterChrome as any).initFilterChromeSvelteIsland();
    } catch (e) {
        console.warn('[event-bindings] failed to load svelte islands', (e as Error)?.message);
    }
}
