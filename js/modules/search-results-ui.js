...
export function renderSearchResultItems(resultsEl, results, renderContext, statusEl) {
    ...
    if (total > visibleCount) {
        ...
        btn.onclick = () => {
            ...
            try { sessionStorage.setItem('searchVisibleCount', String(nextVisibleCount)); } catch {}
...
export function updateSemanticSearchRetryState({ statusEl, trimmedQuery, attempt, nextAttempt, delayMs, retryTotal }) {
    ...
    publish(EVENTS.SEMANTIC_LANE_STATE_REQUESTED, {
        ...
        options: { label: 'Search reconnecting', title: 'Public semantic search is retrying while the current result rail stays visible.' }
    });
...
export function applySemanticSearchDegradedState(resultsEl, statusEl, trimmedQuery, _error) {
    ...
    if (preservingSameQuery) {
        ...
        const searchState = resultsEl._searchStateNamespace;
        if (retryBtn && searchState) retryBtn.onclick = () => searchState.search(trimmedQuery, { preferCachedResults: false });
...
export function startMobileRouteFieldPeek({ resultsEl = null, activeIndex: _activeIndex = null, reason = 'search-corridor' } = {}) {
