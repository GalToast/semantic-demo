...
export function resetFocusThreadDiagnostics(reason: string = 'inactive'): void {
    (state as any).focusThreadDiagnostics = {
        active: false,
        reason,
        edgeCount: 0,
        directEdgeCount: 0,
        ...
        buildMs: 0,
        avgFrameMs: (state as any).focusFrameDiagnostics?.avgFrameMs || 0,
        maxFrameMs: (state as any).focusFrameDiagnostics?.maxFrameMs || 0
    };
}

export function removeFocusSemanticOverlay(): void {
    if (!(state as any).focusSemanticLines) return;
    const parent = (state as any).focusSemanticLines.parent || (state as any).myceliumGroup || (state as any).scene;
    if (parent) parent.remove((state as any).focusSemanticLines);
    (state as any).focusSemanticLines.geometry?.dispose?.();
    (state as any).focusSemanticLines.material?.dispose?.();
    (state as any).focusSemanticLines = null;
    (state as any).focusSemanticConnectionPairs = [];
}

function getFocusCurvePoint(edge: any, t: number): THREE.Vector3 {
    if (typeof getFocusThreadCurvePoint === 'function') {
        return getFocusThreadCurvePoint(edge, t);
    }
    const a = (state as any).nodePositions[edge.a];
    const b = (state as any).nodePositions[edge.b];
...
function buildFocusThreadLineMaterial(): any {
    const baseOpacity = (state as any).navState.focusPocketMeta?.active ? 0.18 : 0.24;
...
export function refreshFocusSemanticOverlay(): void {
    ...
    const focusLineParent = (state as any).myceliumGroup || (state as any).scene;
    if (!focusLineParent) {
    ...
    if ((state as any).navState.threadSource !== 'semantic') {
        resetFocusThreadDiagnostics('non-semantic-thread');
        return;
    }

    const focusIndex = (state as any).navState.focusedIndex;
    const nextFocusIndex = getActiveNextFocusIndex();
    const focusPointAtFocus = (Number.isFinite(focusIndex) && focusIndex >= 0 && focusIndex < (state as any).points.length) ? (state as any).points[focusIndex] : null;
    const focusCluster = focusPointAtFocus?.cluster ?? 0;
    const semanticCandidates = ((state as any).navState.threadCandidates || [])
        .filter((candidate: any) => candidate?.source === 'semantic' && candidate.index !== focusIndex)
        .filter((candidate: any) => isPointVisible(candidate.index, (state as any).points, null, (state as any).activeFilters))
        .slice(0, 10);
    const roleByIndex = (state as any).navState.focusPocketRoleByIndex instanceof Map
        ? (state as any).navState.focusPocketRoleByIndex
        : new Map();
    ...
    const pocketThreadIndices = ((state as any).navState.focusPocketIndices || [])
    ...
    const pocketSet = new Set((state as any).navState.focusPocketIndices || []);
    ...
    const addEdge = (a: number, b: number, role: string = 'direct', edgePriority: number = 0.66): void => {
        ...
        if (!(state as any).nodePositions[a] || !(state as any).nodePositions[b]) return;
        ...
        const candidateCluster = (state as any).points[b]?.cluster ?? focusCluster;
        ...
        for (let segment = 0; segment < (state as any).FOCUS_THREAD_SEGMENTS; segment += 1) {
            const t0 = segment / (state as any).FOCUS_THREAD_SEGMENTS;
            const t1 = (segment + 1) / (state as any).FOCUS_THREAD_SEGMENTS;
            const segmentEdge = { ...edge, t0, t1, cue: isNextEdge ? 1 : 0 };
            (state as any).focusSemanticConnectionPairs.push(segmentEdge);
    ...
    (state as any).focusSemanticLines = new Line2(lineGeometry, lineMaterial);
    (state as any).focusSemanticLines.computeLineDistances();
    (state as any).focusSemanticLines.userData = {
        ...
        supportEdgeCount,
        subduedEdgeCount,
        segmentCount: (state as any).focusSemanticConnectionPairs.length,
        vertexCount: positions.length / 3,
        overlayNodeCount: overlayIndices.length,
        parentKind: (state as any).myceliumGroup ? 'mycelium' : 'scene',
        denseBundleMode,
        buildMs: performance.now() - startedAt
    };
    (state as any).focusFrameDiagnostics = {
        lastFrameAt: 0,
        sampleCount: 0,
        avgFrameMs: 0,
        maxFrameMs: 0
    ...
    focusLineParent.add((state as any).focusSemanticLines);
}

export function updateFocusSemanticOverlayPositions(now: number = performance.now()): void {
    const line = (state as any).focusSemanticLines;
    const pairs = (state as any).focusSemanticConnectionPairs || [];
...
export function getSemanticFocusCueProbeSnapshot(): Record<string, unknown> {
    return {
        visible: !!(state as any).focusSemanticLines && !!(state as any).focusThreadDiagnostics?.active,
        threadSource: (state as any).navState.threadSource || null,
        focusedIndex: Number.isFinite((state as any).navState.focusedIndex) ? (state as any).navState.focusedIndex : null,
        nextIndex: Number.isFinite((state as any).focusSemanticLines?.userData?.nextIndex) ? (state as any).focusSemanticLines.userData.nextIndex : null,
        lineNextIndex: Number.isFinite((state as any).focusSemanticLines?.userData?.nextIndex) ? (state as any).focusSemanticLines.userData.nextIndex : null,
        nextCueSegments: (state as any).focusSemanticLines?.userData?.nextCueSegments || (state as any).focusThreadDiagnostics?.nextCueSegments || 0,
        focusThreadSegments: getLineSegmentCount((state as any).focusSemanticLines),
        threadDiagnostics: { ...((state as any).focusThreadDiagnostics || {}) }
    };
}
