 ...
 * ui-feedback.ts
 *
 * TypeScript shadow for ui-feedback.js
...
export function syncSearchStatusForFocus(point: Point, options: SyncSearchStatusOptions = {}): void {
    ...
    const resultIndices = Array.isArray((getCurrentSearchSummary() as Record<string, unknown>).resultIndices)
    ...
    const queryLabel = (getCurrentSearchSummary() as Record<string, unknown>).query
        ? `"${(getCurrentSearchSummary() as Record<string, unknown>).query}"`
