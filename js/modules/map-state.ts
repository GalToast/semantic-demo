    ...
    try {
        ...
        state.points.forEach((point: Point, index: number) => {
            if (pointHasGeocode(point)) {
                const color = (state.COLORS as readonly string[])[
                    (point.cluster ?? 0) % (state.COLORS as readonly string[]).length
                ]
                ...
                marker.on('mouseover', () =>
                    showMapTooltip(point, marker as unknown as { bindTooltip: Function; openTooltip: Function })
                )
                marker.on('mouseout', () => {
...
export function getRouteEmbodimentIndices(): number[] {
    ...
    const pushIndex = (index: number): void => {
        ...
        if (
            ...
            index < 0 ||
            index >= points.length ||
            !isPointVisible(index, points, null, state.activeFilters)
        )
            return
