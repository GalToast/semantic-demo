...
export const SCENE_ATMOSPHERE = Object.freeze({
    ...
    // instances do not alpha-saturate the field center to a solid pale blob.
    // 0.05 keeps ~40% per-pixel cumulative alpha at the field's dense core
    // (10-ish overlapping spores) so the cluster reads as a soft cyan-gray
    // haze, not a saturated white disc.
    sporeOpacity: 0.05
})
...
export function setNodeSporeInstanceMatrix(
    index: number,
    targetMesh: THREE.InstancedMesh | null = webglContext.nodeSporeMesh,
    scaleMultiplier = 1
) {
...
export function createNodeSporeLayer() {
    ...
    const sporeMat = new THREE.MeshPhongMaterial({
        // Off-white cool tint (not pure white) so the cumulative color of many
        // overlapping transparent spores stays in the cool palette instead of
        // desaturating toward a flat white blob under ACES tone mapping.
        color: 0xc8d4d0,
        emissive: 0x16453f,
    ...
    // 0.85 (was 1.62) keeps every per-instance color channel ≤ 1.0 so ACES
    // tone mapping cannot desaturate the high-luminance cluster into a flat
    // pale white blob at the field's geometric center.
