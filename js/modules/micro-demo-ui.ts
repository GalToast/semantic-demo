/**
 * micro-demo-ui.ts — TypeScript shadow of micro-demo-ui.js
 * DOM UI helpers for the micro-demo.
 */
...
export function showVeil(active: boolean): void {
    ...
    if (!veil) return;
    if (active) {
        void (veil as HTMLElement).offsetWidth;
...
export function bindInputInterceptor(onCancel?: (reason: string) => void): void {
    ...
    document.addEventListener('keydown', onKeydown, { once: false, capture: true });
...
export function showPill(text: string, onSkip?: (reason: string) => void): HTMLDivElement {
...
export function showEndToast(): HTMLDivElement {
    removePill();
    ...
    (toast as any)._autoRemove = autoRemove;
    return toast;
}
