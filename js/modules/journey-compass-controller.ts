...
import { exploreInsideToNextStop } from '@lib/orchestration/lifecycle'
import { resetExplorationFocus, setTrailDepth } from '@lib/stores/lifecycle'
...
export function syncJourneyCompassActions(compassState: any = {}): void {
    ...
    buttons.forEach(([button, action, role]) => {
        ...
        const disabled =
            !action?.action ||
            (action.action === JOURNEY_ACTIONS.NEXT_STOP && appState.strandContinuityState?.phase === 'exploring')
...
export function syncMapTrailStrip(compassState: any = {}, presentationState: any = {}): void {
    ...
    if (!strip) return
    const shouldShow = appState.currentView === 'map' && presentationState.navigationOwner === 'map-trail-strip'
...
export function updateJourneyCompass(): void {
    ...
    if (title) {
        ...
        if (visibleTitle) {
            ...
            const focusedName = focusedPoint
                ? formatBusinessName(focusedPoint.name || 'this business')
                : 'Focused business'
    ...
    const order = [
        ...(CONFIG.JOURNEY_COMPASS_PHASE_ORDER || ['overview', 'search', 'focus', 'inside', 'map'])
    ] as string[]
    ...
    compass.querySelectorAll('[data-journey-step]').forEach((step: Element) => {
        ...
        stepEl.setAttribute(
            'aria-label',
            `${stepIndex + 1}. ${capitalize(stepEl.dataset.journeyStep || '')}: ${description}`
        )
...
export function getViewHandoffModel(view: string): any {
    ...
    const searchLabel = hasSearch
        ? cleanPublicNoteText(
              (appState.currentSearchSummary as any).query ||
                  (appState.currentSearchSummary as any).label ||
                  'current trail'
          )
        : ''
    ...
    if (view === 'map') {
        ...
        const origin =
            appState.terrainHandoffState?.from ||
            (typeof getRouteLayerOrigin === 'function' ? getRouteLayerOrigin() : 'galaxy')
