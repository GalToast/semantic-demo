...
import type { Point, StrandContinuityState } from '../../types/state.js';
...
function shouldSuppressSelectedBusinessNeighborRail(): boolean {
    ...
    const threadSurface: string | undefined = (document.body as any)?.dataset?.threadInspectSurface;
    if (threadSurface && threadSurface !== 'idle') return false;
    const shortLandscapeFocusViewport: boolean = typeof window !== 'undefined' &&
        typeof window.matchMedia === 'function' &&
...
export function updateFocusNeighborRail(): void {
    ...
    if (!Number.isFinite(getNavState()?.focusedIndex) || hasColdDegradedSemanticFallback()) {
        ...
        list.innerHTML = '';
        if (countEl) countEl.textContent = '0 visible neighbors';
        clearThreadInspection({ force: true, preserveJourney: ['exploring', 'arrived'].includes((getStrandContinuityState() as StrandContinuityState).phase) });
        return;
    }
    ...
    const candidates = (nav.threadCandidates || [])
        ...
        .filter((candidate: any) => isPointVisible(candidate.index, getPoints()!, null, getActiveFilters()))
    ...
    candidates.forEach((candidate: any, order: number) => {
        ...
        const point: Point | null = (Number.isFinite(candidate.index) && candidate.index >= 0 && candidate.index < points.length)
        ...
        const focusPoint: Point | null = (Number.isFinite(focusIdx) && focusIdx! >= 0 && focusIdx! < points.length) ? (points[focusIdx!] as Point) : null;
    ...
    list.querySelectorAll('[data-index]').forEach((button: Element) => {
        ...
        const walkToIndex = () => {
            ...
            const nextIndex = Number(btn.dataset.index);
            if (!Number.isFinite(nextIndex)) return;
            walkThreadNeighbor(nextIndex, { surface: 'rail', reason: btn.dataset.reason || 'nearby business relationship' });
        };
        const inspectIndex = () => {
            cancelHoverIntent();
            const nextIndex = Number(btn.dataset.index);
            ...
            setStrandContinuityState('preview', {
                ...
                fromIndex: getNavState()?.focusedIndex ?? null,
                reason: 'rail-inspect'
            });
...
function updateWalkBreadcrumb(hasFocus: boolean = false): void {
    ...
    const points = getPoints()!;
    const history: number[] = ((getNavState()?.walkHistoryIndices || []) as number[])
        .filter((index: number) => Number.isFinite(index) && points[index])
        .filter((index: number, order: number, list: number[]) => list.indexOf(index) === order || order === list.length - 1);
    ...
    breadcrumb.innerHTML = `
        ...
        ${history.map((index: number, order: number) => {
...
export function updateTraversalUi(): void {
    ...
    const currentCandidate = nav.mode === 'trail' && nav.trailCursor >= 0 && neighborCount > 0
        ? nav.threadCandidates[nav.trailCursor]
        : null;
    const sourceLabel: string = nav.threadSource === 'semantic'
        ? 'semantic thread'
    ...
    const nextWalkReason: string = nextWalkCandidate ? summarizeNeighborReason(nextWalkCandidate, nextWalkPoint, currentFocusPoint) : '';
    if (focusRouteEl) focusRouteEl.dataset.state = neighborCount ? (nav.mode === 'trail' ? 'walking' : 'ready') : 'empty';

    if (coldDegradedNoRail) {
        const queryLabel: string = (getSemanticLaneSnapshot() as any)?.query ? `"${(getSemanticLaneSnapshot() as any).query}"` : 'this semantic trail';
    ...
    if ((getTrailDepth() ?? 0) >= 1 && (nav.walkHistoryIndices || []).length >= 0) {
        ...
        const pocketNote: string = nav.threadSource === 'semantic' && focusPocketMeta?.active
