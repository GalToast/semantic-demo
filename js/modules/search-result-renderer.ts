...
export function buildSearchRankLabel({ index, order, topIndex, anchorIndex = null, exploreIndex = null }: SearchRankParams): string {
...
export function buildSearchStageLabel(
    ...
    if (Number.isFinite(topIndex) && topIndex! >= 0 && topIndex! < (state.points as SearchResultPoint[]).length && (state.points as SearchResultPoint[])[topIndex!]?.cluster === point.cluster) return 'Same theme';
    return 'Related match';
}

// ── DOM UPDATERS ───────────────────────────────────────────────────────────

export function revealActiveSearchResultOnCompact(resultsEl: HTMLElement, activeRow: HTMLElement | null = null): boolean {
...
export function setActiveSearchResultRow(
    ...
    const effectiveIndex = focusIsOutsideSearchTrail
        ? null
        : isCommittedExplore && Number.isFinite(navState?.focusedIndex) ? navState!.focusedIndex as number : activeIndex;
    ...
    if (reveal && activeRow) {
        const searchState = (resultsEl as unknown as Record<string, unknown>)._searchStateNamespace as Record<string, unknown> | undefined;
