 ...
 * semantic-lane.ts
 *
 * Typed sibling of semantic-lane.js.
...
interface LaneProbeOptions {
    warm?: boolean;
    reason?: string;
}

interface LaneUiOptions {
    label?: string;
    title?: string;
}
...
export function initSemanticLaneAdapter({ updateLegendGuideState }: { updateLegendGuideState?: () => void } = {}): void {
...
export async function fetchSemanticLaneHealth({ warm = false, signal = null }: { warm?: boolean; signal?: AbortSignal | null } = {}): Promise<LaneHealthPayload> {
    ...
    if (detectStaticDevPHP(responseText) && allowsStaticDevFallback()) {
        ...
        const error: Error & { correlationId?: string } = new Error('Semantic search readiness check returned raw PHP source.');
        ...
        try {
            ...
            Object.defineProperty(error as object, 'correlationId', { value: crypto.randomUUID(), writable: false, configurable: true });
            throw new Error('Semantic search readiness check returned invalid JSON.', { cause: error });
        }
    }

    if (!response.ok || !payload?.ok) {
        const err: Error & { correlationId?: string } = new Error(payload?.error || 'Semantic search readiness check failed.');
...
export function applySemanticLaneHealthPayload(payload: LaneHealthPayload | null | undefined, options: LaneUiOptions = {}): void {
    ...
    if (state.semanticLaneOpsMode) {
        refreshSemanticLaneOpsSummary().catch((err: unknown) => console.warn('refreshSemanticLaneOpsSummary failed:', err));
    }
...
export async function probeSemanticLane({ warm = false, reason = 'interval' }: LaneProbeOptions = {}): Promise<LaneHealthPayload | null> {
    const win = getWindow();
