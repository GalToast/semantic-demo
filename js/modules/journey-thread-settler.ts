...
let _setTimer: (fn: () => void, delay: number) => ReturnType<typeof setTimeout> | undefined = (fn, delay) =>
    typeof setTimeout !== 'undefined' ? setTimeout(fn, delay) : undefined
let _clearTimer: (id: ReturnType<typeof setTimeout>) => void = (id) =>
    typeof clearTimeout !== 'undefined' ? clearTimeout(id) : undefined
...
export function initJourneyTimerAdapter(
    deps: { setTimer?: typeof _setTimer; clearTimer?: typeof _clearTimer } = {}
): void {
...
export function getInsideRelationshipLabel(
    ...
    const sameCity: boolean =
        ...
        !!(point && focusPoint && normalizeCityForFilter(point.city!) === normalizeCityForFilter(focusPoint.city!))
...
export function summarizeNeighborReason(
    ...
    const roleLabel: string = candidate.relationshipRole
        ? getRelationshipRoleLabel(candidate.relationshipRole, 'title')
        : ''
    ...
    if (reason) {
        const normalizedReason = reason
            ...
            .replace(/\bsame business sector\b/i, sharedTopic ? `same ${sharedTopic} topic` : 'nearby business sector')
            .replace(
...
export function walkThreadNeighbor(
    index: number,
    options: WalkThreadNeighborOptions = {}
): { targetIndex: number; fromIndex: number | null; reason: string } | null {
    if (!Number.isFinite(index)) return null
    const fromIndex: number | null = Number.isFinite(options.fromIndex)
        ? options.fromIndex!
        : getCurrentTrailFocusIndex()
    const candidate = ((state.navState as any).threadCandidates || []).find((item: any) => item && item.index === index)
    const targetPoint: Point | null =
        Number.isFinite(index) && index >= 0 && index < (state.points as any).length
    ...
    const reason: string =
        summarizeNeighborReason(
            candidate || {},
            targetPoint,
            Number.isFinite(fromIndex) && fromIndex! >= 0 && fromIndex! < (state.points as any).length
                ? (state.points as any)[fromIndex!]
                : null
        ) ||
        candidate?.reason ||
        options.reason ||
    ...
    const arrivalTid = timerAdapter.setTimer(() => {
        ...
        if (
            (state.strandContinuityState as StrandContinuityState).phase === 'exploring' &&
            (state.strandContinuityState as StrandContinuityState).targetIndex === capturedIndex
        ) {
            ...
            const pointAtArrival: Point | null =
                Number.isFinite(capturedIndex) && capturedIndex >= 0 && capturedIndex < (state.points as any).length
                    ? (state.points as any)[capturedIndex]
                    : null
    ...
    const settleTid = timerAdapter.setTimer(() => {
        if (!state.points) return
        if (
            (state.strandContinuityState as StrandContinuityState).phase === 'arrived' &&
            (state.strandContinuityState as StrandContinuityState).targetIndex === capturedIndex
        ) {
            _clearTrackedTimer('settle')
            clearStrandContinuityState('arrival-settled')
            const pointAtSettle: Point | null =
                Number.isFinite(capturedIndex) && capturedIndex >= 0 && capturedIndex < (state.points as any).length
                    ? (state.points as any)[capturedIndex]
                    : null
