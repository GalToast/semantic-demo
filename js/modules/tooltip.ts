...
export function updateTooltipContent(point: Point): void {
    ...
    if (triviaEl) {
        triviaEl.textContent = sanitizePublicFacingNote(point.trivia || point.public_note || point.public_detail || '')
...
export function initTooltipEventBusSubscriptions(): void {
    ...
    subscribeKeyed('tooltip:position-requested', EVENTS.TOOLTIP_POSITION_REQUESTED, ({ x, y }) => positionTooltip(x as number, y as number));
    subscribeKeyed('tooltip:content-update-requested', EVENTS.TOOLTIP_CONTENT_UPDATE_REQUESTED, ({ point }) => updateTooltipContent(point as Point));
