...
import { getSemanticCandidateSlice, type NeighborhoodPersonality, type SemanticCandidate } from './focus-pocket-personality.js';
...
export function getFocusConstellationMotifForPersonality(index: number, personality: NeighborhoodPersonality | null): ConstellationMotif {
...
export function getFocusBeaconDeclutterProfile(viewportProfile: Record<string, unknown> = {}): Record<string, unknown> {
    ...
    return {
        ...
        pulseOpacityScale: Number.isFinite(viewportProfile.pulseOpacityScale as number) ? viewportProfile.pulseOpacityScale : 1,
...
export function applyRelationshipRolePlacementBias(placement: PlacementParams, relationshipRole: string, order: number, group: string): PlacementParams {
...
export function getFocusThreadCurvePoint(edge: ThreadEdge, t: number): THREE.Vector3 {
    ...
    const bend = Math.min(bendCap, Math.max(bendFloor, spanLength * (edge.curveLift as number) * roleLift * (1 + longArc * 0.72)));
...
export function buildFocusedPocketStagedPositions(index: number, pocketEntries: Map<number, PocketEntry>): PocketStagedResult {
    ...
    return { positions: pocketPositions, motion, roles, motif, viewportProfile: null };
}
...
export function buildFocusedSemanticPocket(index: number): SemanticPocketResult | null {
    ...
    const supportScores = new Map<number, { count: number; score: number; sameCity: number; relationshipRole: string; relationshipAxis: string; roleReason: string }>();
    primaryCandidates.slice(0, vpProfile.supportSeedLimit).forEach((candidate: SemanticCandidate) => {
        getSemanticCandidateSlice(candidate.index, vpProfile.supportNeighborLimit).forEach((support: SemanticCandidate) => {
