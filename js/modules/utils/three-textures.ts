 ...
 *
 * Accepts the THREE namespace as a parameter so this module can be
 * unit-tested without a live WebGL context.
 */

import type { CanvasTexture } from 'three';

export function createSporeTexture(THREE: typeof import('three')): CanvasTexture {
...
export function createFocusRingTexture(THREE: typeof import('three')): CanvasTexture {
    const canvas = document.createElement('canvas');
    canvas.width = 96;
    canvas.height = 96;
    const ctx = canvas.getContext('2d')!;
...
export function createFocusNextCueTexture(THREE: typeof import('three')): CanvasTexture {
    const canvas = document.createElement('canvas');
    canvas.width = 128;
    canvas.height = 128;
    const ctx = canvas.getContext('2d')!;
