...
export type DomChild = Node | string | number | boolean | null | undefined | unknown[];
...
export type DomAttributes = Record<string, string | number | boolean | DomEventHandler | Record<string, unknown> | null | undefined>;

const SVG_TAGS = new Set(['svg', 'path', 'circle', 'rect', 'line', 'polyline', 'polygon', 'ellipse', 'text', 'g', 'use']);
 ...
 */
export function el(tag: string, attributes: DomAttributes = {}, ...children: DomChild[]): HTMLElement | SVGElement {
    ...
    const appendChild = (child: DomChild): void => {
        ...
        if (isNodeLike(child)) {
            ...
            child.forEach((nestedChild) => appendChild(nestedChild as DomChild));
        } else {
