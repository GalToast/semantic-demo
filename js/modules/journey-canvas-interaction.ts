...
} from './journey-canvas-hit-test.js';
...
export function ensureCanvasNodeInteractionBindings(): void {
    ...
    const walkCanvasThreadFromPointerEvent = (event: PointerEvent | MouseEvent): boolean => {
        if ((state as any).currentView !== 'galaxy' || !Number.isFinite((state as any).navState.focusedIndex)) return false;
        ...
        if (!candidate && (document.body.dataset as any).threadInspectSurface === 'canvas' && Number.isFinite((state as any).inspectedThreadIndex)) {
    ...
    if ((document.documentElement.dataset as any).canvasHoverDocumentClearBound !== 'true') {
        ...
        document.addEventListener('pointermove', (event: PointerEvent) => {
            ...
            if (!activeCanvas || event.target === activeCanvas || activeCanvas.contains(event.target as Node | null)) return;
