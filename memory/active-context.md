...
|---|---|---|---|
| **S1** | `src/lib/ui/loading.ts` (3 TODOs) | Port weather + thread hydration flow into the Svelte loading pipeline | Worker ready, launching |
| **S2** | `src/lib/orchestration/url-state.ts:412` | Switch URL-state to direct Svelte filter store mutations | Prompt drafted, ready to fire |
| **S3** | `src/lib/orchestration/view-controller.ts:293,296` + `src/App.svelte:389` + `src/lib/orchestration/url-state.ts:537` | Toast notifications + semantic-guide icon (4 TODOs) | Prompt drafted, ready to fire |
| **S4** | `js/modules/legend-ui.ts:287` + `js/modules/tooltip.ts:149` | Move legacy call sites to Svelte component lifecycle | Prompt drafted, ready to fire |
| **S5** | `tests/unit-active/todo-without-ticket-invariant.test.ts` + `memory/active-context.md` + `docs/both-pattern-follow-ups-2026-06-13.md` | Drop baseline to 0; close-out | Prompt drafted, ready to fire |
...
- **51+ commits this session** (target: 56+ after S6 arc lands), all pushed to origin. Wave included the BOTH-pattern follow-ups (1+2, 3, 4, 5, 6, 8), Wave 9 (9A, 9B, 9C, 9D, 9E), the A worker (9D-Option-B), 5 invariant tests, the S6-arc prompt drafting, and the docs close-out + retirement publication.

## Next session entry (post-A, pre-S1)
...
3. **S6 arc ready to fire.** 5 S-tickets drafted. S1 is the largest (1 h); fire it first. Then S2 and S4 in parallel (independent files, 30-60 min each). Then S3 (depends on S2, 1-2 h). Then S5 (15 min, main lane).
4. **After S6:** Wave 10 (legacy runtime retirement) is the next big arc. The legacy `js/modules/*` tree is the only remaining retirement work.
