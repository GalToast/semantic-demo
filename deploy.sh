...
run "ssh -p $PORT $SSH_TARGET 'mkdir -p \"$BACKUP_DIR/dist\" \"$BACKUP_DIR/js\" \"$BACKUP_DIR/css\" \"$BACKUP_DIR/scripts\" && cp -p \"$REMOTE_DIR/dist/bundle.js\" \"$BACKUP_DIR/dist/bundle.js\" && cp -p \"$REMOTE_DIR/semantic-demo.css\" \"$BACKUP_DIR/semantic-demo.css\" && cp -p \"$REMOTE_DIR/vector-explorer-pandora.css\" \"$BACKUP_DIR/vector-explorer-pandora.css\" && if [ -d \"$REMOTE_DIR/css\" ]; then cp -p \"$REMOTE_DIR/css/\"*.css \"$BACKUP_DIR/css/\" 2>/dev/null || true; fi && cp -p \"$REMOTE_DIR/vector-explorer-polished.html\" \"$BACKUP_DIR/vector-explorer-polished.html\" && cp -p \"$REMOTE_DIR/.htaccess\" \"$BACKUP_DIR/.htaccess\" && cp -p \"$REMOTE_DIR/data.dat\" \"$BACKUP_DIR/data.dat\" 2>/dev/null && cp -p \"$REMOTE_DIR/scripts/leadEnrichment.public.json\" \"$BACKUP_DIR/scripts/leadEnrichment.public.json\" 2>/dev/null; cp -p \"$REMOTE_DIR/js/scanner.js\" \"$BACKUP_DIR/js/scanner.js\" 2>/dev/null; cp -p \"$REMOTE_ROOT/js/scanner.js\" \"$BACKUP_DIR/scanner-root.js\" 2>/dev/null; true'"
