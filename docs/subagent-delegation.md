...
4. **Delegate** — call `external_subagent_free_models` first, then pass an exact provider-qualified `launch_ref` to `external_subagent_start`. Provide: scope, allowed files, no-revert boundaries, expected evidence (diff + verification output + tmp/report.md), verification commands. Worker timeouts have a 30-minute minimum (`timeout_seconds` values below 1800 are clamped). `mode: yolo`. `mcp_profile: subagent`. `owner_tag: kimi-main`. Acquire session lock if multi-commit. For progressive MCP research, also pass a scoped `mcp_config_path` containing only the needed server (for example websearch); after the broker refresh, Pi workers can discover `mcp__websearch__web_search` through `tool_search` without loading browser MCPs.
    - **Pre-fleet route-probe launch-gate (lf2 proposal, 2026-08-15):** after pre-warm, fire ONE cheap one-shot probe (a trivial completion+tool-call worker, ≤4 tool-calls, writes one marker file) on the chosen model/route BEFORE fanning N workers — a dead route (200-but-zero-tokens, e.g. hy3:free 2026-08-15; cold spawn 404s 2026-08-13) costs a whole round; a 30s probe costs nothing. Falls back cleanly: probe FAIL → pick a proven lane (poolside/laguna, agnes-2.5-flash, logfare/glm-5.2 once certified) or repair the route.
    - **Pre-warm the catalog before EVERY dispatch round (fixed 2026-08-14):** run `bash scripts/prewarm-catalog.sh` right before the first `external_subagent_start` of a round. Workers spawn a fresh Pi CLI that resolves `--model` against the async-pivoted pi-model-providers extension; a cold spawn can die instantly with `Error: Model "router-.../..." not found` even though `pi --list-models` lists the model (hit all routes on 2026-08-13). The pre-warm forces extension registration to complete in a fresh process; verified to make worker spawns boot normally.
