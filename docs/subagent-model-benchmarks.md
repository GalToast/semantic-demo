...
| -------------------- | ---------------- | --------- | --------------------------------------------------------------------------------------------- |
| direct-freeinference | `kimi-k2.7-code` | ⚠️ Cache-miss / auto-fallback 2026-07-29 | 2026-07-23: `Provider finish_reason: unexpected_state`. **2026-07-29 03:34Z**: main-lane session observed `cacheRead: 0` on every turn for ~5 turns (Harness auto-fell-back to `minimax-m3` after the kimi turn aborted with `stopReason: aborted, usage: all zeros`). Earlier today (~00:00-00:02Z) kimi-k2.7-code was hitting **99.4–99.7% cache** (PID 19120: 129,664 / 130,419 → 130,240 / 130,640 → 130,432 / 147,132 → 146,560 / 147,442). The current 0-cache symptom is transient + correlated with the upstream `Request aborted` failure (not a real cache miss). Re-test in 15–30 min before re-promoting to main lane. |
