...
| -------------------- | ---------------- | ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| direct-freeinference | `kimi-k2.7-code` | ❌ Cache-disabled upstream (model alive, prompt cache off)        | 2026-07-23: `Provider finish_reason: unexpected_state`. **2026-07-29 ~03:30Z**: main-lane session observed `cacheRead: 0` on every turn. Harness auto-fell-back to `minimax-m3` after the kimi turn aborted with `stopReason: aborted, usage: all zeros`. **2026-07-29 root-cause verified via authenticated direct probe** (`Authorization: Bearer $FREEINFERENCE_API_KEY`, POST <https://freeinference.org/v1/chat/completions>): the model IS alive (HTTP 200, content="OK", TTFT 1.4–7.8 s) BUT **`cached_tokens: 0` on every turn** across 3-turn probes with ~10K-token static prefix. **Cross-model comparison on the same upstream at the same minute**: `minimax-m3` (cached: 1.6% → **99.5% → 99.2%**), `deepseek-v4-flash` (cached: 0% → **97.7% → 99.7%**), `glm-5.1` (cached: 0% → **99.3% → 99.0%**). **`kimi-k2.7-code` is the only model on the upstream that doesn't cache.** Probe script: `tmp/freeinference-cache-probe.mjs`. **Conclusion**: the model is alive and responds, but the upstream has specifically disabled prompt-cache for kimi-k2.7-code (possibly retired the cache layer for this slug, possibly reduced a quota, possibly a partial retirement). Cost impact: kimi prompt tokens now bill at full input rate (no cache_read discount). Earlier today (~00:00-00:02Z) kimi-k2.7-code WAS hitting 99.4–99.7% cache (PID 19120: 129,664 / 130,419 → 130,240 / 130,640 → 130,432 / 147,132 → 146,560 / 147,442). The cache disablement happened sometime between 00:02Z and 03:30Z. **Fix**: repoint the `direct-freeinference` main lane to `minimax-m3` in `~/.pi/agent/model-providers.json` (or use opencode-zen/minimax-m3 / nvidia/minimax-m3 which are healthier routes). |
| direct-freeinference | `minimax-m3`     | ✅ Stable 2026-07-29 (auto-fallback after kimi-k2.7-code aborted) | `cacheRead: 92288 / totalTokens: 97143` (~95% cache hit) at 03:34:46Z. 1M context window, 512K max tokens. compat.thinkingFormat = `minimax` (Pi harness `dist/api/openai-completions.js` strips the `<think>`-tagged duplicate thinking block per 2026-07-22 patch). Upstream ALWAYS returns thinking in both `content` (wrapped `<think>` tags) AND the `reasoning` field — local patch is required; without it, use opencode-zen/minimax-m3 or nvidia/minimaxai/minimax-m3 to avoid duplication.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
### W56 vision faceoff: inkling vs minimax-m3 (nvidia route) — 2026-07-29

**Setup**: same neutral image-diff prompt, 2 screenshots (map-mode before/after W56 fixes), max reasoning, 360s timeout, one-shot. Config fixed first: `nvidia/thinkingmachines/inkling` `supportsVision: true` (was undefined); `nvidia/minimaxai/minimax-m3` reasoning effort medium→max (user wants max even if slow).
...
Proven during the faceoff: `external_subagent_poll` reported **both** workers identically as `status: running, output_state: logs_only, rate_limited: false, quiet_for_seconds: N` while they were doing completely different things — inkling was **failing + retrying** (5× Connection error), minimax was **deep in thought** (414 thinking deltas). A failing-and-retrying worker was visually identical to a deep-thinking worker.

Three concrete gaps:

1. **No `retry_count` / `last_error` / `provider_health` in poll** — the silent retry loop was invisible.
2. **`thinking_seen` unreliable** — poll reported `thinking_seen: false` for inkling despite 10+ `thinking_delta` in the raw log.
3. **No per-attempt timeline** — can't separate harness/upload time from provider latency; the failure was before first byte with no structured event.

**Mitigation shipped**: `scripts/worker-health.mjs` — reads the worker's raw stdout log and surfaces `connErr / retry_count / last_error / thinking-vs-text deltas / TTFT / final usage` + a collapsed VERDICT. Collapses the diagnosis loop from "poll 4× over 3 min wondering if it's thinking" → "one glance: it's failing, check the router / relaunch". Reusable across sessions; documented in the `subagent-worker-health-forensics` project skill. Full report: `reports/w56-vision-faceoff-2026-07-29.md`.

**W56 closeout**:

- ✅ Map-mode header overlap fixed (`css/journey_active.css` sr-only under `body.surface-map`); vision-confirmed.
- ✅ Legend scrollbar selector mismatch fixed (`src/components/Legend.svelte` styled scrollbar on `.legend`); vision-confirmed.
