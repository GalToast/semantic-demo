...
- **Cross-check script:** `tmp/logfare-crosscheck.cjs` — catalog vs opencode
  ...
  after any model add/remove.
    ...
    **Side note on the same probe:** `gemma-4-26b` at `max_tokens:64` returned
    ...
    same reasoning-starvation pattern as the original `max_tokens:10` false
    positive. At `max_tokens:131072` it returns `finish=stop` cleanly. A
    reasoning model must never be probed with a tiny max_tokens.
