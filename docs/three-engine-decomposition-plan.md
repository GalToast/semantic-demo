...
>
> - 2026-07-04 — **A6+A8+A9+A10+A13 wave** extracted from `animate()` (post-A5 core, L409–508) into five helpers in `three-engine-frame-updates.ts`: `lerpCameraForReveal()` (A6, scene-reveal camera lerp + completion cleanup), `updateFogDensity()` (A8, fog density scale), `updateReferenceSphereOpacity()` (A9, ref-sphere sin boost), `updateSporeOpacity()` (A10, spore material lerp), `updateThreadLayerOpacities()` (A13, per-layer thread opacity). Import cleanup cascade: `Material`/`FogExp2` (three) and `PORT_SCENE_ATMOSPHERE`/`getThreadPulseOpacityPort`/`getMyceliumPresentationProfilePort` removed from `three-engine-core.ts`; `FogExp2`/`Material` added to `three-engine-frame-updates.ts`; `getThreadPulseOpacityPort`/`getMyceliumPresentationProfilePort` added there too. Contract updates: `motion-state-contract.mjs` and `scene-atmosphere-contract.mjs` updated to read the extracted patterns from `three-engine-frame-updates.ts` instead of `three-engine-core.ts`. `animate()` now ~178 LOC (down from ~260 pre-wave). Verified: build clean (18.72s), core contract group 11/11, cancel-animate/three-setup-loop/motion-state/focus-pocket-motion/three-setup-init dewindowing + scene-atmosphere/three-visual-polish/residual-window-bridge/js-reduced-motion-guard contracts all green, `three-engine-animate-regression`/`three-engine-core`/`three-engine-frame-updates` unit tests 66/66. The `reduced-motion-coverage-contract` failure on `Filters.svelte` is pre-existing (parallel UX-2 lane, unrelated to this wave).
> - 2026-08-12 — **Restore ownership arbitration** added to `three-listener-registration.ts` and `app-init.ts`: the live renderer canvas has one owner for the full C5/C6/C7/C10 listener set. A fallback claims a releasable token, engine initialization can replace that fallback, and a duplicate live engine registry returns before attaching any partial listener set. Disposal removes the owned listeners and releases ownership. Focused ownership/restore tests cover fallback-first, engine-first, duplicate-registry, context-lost, context-restored, and disposal paths.
...
| **A6 — Camera reveal lerp** ✅               | 759–782 | 24  | `lerpVectors` for scene-reveal camera path, clear reveal state at completion. **Extracted 2026-07-04 → `lerpCameraForReveal()` in `three-engine-frame-updates.ts`** (see decomposition log).                                              |
| **A7 — Points material update**              | 784–801 | 18  | Compute opacity/size scales from focus/semantic-dive state, update shader uniforms                                                                                                                                                   |
| **A8 — Fog density** ✅                      | 803–807 | 5   | Scale `FogExp2.density` by reveal progress. **Extracted 2026-07-04 → `updateFogDensity()` in `three-engine-frame-updates.ts`** (see decomposition log).                                                                                |
| **A9 — Reference sphere wireframe** ✅       | 809–818 | 10  | Sin-curve opacity boost during reveal. **Extracted 2026-07-04 → `updateReferenceSphereOpacity()` in `three-engine-frame-updates.ts`** (see decomposition log).                                                                       |
| **A10 — Spore material opacity** ✅          | 820–828 | 9   | Lerp `nodeSporeMaterial.opacity` toward target. **Extracted 2026-07-04 → `updateSporeOpacity()` in `three-engine-frame-updates.ts`** (see decomposition log).                                                                          |
...
| **A13 — Thread opacity update** ✅           | 863–891 | 29  | Compute per-layer (core/wispy/bridge) opacity from `getThreadPulseOpacityPort`. **Extracted 2026-07-04 → `updateThreadLayerOpacities()` in `three-engine-frame-updates.ts`** (see decomposition log).                              |
