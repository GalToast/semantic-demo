...
|---|---|---|---|---|
| `data-panel-surface` | `idle`, `search`, `focus`, `focus-search`, `semantic-dive`, `map-trail`, `map-focus`, `map-focus-search`, `map-search` | Primary panel/sheet visibility and layout. All non-map panels use this as the primary gate. Map variants gate map-specific chrome. | `css/layout_base.css`, `css/mobile_premium__*.css`, `css/journey_steps.css`, `css/journey_active.css`, `css/animations.css` | `js/modules/lifecycle.js` (`refreshCompositionState()` / `derivePanelSurface()`) |
| `data-panel-surface-detail` | `none`, `peek`, `expanded` | CSS-facing detail state for mobile search/focus-search drawer density. | `css/mobile_premium__state.css` | `js/modules/lifecycle.js` mirrors `data-mobile-search-sheet`; `js/modules/search-panel-adapter.js` owns the search-sheet interaction write |
| `data-focus-panel-mode` | `field-node`, `overview`, `manual-panel`, `manual-collapsed`, `legend-open` | Focus HUD density and temporary mobile panel submodes. `field-node` controls compact canopy/walk-dock styling; manual and legend values track user-opened panel states. | `css/mobile_premium__focus-dive.css` / `css/mobile_premium__surfaces.css` for active mobile focus composition, active/non-active field-node, and late backstops; `css/journey_active.css` for journey-compass base states; `css/progressive_disclosure.css` for legacy show/hide | `js/modules/lifecycle.js`, `js/modules/event-bindings.js`, QA scripts |
...
`css/mobile_premium__state.css` owns drawer geometry and compact result presentation for these `peek`/`expanded` states. `css/mobile_premium__chrome.css` owns state-agnostic result chrome; supporting modules must not become `data-panel-surface-detail` owners.
...
- Style owner: dedicated selected-map summary presentation lives in `css/mobile_premium__map.css`; selected-card/map-state visibility, final drawer geometry, and suppression backstops live in `css/mobile_premium__surfaces.css`.
...
4. **Changing focus HUD density** -> edit `css/mobile_premium__focus-dive.css` for active field-node/focus-dive composition and `css/mobile_premium__surfaces.css` for fallback backstops; edit `css/journey_active.css` only for journey-compass base states. Do not invent a new `data-focus-panel-mode` value without a corresponding JS writer and contract update.
