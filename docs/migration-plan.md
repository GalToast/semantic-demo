...
| **Compat shims**  | `legacy-state.ts` + `legacy-state-adapter.ts` are **durable compat seams** (23 + 116 LOC, 9 + 6 importers respectively). `three-micro-demo-bridge.ts` is under audit; treat as load-bearing until deletion decision documented.                                                                           |
