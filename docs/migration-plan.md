...
> **Note (2026-08-19, correction):** `src/lib/state/legacy-state.ts` and `src/lib/state/legacy-state-adapter.ts` were **deleted** in `062b0035` (P1-F LegacyState → AppState reconciliation, 2026-07-23). State readers now route through `src/lib/state/app.svelte.ts` (source of truth, tracked). The active compat seam is **not** legacy-state — it is the `appState ↔ svelteState` dual-state mirror in `src/lib/state/create-state-mirror.ts` (each Svelte store `Writable` is mirrored back into `appState` via `getCompatSources()` def `src/main.ts:292`, with `window.__LEGACY_APP_STATE__` set in `src/lib/engine/lifecycle.ts:418` (appState) and `src/lib/engine/three-engine-state.ts:195` (legacyState)). This is the **#1 load-bearing scaffold — do not retire** (see `AGENTS.md` invariant).

> `src/lib/engine/three-micro-demo-bridge.ts` was **retired as a no-op in W52 (`4da700a0`) but restored (`be9d4f42`)** as a deprecated no-op seam still referenced by `three-interaction-visuals.ts`'s barrel contract — it is **not** deleted. _(Stale comment at `three-interaction-visuals.ts:610` still reads "was deleted" — see correction item A2.)_

---

## TL;DR

| Dimension         | Current state (post-W48)                                                                                                                                                                                                                                                                                                                                          |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Artifact                                                                                                                         | Status                        | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Legacy islands (`selected-details-svelte-island.{ts,js}`, `search-results-svelte-island.{ts,js}`, `island-mount-helper.{ts,js}`) | **Archive-only (2026-06-29)** | Last revert on 2026-06-12 (`ec520da`) restored them; subsequent scans on 2026-06-29 (`find src/`) return zero matches. The only surviving on-disk copies live under `docs/archive/legacy-reference/island-mount-helper.ts`. No `src/` importer, no `tests/` importer. Treat as **archive-only reference**, not live code; a clean-up PR can sweep them entirely. Before doing so, apply the 4-signal bridge rule: (1) runtime import, (2) docs/tests/legacy-reference reference, (3) public type export consumers, (4) commits in last 60 days. Historical design rationale for the BOTH pattern lives at `docs/archive/both-pattern-exit-criteria.md` and `docs/archive/agents-full-reference-2026-06-19.md`. |
...
> **Note:** `docs/archive/bridge-load-bearing-2026-06-18.md` is a historical audit input. The living bridge status is Phase 7 complete with 0 remaining `src/lib/engine/*-bridge.ts` files.
...
**Reference:** `docs/archive/phase-7-state-bridge-retirement-2026-06-20.md`
...
**2026-08-10 closeout:** `src/lib/stores/lifecycle.ts` no longer writes
`legacyState.navState` or the legacy `trailDepth` slot during composition.
Navigation mutations now remain behind `updateNavState()` /
`writeNavStateMirror()`; `npm run lint:nav-mirror` is the enforcement gate.
The browser-level back/forward proof lives in
`tests/widget-journey-keyboard-history.spec.js`.

### 4. Prod-Preview Parity Smoke

**Status:** ✅ Complete (W9-A, 2026-06-20). W8 Bridge Retirement preserved parity. Refresh baseline: `docs/archive/production-preview-parity-baseline-w9-2026-06-20.md`. Smoke contract lives in `tests/production-preview-parity-contract.mjs` (registered under `smoke` group in `tests/contracts.manifest.json`).
