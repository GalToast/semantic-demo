...
> - **§1 #6 (Placeholder2D `PlaceholderCategoryLegend` extraction) — DONE via `fe08db71 refactor(placeholder): extract PlaceholderCategoryLegend from Placeholder2D`** (2026-07-23). Landed as part of the parallel session Wave-3 presentational-extract cycle.
> - **§1 #7 (MapView status/back extraction) — DONE via `bb8c5a78 refactor(mapview): extract MapStatusOverlay and MapBackButton from MapView`** (2026-07-23). The “engine-owner sign-off” + “coverage gap landing” wait conditions were satisfied by these parallel commits + the earlier `cca058bb` MapView/Placeholder2D journey coverage; `src/lib/components/MapStatusOverlay.svelte:9,33` consumes the shared `ErrorState` `variant="map"` (so the §3 cross-cutting seam is closed via the bb8c5a78 indirection). Note: the parallel session shipped `MapStatusOverlay.svelte` under `src/lib/components/` (NOT `src/lib/components/mapview/`), a directory-choice divergence from the search-extract Wave-3 layout (`src/lib/components/search/`).
>
> **All 7 §5 Recommended Execution Order steps LANDED at HEAD `c7e99114` (2026-07-23 verified)** — Wave-1 Big-Components cleanup plan is FULLY CLOSED:
>
> 1. `refactor(header): extract HelpDialog from Header` → `4dde21b7` (W52)
> 2. `refactor(header): extract ModeChipRail from Header` → `2833be6c` (W52)
> 3. `refactor(journey): extract Compass children from JourneyCompass` → `12ae8927` + `2537a84c` (W52 Heads: `CompassHeader` + `CompassActionButton` + `CompassDiveSurface`)
> 4. `refactor(search): extract SearchErrorState + SearchEmptyState + SearchResultList` → `c7e99114` (Wave-3, presentational extract of `SearchResults.svelte` into new `src/lib/components/search/`; main-lane audit verified svelte-extract-scoped-css-reactivity + ErrorState REUSE — see `docs/cleanup-plans/search-layer-cleanup-plan.md` Wave-3 extension ann. block for the full cite)
> 5. `refactor(ui): extract shared ErrorState + RetryButton` → `c7fda649 refactor(state): reconcile LegacyState→AppState in engine helpers + extract ErrorState` (2026-07-20). `src/components/ErrorState.svelte:44` exposes `variant?: 'card' | 'map'`; SearchResults consumes `variant="card"` via the Wave-3 `SearchErrorState.svelte:13,36` wrapper; MapView consumes `variant="map"` via the `bb8c5a78` MapView-extract `MapStatusOverlay.svelte:9,33` wrapper. §3 cross-cutting shared-seam CLOSED.
> 6. `refactor(placeholder): extract PlaceholderCategoryLegend from Placeholder2D` → `fe08db71`
> 7. `refactor(mapview): extract MapStatusOverlay + MapBackButton from MapView` → `bb8c5a78`
>
> Additional post-W52 engine-side hygiene with the same Wave-1 hygiene spirit (NOT §5 byte-listed): `a636addf` (extract `_collectNeighborCandidates` + `_renderNeighborPill`), `a975dc0f` (extract `_trySemanticPocket`), `11c5588c` (extract `_buildOverlayEdges` + `_buildOverlayEdgeArrays`), `a241c2d3` (extract `_buildPocketNode` from `_readFocusSnapshot`). Doc lineage: status banner above now reflects all closures since W52; the W52 superseded note below remains as historical narrative for §1 #1 + §1 #2 (Header + JourneyCompass extracts — those landed in W52) + §1 #3 (SearchResults/MapView shared seam — CLOSED via §5 #4 + §5 #5 above).
