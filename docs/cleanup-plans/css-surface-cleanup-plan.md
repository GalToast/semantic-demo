...
**Current actual blocker:** `npm run check:ownership` fails at `tests/info-panel-surface-ownership-contract.mjs:145` (`SelectedBusinessDetails.svelte must own prefixed #selected-match-panel`). That file is on the parallel session's dirty list and still in-flight. This is a **parallel-session drift issue**, not a CSS ownership baseline issue. Steps 3–5 remain deferred until the parallel session settles AND all contract tests are green.
...
- `npm run test:contract` — includes CSS-touching contract tests: `tests/unit-active/css-important-invariant.test.ts` (merges must not alter `!important` counts), `tests/unit-active/component-SelectedBusinessDetails.test.ts`, `tests/unit-active/legend-display-none-offscreen.test.ts`, `tests/shell-contract-check.js`, `tests/cache-buster-check.js` (link injection order), `tests/sd143-map-search-visual.spec.js`.
    - **ADDITIONALLY (missed in original plan — these gate the merges):** `tests/mobile-chrome-ownership-contract.mjs` (`MOBILE_PREMIUM_SPLIT` hard-codes the 7 shard filenames; update per merge) and `tests/css-ownership-check.mjs` (`selectorBaselines` hard-codes per-file selector counts; update per merge). **`css-ownership-check.mjs` is currently GREEN** (verified 2026-07-23). The current blocker for `npm run check:ownership` is `tests/info-panel-surface-ownership-contract.mjs:145` (`SelectedBusinessDetails.svelte must own prefixed #selected-match-panel`) — a **parallel-session drift issue**, not a CSS ownership baseline issue. Steps 3–5 remain deferred until the parallel session settles AND all contract tests are green.
...
- [ ] **3. `css: merge chrome+narrow → mobile_premium__layout.css`** (update `vite.config.ts` link list) — **BLOCKED** (see §0). Requires updating `MOBILE_PREMIUM_SPLIT` in `tests/mobile-chrome-ownership-contract.mjs` and `selectorBaselines` in `tests/css-ownership-check.mjs` for the renamed/merged files. Currently blocked by `info-panel-surface-ownership-contract.mjs:145` (parallel session drift on `SelectedBusinessDetails.svelte`), not by CSS ownership baseline.
...
**Precondition to resume steps 3–6:** parallel session settled AND `tests/css-ownership-check.mjs` + `tests/info-panel-surface-ownership-contract.mjs` + `tests/cache-buster-check.js` green. Each merge must update the three hard-coded contract tests (§6) in the same commit as the CSS/link change, so the merge is verifiable.
