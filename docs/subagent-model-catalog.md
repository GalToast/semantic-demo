...
|---|---|---|---|---|
| 1 | `nvidia/moonshotai/kimi-k2.6` | Combined code + UI/screenshot scout for hard visual seams | Official Kimi post describes K2.6 as an open-source long-horizon coding model; NVIDIA Build describes the hosted route as native multimodal with image/video input support. Sources: <https://www.kimi.com/blog/kimi-k2-6>, <https://build.nvidia.com/moonshotai/kimi-k2.6/modelcard> | 2026-06-13 Pi/browser smoke: strong PNG critique of Semantic Explorer mobile focus-search. WEBM was not inspected, so do not call video support proven through Pi yet. |
| 2 | `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning` | Direct media-payload smoke candidate, not default Pi worker | NVIDIA Build says it unifies video, audio, image, and text understanding for Q&A, summarization, transcription, GUI/OCR, and rich enterprise content. Source: <https://build.nvidia.com/nvidia/nemotron-3-nano-omni-30b-a3b-reasoning/modelcard> | 2026-06-13 Pi/browser worker launched but produced garbled unusable output and no report. Test direct image/video payloads before assigning product QA. |
...
| 2026-06-13 | `nvidia/moonshotai/kimi-k2.6` | NVIDIA | Pi + browser profile | Semantic Explorer mobile focus-search screenshot critique from Playwright PNG + WEBM artifact | Completed; wrote `tmp/semantic-vision-input/2026-06-14T00-12-24-732Z/kimi-clip-critic.md` | Inspected PNG visually and used metadata; did not inspect WEBM video | High-signal static UI critique; correctly flagged focus-card overlap, ghost text bleed, and low-contrast canvas | Log-heavy, stdout hit 2MB retained cap; video capability not proven in this harness path | Screenshot/layout critique, especially paired with deterministic DOM checks | Strong |
| 2026-06-13 | `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning` | NVIDIA | Pi + browser profile | Same Semantic Explorer mobile screenshot/clip critique smoke | Launched but produced no usable report; canceled after terminal garbled output | Pi RPC launched and generated tokens, but no visible assistant report or tool output; no file written | None confirmed through Pi worker path | Garbled repetitive text, no report file, no usable visual critique despite official omni-modal card | Do not use as Pi worker for UI QA until a direct image/video payload smoke passes | Broken |
...
| `nvidia/moonshotai/kimi-k2.6` | yes | ✅ Strong (3 prior) | High-value scout; good code/UI/static-vision reasoning. 2026-06-13 PNG critique worked; WEBM video not proven through Pi. Use followup pattern. |
...
| `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning` | yes | ❌ Broken (1 prior) | Pi/browser screenshot critique smoke produced garbled unusable output and no report; direct media-payload smoke still needed |
