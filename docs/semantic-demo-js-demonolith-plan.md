...
`journey.js` calls `window.syncSemanticDiveUi`, `window.showExperienceToast`, `window.focusOnPoint`, `window.updateJourneyCompass`, etc. All are window-wired by lifecycle.js. To extract journey.js cleanly, these must become proper named imports.
