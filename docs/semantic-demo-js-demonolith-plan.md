...
`window.updateJourneyCompass`, `window.showExperienceToast`, runtime `window.syncSemanticDiveUi` callers, runtime `window.focusOnPoint` callers, and search-state `window.refreshCompositionState` callers were dewindowed 2026-05-25; callers now use direct named imports or leaf adapters from their owner modules. `window.syncSemanticDiveUi`, `window.focusOnPoint`, and `window.refreshCompositionState` remain as temporary compatibility bridges for tests/external callers. To extract `journey.js` cleanly, the remaining bridge calls must become proper named imports.
