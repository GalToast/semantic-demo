...
`window.updateJourneyCompass`, `window.showExperienceToast`, runtime `window.syncSemanticDiveUi` callers, and runtime `window.focusOnPoint` callers were dewindowed 2026-05-25; callers now use direct named imports from their owner modules. `window.syncSemanticDiveUi` and `window.focusOnPoint` remain as temporary lifecycle compatibility bridges for tests/external callers. To extract `journey.js` cleanly, the remaining bridge calls must become proper named imports.
