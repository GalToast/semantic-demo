...
| `.journey-compass-actions` / `.journey-compass-rail` | `css/journey_active.css` for phase and field-node choreography | `css/mobile_base.css`, the `css/mobile_premium__*.css` split (state, chrome), `css/strands.css`, and the existing `css/progressive_disclosure.css` actions modifier | `npm run check:ownership`, `npm run qa:contract:field-node`, `npm run qa:contract:compass-rail` |
