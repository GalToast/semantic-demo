...
| `.journey-compass` | `css/layout_base.css`, `css/journey_steps.css` | `css/journey_active.css`, `css/mobile_base.css`; duplicate mobile layout in `css/progressive_disclosure.css` was removed on 2026-05-19 | `css/mobile_premium.css` for collapsed final mobile compass variants across focus/dive, search, idle, map, and surface correction sections | `field-node` is live JS state. Do not delete `journey_active.css` field-node blocks as dead duplicates. | `01-mobile-idle`, `02-mobile-search-coffee`, `qa:contract:field-node`, `qa:contract:compass-rail`, mobile canvas-node focus proof |
...
| Mobile final overrides | n/a | n/a | `css/mobile_premium.css` collapsed single owner loaded directly by the app shell | Keep scoped to mobile/state selectors; avoid `!important`; verify desktop is untouched. | All mobile states plus `07-desktop-idle` |
...
|---|---|---|---|
| `.journey-compass-title` | `css/journey_active.css` for base/legacy phase variants | `css/mobile_base.css` for mobile base, `css/mobile_premium.css` for collapsed focus/dive/search/map/idle mobile variants, `css/strands.css` for legacy galaxy/mobile variants, `css/layout_base.css` for search base | `npm run check:ownership`, `npm run qa:contract:real-route`, affected `qa:contract` surfaces |
| `.journey-compass-actions` / `.journey-compass-rail` | `css/journey_active.css` for phase and field-node choreography | `css/mobile_base.css`, `css/mobile_premium.css`, `css/strands.css`, and the existing `css/progressive_disclosure.css` actions modifier | `npm run check:ownership`, `npm run qa:contract:field-node`, `npm run qa:contract:compass-rail` |
| `.journey-compass-action.primary` | `css/journey_active.css` for base primary action behavior | `css/mobile_premium.css` for focus/dive mobile sizing and final mobile touch-target harmonization, existing `css/mobile_base.css`, `css/search.css`, `css/strands.css`, and `css/animations.css` modifiers | `npm run check:ownership`, `npm run qa:contract:real-route` |
