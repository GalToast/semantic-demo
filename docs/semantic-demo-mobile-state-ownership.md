...
| `data-journey-phase` | `overview`, `search`, `focus`, `inside`, `map` | Journey compass phase rail. Currently a **read-only CSS input** - no CSS rules directly gate on this attribute; it is consumed by JS and by the journey compass widget's own `data-phase`. | None for `body[data-journey-phase]`; CSS reads `.journey-compass[data-phase]` instead. | `js/modules/lifecycle.js` (`document.body.dataset.journeyPhase = phase`), `js/modules/journey.js`, `js/modules/semantic-dive-ui.js` |
...
- `js/modules/journey.js` - reads `journeyPhase`
