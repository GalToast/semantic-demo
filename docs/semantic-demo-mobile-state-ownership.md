...
|---|---|---|---|---|
| `data-panel-surface` | `idle`, `search`, `focus`, `focus-search`, `semantic-dive`, `map-trail`, `map-focus`, `map-focus-search`, `map-search` | Primary panel/sheet visibility and layout. All non-map panels use this as the primary gate. Map variants gate map-specific chrome. | `css/layout_base.css`, `css/mobile_premium_state.css`, `css/journey_steps.css`, `css/journey_active.css`, `css/animations.css` | `js/modules/lifecycle.js` (`refreshCompositionState()` / `derivePanelSurface()`) |
| `data-panel-surface-detail` | `none`, `peek`, `expanded` | CSS-facing detail state for mobile search/focus-search drawer density. | `css/mobile_premium_state.css`; `css/mobile_premium_surfaces.css` must not read this state. | `js/modules/lifecycle.js` mirrors `data-mobile-search-sheet`; `js/modules/search-panel-adapter.js` owns the search-sheet interaction write |
| `data-focus-panel-mode` | `field-node`, `overview`, `manual-panel`, `manual-collapsed`, `legend-open` | Focus HUD density and temporary mobile panel submodes. `field-node` controls compact canopy/walk-dock styling; manual and legend values track user-opened panel states. | `css/journey_active.css` for legacy/active journey choreography; `css/mobile_premium_focus.css` for active mobile focus composition; `css/mobile_premium_surfaces.css` only for non-active field-node and late backstops; `css/progressive_disclosure.css` for legacy show/hide | `js/modules/lifecycle.js`, `js/modules/event-bindings.js`, QA scripts |
...
At `896x414`, `css/mobile_premium_focus.css` owns compact focus-stage suppression for nonessential `.focus-stage-meta` and `.focus-stage-badges` rows so the primary name, short description, and Step Inside action remain inside the viewport. `css/mobile_premium_chrome.css` owns the competing chrome lane at the same breakpoint: focus/semantic states hide canvas utility controls, share/help/panel toggles, weather/time widgets, and clipped compass note copy while preserving the view toggle.
...
`css/mobile_premium_state.css` owns both the drawer geometry and compact result presentation for these `peek`/`expanded` states. `css/mobile_premium_chrome.css` owns state-agnostic result chrome, and `css/mobile_premium_surfaces.css` is guarded against reintroducing `data-panel-surface-detail` selectors.
...
- Style owner: dedicated selected-map summary presentation lives in `css/mobile_premium_map_summary.css`; selected-card/map-state visibility lives in `css/mobile_premium_state.css`; final drawer geometry and suppression backstops live in `css/mobile_premium_surfaces.css`.
...
4. **Changing focus HUD density** -> edit `css/journey_active.css` for legacy field-node choreography and `css/mobile_premium_focus.css` for active mobile focus-search/semantic-dive composition. Use `css/mobile_premium_surfaces.css` only for non-active field-node fallback or unavoidable loaded-last backstops. Do not invent a new `data-focus-panel-mode` value without a corresponding JS writer and contract update.
