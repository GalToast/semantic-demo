...
- **Defender-proof (2026-08-17, measured):** MsMpEng CPU-delta was 34.5% of one core pre-throttle → **0.03 s / 60 s = 0.5%** post (exclusions + ScanAvgCPULoadFactor 20). The #1 hog → noise, banked.
- **jsdom landmine (2026-08-17):** vitest 4 hardcodes `jsdom/lib/api.js`; jsdom 29 dropped that layout (empty `lib/` broke the vitest env). Healed node_modules with jsdom@26.1.0 (`--no-save`, lib/api.js restored; app-init 23/23). **package.json still declares ^29.1.1 → next npm ci reinstalls the break.** When the lane touches package.json, pin <jsdom@26.x>. No tracked mutation made here (--no-save).
- **Simplex #1 landed (J52, 2026-08-17):** JS brotli/gzip precompression enabled (vite.config.ts COMPRESS_JS, mirrored CSS path). 37 .br twins in dist; measured cut: ~1,690,707 raw → 387,362 br = **77.1
- **Simplex #1 landed (J52, 2026-08-17):** JS brotli/gzip precompression enabled (vite.config.ts COMPRESS_JS, mirrored CSS path). 37 .br twins in dist; measured cut: 1,690,707 raw → 387,362 br = 77.1% transfer saved (~1,303 KB); .htaccess already serves .js.br (Content-Encoding lines 103-116) — live + local both benefit. Meter PASSED (uncompressed +1.2 KB unchanged).
- **qa-server gate wedge (2026-08-17):** `scripts/qa-server.mjs start` hangs mid-start (twice tonight; health probe never returns). LH GATE re-verification deferred to s2's wave-flip run or a calm retry. J52's twins verified by build+disk (37 .br files) — the transfer win stands on artifact inspection; browser-level re-gate is a TODO.
- **LH gate EPERM verdict (2026-08-17, 3 runs):** gate audits COMPLETE both presets ("No browser errors") but the final write phase dies every run on chrome-teardown EPERM (LH child can't be taskkilled → holds %TEMP%\lighthouse.* lock; TMPDIR/CHROME_USER_DATA_DIR workarounds bypassed since lighthouse uses os.tmpdir()). t3's seed day proves the flow works when no zombie chrome holds AppData\Local\Temp. Runbook for the NEXT clean attempt: pre-kill leftover chrome children, then run; fallback = run gate on the CI-less other-end lane. Transfer win itself is PROVEN: 1,270,657 → 312,804 B (75.4%) via Content-Encoding: br negotiation (curl-verified). J52 = released already (5ec714a5).
