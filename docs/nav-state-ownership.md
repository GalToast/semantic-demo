...
| `focusPocketIndices` | `number[]` | `pocket.ts`, `focus-pocket.ts` | inline `focusStore.update()` + direct writes | `compass-state.ts`, `parity-attrs.svelte.ts` | **Not using writeNavStateMirror** (focusStore has no PocketMeta helpers; inline mirror pattern from `withFocusNotify`) |
| `focusPocketRoleByIndex` | `Map<number, string>` | `pocket.ts`, `focus-pocket.ts` | inline `focusStore.update()` | `compass-state.ts` | Same as focusPocketIndices |
| `focusPocketMeta` | `FocusPocketMeta \| null` | `pocket.ts:setFocusPocketMeta` | inline `focusStore.update()` | `camera-choreography/focus.ts:viewportProfile` | |
...
- `writeNavStateMirror(patch: Partial<NavState>)` — `src/lib/stores/navigation.svelte.ts:395-403`. The canonical batch helper. Writes to both stores.
...
- **focusStore mirror for `focusPocketIndices` / `focusPocketRoleByIndex` / `focusPocketMeta`** uses inline `focusStore.update(...)` calls rather than a `writeFocusPocketMirror` helper. The pattern is consistent (`withFocusNotify` style) but a `writeFocusPocketMirror(patch)` helper would be cleaner. Not blocking.
- **The pre-bundled `dist/svelte/assets/panel-bindings-*.js`** no longer exists in the new build (consolidated into `index-BxniA3vw.js`). The W15 deeper parity-attrs gap is now fully resolved at both the dev (5175) and preview (4174) levels.
