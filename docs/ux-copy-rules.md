...
User-facing copy in Semantic Explorer must not leak engineering jargon. Words like "semantic", "node", "cluster", "signal", "thread", and "mycelium" describe *how the dataset is wired up internally* — end-users do not need to know that to find a plumber in Conroe. They make copy that should be inviting instead feel like database documentation.
