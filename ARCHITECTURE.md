...
- **State ownership** lives in `src/lib/state/app.svelte.ts` (+ `session.svelte.ts`, `with-state-mutation.ts`). Former `legacy-state.{ts,adapter.ts}` compat shims were **deleted** in `062b0035` (P1-F LegacyState → AppState reconciliation); state readers now route through app.svelte.ts. reads via `unknown` so consumers don't have to do `appState as any`. 9 importers each. Both are durable compat seams, **not** part of the bridge-retirement arc — see `docs/migration-plan.md` § "Compat shims".
- **`src/lib/state/{create-state-mirror,with-state-mutation,state-validation,session.svelte}.ts`** — Mirror discipline, mutator helpers, validation, persistence. (Former `mutators.ts` module was folded into these during the dead-code remediation `9a3f85828`.)
