  ...
  req.on('end', async () => {
    try {
      if (req.method === 'GET' && req.url.startsWith('/v1/models')) {
        res.end(JSON.stringify({ object: 'list', data: MODELS.map(m => ({ id: m.id, object: 'model', owned_by: 'cline-free' })) }))
        return
      }
...
server.listen(PORT, '127.0.0.1', () => console.log(`[cline-shim] listening on http://127.0.0.1:${PORT}/v1 (models: ${MODELS.map(m => m.id).join(', ')})`))